...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr>
			<th><label for="scdSe">${title}</label> <span class="pilsu">*</span></th>
			<td class="left">
			    <c:forEach items="${scdSe}" var="schdulSeInfo" varStatus="status">
			    <c:if test="${schdulSeInfo.code eq resultList.scdSe}">
		...
		<tr>
			...
			<td class="nopd">
			    <c:forEach items="${scdIpcrCode}" var="schdulSeInfo" varStatus="status">
			    <c:if test="${scdulSeInfo.code eq resultList.scdIpcrCode}">
			     <c:out value="${fn:replace(schdulSeInfo.codeNm , crlf , '<br/>')}" escapeXml="false" />
			    </c:if>
			    </c:forEach>
			</td>
		...
		<tr>
			...
			<td class="nopd">
				<c:out value="${fn:replace(resultList.scdNm , crlf , '<br/>')}" escapeXml="false" />
			</td>
		</tr>
		<!-- 일정내용 -->
		...
		<tr>
			<th><label for="infoProvdAgreCn">${title}</label> <span class="pilsu">*</span></th>
			<td class="nopd">
				<c:out value="${fn:replace(resultList.scdDesc , crlf , '<br/>')}" escapeXml="false" />
			</td>
		</tr>
		<!-- 반복구분 -->
		...
		<tr>
			...
			<td class="nopd">
			    ...
			    <c:if test="${schdulSeInfo.code eq resultList.reptitSeCode}">
			     <c:out value="${fn:replace(schdulSeInfo.codeNm , crlf , '<br/>')}" escapeXml="false" />
			    </c:if>
			    </c:forEach>
			</td>
		...
		<tr>
			<th><label for="infoProvdAgreCn">${title}</label> <span class="pilsu">*</span></th>
			<td class="nopd">
				${fn:substring(resultList.scdBgnde, 0, 4)}-${fn:substring(resultList.scdBgnde, 4, 6)}-${fn:substring(resultList.scdBgnde, 6, 8)} ${fn:substring(resultList.scdBgnde, 8, 10)}시  ${fn:substring(resultList.scdBgnde, 10, 12)}분 ~
    			${fn:substring(resultList.scdEndde, 0, 4)}-${fn:substring(resultList.scdEndde, 4, 6)}-${fn:substring(resultList.scdEndde, 6, 8)} ${fn:substring(resultList.scdEndde, 8, 10)}시  ${fn:substring(resultList.scdEndde, 10, 12)}분
			</td>
		</tr>
		<!-- 첨부파일 -->
	...
	<div class="btn">
		...
		<form name="IndvdlSchdulManageForm" id="IndvdlSchdulManageFormh" action="<c:url value='/schedule/Modify.do'/>" method="post" style="float:left;">
		<input type="submit" class="s_submit" value="<spring:message code="button.update" />" title="<spring:message code="title.update" /> <spring:message code="input.button" />" />
		<input name="scdId" type="hidden" value="${resultList.scdId}">
		<input name="linkType" type="hidden" value="${sLinkType}">
		<input name="cmd" type="hidden" value="<c:out value=''/>"/>
		</form>

		<form name="formDelete" action="<c:url value='/schedule/Detail.do'/>" method="post" style="float:left; margin:0 0 0 3px;">
		...
		<form name="formSubDiaryManage" action="<c:url value='/schedule/List.do'/>" method="post" style="float:left; margin:0 0 0 3px;">
		...
		<form name="formList" action="<c:url value='/schedule/List.do'/>" method="post" style="float:left; margin:0 0 0 3px;">
