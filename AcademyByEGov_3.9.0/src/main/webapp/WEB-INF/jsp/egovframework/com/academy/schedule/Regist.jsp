...
function fn_egov_init_IndvdlSchdulManage(){
	...
	//------------------------- 첨부파일 등록 End
	   ...
	   if("${ScheduleVO.scdBgnde}".length > 0){
		   var scdBgnde = "${ScheduleVO.scdBgnde}";
		   document.getElementById("scdBgndeYYYYMMDD").value = scdBgnde.substring(0,4) + "-" + scdBgnde.substring(4,6) + "-" + scdBgnde.substring(6,8);
	   }

	   if("${ScheduleVO.scdEndde}".length > 0){
		   var scdEndde = "${ScheduleVO.scdEndde}";
		   document.getElementById("scdEnddeYYYYMMDD").value = scdEndde.substring(0,4) + "-" + scdEndde.substring(4,6) + "-" + scdEndde.substring(6,8);
	   }
}
/* ********************************************************
 * 목록 으로 가기
...
function fn_egov_list_IndvdlSchdulManage(){
	location.href = "<c:url value='/schedule/List.do' />";
}
/* ********************************************************
 * 저장처리화면
 ******************************************************** */
function fn_egov_save_IndvdlSchdulManage(form){
	...
	if(confirm("<spring:message code="common.save.msg" />")){
		var scdBgndeYYYYMMDD = document.getElementById('scdBgndeYYYYMMDD').value;
		var scdEnddeYYYYMMDD = document.getElementById('scdEnddeYYYYMMDD').value;

		form.scdBgnde.value = scdBgndeYYYYMMDD.replaceAll('-','') + fn_egov_SelectBoxValue('scdBgndeHH') +  fn_egov_SelectBoxValue('scdBgndeMM');
		form.scdEndde.value = scdEnddeYYYYMMDD.replaceAll('-','') + fn_egov_SelectBoxValue('scdEnddeHH') +  fn_egov_SelectBoxValue('scdEnddeMM');
...
<form:form commandName="ScheduleVO" action="${pageContext.request.contextPath}/schedule/insert.do" name="ScheduleVO" method="post" enctype="multipart/form-data" onSubmit="fn_egov_save_IndvdlSchdulManage(document.forms[0]); return false;"> 
 <div class="wTableFrm">
	<!-- 타이틀 -->
	...
	<tbody>
		...
		<tr>
			...
			<td class="nopd">
				<form:select path="scdSe">
					<form:option value="" label="${inputSelect}"/>/>
					<form:options items="${scdSe}" itemValue="code" itemLabel="codeNm"/>
				</form:select>
				<div><form:errors path="scdSe" cssClass="error"/></div>
			</td>
		</tr>
		<!-- 중요도 -->
		...
		<tr>
			...
			<td class="nopd">
		        <form:select path="scdIpcrCode">
		            <form:option value="" label="선택"/>
		            <form:options items="${scdIpcrCode}" itemValue="code" itemLabel="codeNm"/>
		        </form:select>
		        <div><form:errors path="scdIpcrCode" cssClass="error"/></div>
			</td>
		</tr>
		<!-- 부서 -->
		<form:hidden path="scdDeptName" />
		<form:hidden path="scdDeptId" />
		<!-- 일정명 -->
		...
		<tr>
			...
			<td class="nopd">
				<form:input path="scdNm" size="73" cssClass="txaIpt" maxlength="255" />
				<div><form:errors path="scdNm" cssClass="error"/></div>
			</td>
		</tr>
		<!-- 일정내용 -->
		...
		<tr>
			<th><label for="infoProvdAgreCn">${title}</label> <span class="pilsu">*</span></th>
			<td class="nopd">
				<form:textarea path="scdDesc" rows="3" cols="20" cssClass="txaClass"/>
				<div><form:errors path="scdDesc" cssClass="error"/></div>
			</td>
		</tr>
		<!-- 반복구분 -->
		...
		<tr>
			<th><label for="infoProvdAgreCn">${title}</label> <span class="pilsu">*</span></th>
			<td class="nopd">
				<form:input path="scdBgndeYYYYMMDD" size="10" readonly="true" maxlength="10" style="width:70px;"/>
				&nbsp;&nbsp;~&nbsp;&nbsp;
				<form:input path="scdEnddeYYYYMMDD" size="10" readonly="true" maxlength="10" style="width:70px;"/>
				&nbsp;
				
        <form:select path="scdBgndeHH">
            <form:options items="${scdBgndeHH}" itemValue="code" itemLabel="codeNm"/>
        </form:select><spring:message code="comCopSmtSdm.regist.schdulYYYY"/> <!-- 시 -->
        <form:select path="scdBgndeMM">
            <form:options items="${scdBgndeMM}" itemValue="code" itemLabel="codeNm"/>
        </form:select><spring:message code="comCopSmtSdm.regist.schdulMM"/> <!-- 분 -->
		~
        <form:select path="scdEnddeHH">
            <form:options items="${scdEnddeHH}" itemValue="code" itemLabel="codeNm"/>
        </form:select><spring:message code="comCopSmtSdm.regist.schdulYYYY"/> <!-- 시 -->
        <form:select path="scdEnddeMM">
            <form:options items="${scdEnddeMM}" itemValue="code" itemLabel="codeNm"/>
        </form:select><spring:message code="comCopSmtSdm.regist.schdulMM"/> <!-- 분 -->
			</td>
		</tr>
		<!-- 담당자 -->		
		<form:hidden path="scdChargerName" />
		<form:hidden path="scdChargerId" />
		<!-- 첨부파일 -->
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/schedule/List.do' />"  title="<spring:message code="button.list" />  <spring:message code="input.button" />"><spring:message code="button.list" /></a></span><!-- 목록 -->
	</div><div style="clear:both;"></div>
	
</div><!-- div end(wTableFrm)  -->

<input name="cmd" id="cmd"type="hidden" value="<c:out value='save'/>"/>
<input type="hidden" name="scdKindCode" id="scdKindCode" value="2" />
