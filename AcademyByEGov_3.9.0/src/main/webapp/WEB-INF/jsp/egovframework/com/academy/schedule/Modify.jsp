...
<!--  상단타이틀 -->
<form:form commandName="ScheduleVO" name="ScheduleVO" action="${pageContext.request.contextPath}/schedule/update.do" method="post" enctype="multipart/form-data" onSubmit="fn_egov_save_IndvdlSchdulManage(document.forms[0]); return false;"> 
<div class="wTableFrm">
	<!-- 타이틀 -->
	...
	<tbody>
		...
		<tr>
			...
			<td class="nopd">
			 <!--첨부목록을 보여주기 위한 -->
			<c:if test="${ScheduleVO.atchFileId ne null && indvdlSchdulManageVO.atchFileId ne ''}">
			<c:import charEncoding="utf-8" url="/cmm/fms/selectFileInfsForUpdate.do" >
			<c:param name="param_atchFileId" value="${ScheduleVO.atchFileId}" />
			</c:import>
			</c:if>
  			
  			 <!-- 첨부화일 업로드를 위한 Start -->
			<c:if test="${ScheduleVO.atchFileId eq null || ScheduleVO.atchFileId eq ''}">
			<input type="hidden" name="fileListCnt" value="0" />
			<input name="atchFileAt" type="hidden" value="N">
			</c:if>
			<c:if test="${ScheduleVO.atchFileId ne null && ScheduleVO.atchFileId ne ''}">
...
<form:hidden path="scdId" />
<form:hidden path="scdKindCode" />
...
<input type="hidden" name="returnUrl" value="<c:url value='/schedule/Modify.do'/>"/>
</form:form>
