...
<HEAD>
	...
	* 주관 부서 팝업창열기
	******************************************************** */
	function fn_egov_detail_DeptSchdulManage(scdId){
	...
	function fnEgovSchdulSe(setValue) {
		location.href="<c:url value='/schedule/WeekList.do' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}

	function fnEgovSchdulTodate() {
		location.href="<c:url value='/schedule/WeekList.do' />?searchCondition=SCHDUL_SE&searchKeyword=" + document.deptSchdulManageVO.schdulSe.value;
	}
...
%>
  <tr>
	...
	for(int j=0;j < listResult.size(); j++){
		...
		int iBeginDate = Integer.parseInt( ((String)egovMap.get("scdBgnde")).substring(0, 8) );
		int iBeginEnd = Integer.parseInt( ((String)egovMap.get("scdEndde")).substring(0, 8) );

		if(iUseDate >= iBeginDate && iUseDate <= iBeginEnd){
		out.print("<table><tr><td nowrap><div>");
		%>
		<a href="<c:url value='/schedule/Detail.do' />?scdId=<%=(String)egovMap.get("scdId")%>" target="_parent">
		<%
		out.print( ((String)egovMap.get("scdBgnde")).substring(8,10) +"시");
		out.print( ((String)egovMap.get("scdBgnde")).substring(10,12) +"분<br/>~");
		out.print( ((String)egovMap.get("scdEndde")).substring(8,10) +"시");
		out.print( ((String)egovMap.get("scdEndde")).substring(10,12) +"분 ");
