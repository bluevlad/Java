...
<HEAD>
	...
	var gOpener = parent.document.all? parent.document.ScheduleVO : parent.document.getElementById("ScheduleVO") ;
	/* ********************************************************
	* 주관 부서 팝업창열기
	******************************************************** */
	function fn_egov_regist_IndvdlSchdulManage(sDate){
		...
		gOpener.scdBgnde.value = sDate;
		gOpener.scdEndde.value = sDate;
		gOpener.action = "<c:url value='/schedule/Regist.do' />";
		gOpener.submit();
	}
	...
	* 주관 부서 팝업창열기
	******************************************************** */
	function fn_egov_detail_IndvdlSchdulManage(scdId){

		gOpener.scdId.value = scdId;
		gOpener.action = "<c:url value='/schedule/Detail.do' />";
		gOpener.submit();
	}
	...
	function fnEgovSchdulSe(setValue) {
		location.href="<c:url value='/schedule/MonthList.do' />?year=<%=year%>&month=<%=month%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}
	
	function fnEgovSchdulTodate() {
		location.href="<c:url value='/schedule/MonthList.do' />?year=<%=sTodate.substring(0, 4)%>&month=<%=Integer.valueOf(sTodate.substring(4, 6))-1%>&searchCondition=SCHDUL_SE&searchKeyword=" + document.ScheduleVO.scdSe.value;
	}
...
<div class="sort_area">
	<div class="select_group">
     	<select name="scdSe" class="select" id="scdSe" onChange="fnEgovSchdulSe(document.forms[0].schdulSe.options[document.forms[0].schdulSe.selectedIndex].value);" title="일정구분선택">
	...
	<div class="date_view" style="left:39%;">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/schedule/MonthList.do' />?year=<%=year-1%>&amp;month=<%=month%>">prev</a></li>
			<li class="date"><%=year%> <spring:message code="comCopSmtSim.Navi.Year"/></li><!-- 년 -->
			<li><a class="next" href="<c:url value='/schedule/MonthList.do' />?year=<%=year+1%>&amp;month=<%=month%>">next</a></li>
			<li class="date">&nbsp;</li>
			
			<%if(month > 0 ){ %>
			<li><a href="<c:url value='/schedule/MonthList.do' />?year=<%=year%>&month=<%=month-1%>" title="<spring:message code="comCopSmtSim.goPrevMonth"/>" class="prev">prev</a></li><!-- 이전달로 가기 -->
			<%}%>
			<li class="date"><%=month+1%> <spring:message code="comCopSmtSim.Navi.Month"/></li><!-- 월 -->
			<%if(month < 11 ){ %>
			<li><a href="<c:url value='/schedule/MonthList.do' />?year=<%=year%>&month=<%=month+1%>" class="next" title="<spring:message code="comCopSmtSim.goNextMonth"/>">next</a></li><!-- 다음달로 가기 -->
			<%}%>
		</ul>
	</div>
</div>
			...
			for(int index = 1; index <= endDay; index++) {
				...
				if(listResult != null){   
					for(int i=0;i < listResult.size(); i++){
						...
						iBeginDate = Integer.parseInt(((String)egovMap.get("scdBgnde")).substring(0, 8));
						iBeginEnd = Integer.parseInt(((String)egovMap.get("scdEndde")).substring(0, 8));
		...
		for(int i=0;i < listResult.size(); i++){
			...
			if(iUseDate >= iBeginDate && iUseDate <= iBeginEnd){
				...
				if(iSchdulWeekFirest == 1 
					...
					out.print("<div class='divWeekBar' style='margin-top:"+(iDayCount*20)+"px;position:absolute;width:"+iDotTextWidth+"px;border:solid 0px;background-color:"+ egovMap.get("schdulWeekColor") +";'><a href=\"JavaScript:fn_egov_detail_IndvdlSchdulManage('" + (String)egovMap.get("scdId") + "')\" class='linkWeek'>");
					out.print((String)egovMap.get("scdNm"));
					out.println("</a></div>");
				}				
				//일별스케줄갯수
				iDayCount++;				
