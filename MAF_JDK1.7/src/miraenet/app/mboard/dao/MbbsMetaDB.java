...
import miraenet.app.mboard.beans.MbbsGroupBean;
...
public class MbbsMetaDB {
	...
	public MbbsMetaBean view(MdbDriver oDb, String v_BID) throws MdbException {
        ...
        if (v_BID != null) {
            String sql = " SELECT m.bid, m.subject, m.cnt_t, m.cnt_w, m.grp, m.cnt_d, m.nrows, m.fl_write,"
                    ...
                    + "  WHERE m.bid = :v_bid   AND m.grp = g.grp";
	 ...
	 * @throws SQLException
	 */
	public int insert(MdbDriver oDb, MbbsMetaBean ob) 
	throws Exception
	{
	    int affected_rows = -1;
...
//	    	  + " nrows,  fl_write, fl_read, fl_list, fl_notice, fl_auth, " 
//	    	  + " fl_reply, fl_comment, fl_modify_date, number_attach, max_attach_size, " 
//	    	  + " fl_board_type, category, " 
//	    	  + " pgid, admin_usn, ext_usersel, fl_html, fl_category, " 
//	    	  + " fl_waste, max_attach_limit,  club_id, leccode," 
...
//	    	  + ") ";
	    ...
	    final String sql = "INSERT INTO MBBS_META (" 
	    	  ...
	    	  + " pgid, admin_usn, ext_usersel, fl_html, fl_category, " 
	    	  + " fl_waste, max_attach_limit, header, club_id, leccode, " 
	    	  + " file_header, file_footer" 
	    	  + "  " 
	    	  + " ) "
	    	  + " VALUES ("
...
//	    	  + " WHERE  bid = :bid ";
	...
	throws Exception {
		...
		if(chk) {
		    ...
		    throw new MBoardException("게시판 삭제중 오류가 발생하였습니다1.!!!" );
		}
	}
    ...
    public List getBbsMetaBeans(MdbDriver oDb,  String v_grp, String v_srch) {
        List ListItem = null;
        String sql = " SELECT    bid, subject, cnt_t, cnt_w, grp, cnt_d, nrows, last_index, fl_write,"
				+ "          fl_auth, fl_read, fl_list, fl_notice, fl_reply, fl_comment, fl_modify_date, number_attach,"
				+ "          max_attach_size, fl_board_type, total_attach_size, pgid, is_use, admin_usn," 
				+ " leccode,"
				+ "			m.grant_list, m.grant_view, m.grant_write, "
                + "       m.grant_comment, m.grant_reply, m.grant_delete, m.grant_html, m.grant_notice, "
                + "       m.grant_secret "				
