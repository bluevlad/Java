...
public class MdbOCI8 extends MdbDriver {
	...
	public CallableStatement prepareProcedure(String procedureName, MdbParameters p) throws MdbException {
	 ...
	 * 이후에 필히 Commit을 해줄껏.
	 */
	public void updateCLOB(String tableName, String lobColumnName, String clobString, MdbParameters p) throws MdbException {
		boolean oldCommit = true;
		String sql = "Select " + lobColumnName + " from " + tableName + " where ";
		String updateSql = "UPDATE " + tableName + " SET "+ lobColumnName + " = empty_clob() where ";
...
//			oRS.close();
			...
			try {super.conn.rollback();} catch (SQLException ex) {}
			...
			if (oRS != null)try {oRS.close();} catch (SQLException ex) {}
			if (rs != null)try {rs.close();} catch (SQLException ex) {}
			if (stmt != null)try {stmt.close();} catch (SQLException ex) {}
			try {super.conn.setAutoCommit(oldCommit);} catch (SQLException ex) {}
			oRS = null;
			rs = null;
			stmt = null;
		}
	...
	public boolean updateBLOB(String table, String lobColumnName, File file, MdbParameters p) throws MdbException {
		String sql = "Select " + lobColumnName + " from " + table + " where ";
		String updateSql = "UPDATE " + table + " SET "+ lobColumnName + " = empty_blob() where ";
		...
		try {
			...
			if(oldCommit) {
				...
				if (oRS.next()) {
					chk = setBlob(oRS, file, lobColumnName);
				}
			}
		} catch (Exception e3) {
			try {super.conn.rollback();} catch (SQLException ex) {}
	 ...
	 * @throws IOException
	 */
	private synchronized boolean setBlob(OracleResultSet rs, File file, String fieldName)
	throws MdbException {
...
//                System.out.print("#");
			...
			if(instream != null) { try {instream.close();} catch ( Throwable e) {}}
			if(outstream != null) { try {outstream.close();} catch ( Throwable e) {}}
			instream = null;
			outstream = null;
		}
		return chk;
	...
	protected void moveCursor(ResultSet rs) throws SQLException {
		...
		logging(" Move to Row : " + offset + ", Page:" + page + ", fetchSize : " + fetchSize + ",count :" + count);
