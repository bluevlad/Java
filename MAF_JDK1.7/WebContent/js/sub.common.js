...
function open_msgbox() {
	 var oWin = window.open(CPATH+'/pager/default.do','msg_win','width=520,height=610,status=yes,menubars=no,scrollbars=no,top=0px,left=0px');
	windows_focus(oWin);
}
...
function ZipCodeSeach(formName, param) {
	var urlname = CPATH + "/etc/Zipcode.do?form="+formName+"&"+param;
	var oWin = window.open(urlname,
		"zipcodewin",
		"top=100px,left=100px,status=yes,resizable=no,menubar=no,scrollbars=no,width=100,height=100");
	windows_focus(oWin);
...
//  }
	...
	* 시험 시작 
	*/
	function eTestGo(exmid, lecnumb) {
	var urlname = CPATH + "/etest/doTest.do?exmid="+exmid +"&lecnumb=" + lecnumb;
