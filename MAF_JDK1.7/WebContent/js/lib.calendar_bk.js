...
PopUpCalendarClass = function() {
    ...
    this.imgDir = CPATH+'/js/lib.calendar/'; // 이미지 디렉토리 - ./ : 현재 디렉토리
...
PopUpCalendarClass.prototype.hideElement=function(elmID, overDiv) {
	if(this.ie) {
...
PopUpCalendarClass.prototype.showElement=function(elmID) {
	if(this.ie) {
		for(i = 0; i < document.all.tags(elmID).length; i++) {
			obj = document.all.tags( elmID )[i];
...
PopUpCalendarClass.prototype.popUpMonth=function () {
	...
	this.hideElement('APPLET', document.getElementById("calSelectMonth"));
}
...
PopUpCalendarClass.prototype.constructCalendar=function () {
	...
	if(this.showWeekNumber==1) {
        ...
        sHTML += "<img src='"+this.imgDir+"divider.gif' width=1 border=0></td>";
	}
...
if(popCalObj.dom) {
	...
	document.write ("<div onclick='popCalObj.bShow=true' id='calCalendar' style='padding:0;margin:0;position:absolute;z-index:+999;visibility:hidden;'>");
...
/*
			while (aTag)
			{
				...
				if(aTag.tagName == "BODY" || aTag.tagName == "DIV") {
					aTag = null;
				}
			}
*/			
			...
			popCalObj.crossobj.left = fixedX==-1 ? ctl.offsetLeft	+ leftpos :	fixedX;
			popCalObj.crossobj.top = fixedY==-1 ?	ctl.offsetTop +	toppos + ctl.offsetHeight +	2 :	fixedY;
//			popCalObj.left = fixedX==-1 ? ctl.offsetLeft + leftpos : fixedX;
//			popCalObj.top = fixedY==-1 ? ctl.offsetTop + toppos + ctl.offsetHeight + 2 : fixedY;
					
			alert(popCalObj.crossobj.name + ", class : " + leftpos + " x " +  toppos + "," + ctl.offsetLeft + "," + ctl.offsetTop + " | " +
				popCalObj.crossobj.left + "," +  popCalObj.crossobj.top);
...
//			popCalObj.hideElement('SELECT', document.getElementById("calCalendar"));
			popCalObj.hideElement('APPLET', document.getElementById("calCalendar"));
...
//document.onkeypress = popCalObj.lw_escLayer;
//document.onclick = popCalObj.lw_closeAllLayers;
