...
function forminit(select_form,cat_cd_param,crs_cd_param,prtcode_param,leccode_param)
{
...
function set_leccode()
{
	...
	var select_cat_cd = form.cat_cd[form.cat_cd.selectedIndex].value;
	var select_crs_cd = form.crs_cd[form.crs_cd.selectedIndex].value;
	var select_sjt_cd = form.prtcode[form.prtcode.selectedIndex].value;
    form.leccode.length=0;
	
	for(j=0 ; j < COUNT ; j++){
	    if(j==0) form.leccode.options[i++]=new Option("--Class선택--","");
		if( (INFO_1[j] == select_cat_cd) && (INFO_3[j] == select_crs_cd) && (INFO_5[j] == select_sjt_cd) && !chk_leccode(INFO_7[j]) ) {   // name, code
	...
	if(form.leccode.selectedIndex < 1)
