...
public class <c:out value="${DaoClassName}"/> extends BaseDAO {
	...
	public static void getList(MdbDriver oDb, NavigatorInfo navigator, Map param, String sWhere, boolean isAll) throws MdbException {
		...
		final String sql = " select <c:forEach var="item" items="${listColumnsList}" varStatus="status"><mh:out value="${item.name}" case="lower"/><c:if test="${!status.last}">, </c:if></c:forEach> " +
	...
	private static long getRecordCount(MdbDriver oDb, Map param, String sWhere) throws MdbException {
		final  String sql = "select count(*) from <c:out value="${tabsname}"/> " + sWhere;
	...
	public static Map getOne(MdbDriver oDb, Map param) throws MdbException {
		final String sql = "select <c:forEach var="item" items="${selColumns}" varStatus="status"><mh:out value="${item.name}" case="lower"/><c:if test="${status.count != ColumnCnt}">, </c:if></c:forEach> " 
				+ "from <c:out value="${tabsname}"/> " 
	...
	public static int updateOne(MdbDriver oDb, Map param) throws MdbException {
		final String sql = " update <c:out value="${tabsname}"/> set "   +
			<c:forEach var="item" items="${selColumns}" varStatus="status">"<mh:out value="${item.name}" case="lower"/> = :<mh:out value="${item.name}" case="lower"/>  ,  " +
			</c:forEach>
			" update_dt = sysdate, " +
