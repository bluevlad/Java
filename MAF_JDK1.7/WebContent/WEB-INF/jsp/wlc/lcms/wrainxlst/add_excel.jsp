...
<!--
  ...
  function frmSubmit(frmName) {
    ...
    if(frm.excel_file.value=='') {
        alert("<mfmt:message bundle="table.wra_inx_lst" key="script.msg10" param="*.xls, *.csv"/>");
...
<mf:form action="${control_action}" method="post" name="myform" id="myform"  onSubmit=" frmSubmit('myform');return false;"  enctype="multipart/form-data">
    ...
    <tr>
        <td colspan="4" height="28" class="view"><mfmt:message bundle="common.message" key="message.reqbatch.info"/><mfmt:message bundle="common.message" key="message.reqbatch.info1"/><br><mfmt:message bundle="table.wra_inx_lst" key="script.msg10" param="*.xls, *.csv"/>[<a href="<mh:out value="${CPATH}"/>/contents/sample_content.zip" target="_new"><mfmt:message bundle="common.message" key="message.reqbatch.info3"/></a>]
        </td>
    </tr>
    <tr>
        <th height="28" class="view"><mfmt:message bundle="table.wra_inx_lst" key="batchupload"/></th>
