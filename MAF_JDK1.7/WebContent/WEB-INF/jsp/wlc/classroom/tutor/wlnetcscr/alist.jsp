...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        ...
        <tr>
          ...
          <th class="list"><mfmt:message bundle="table.Wlaratmst" key="wla_rat_mst.ratetc.score"/><mform:THSort id='attscore'/></th>
          <th class="list BR">
     ...
     <c:forEach var="item" items="${navigator.list}" varStatus="status">
        <tr >
           <td class="list" align="center"><input type="checkbox" name="userid_chk" value="${item.userid}#${item.reqnumb}" class="checkbox"  hname="<mfmt:message bundle="common.scripts" key="script.alert.userid"/>"></td>
...
<sql:setDataSource dataSource="jdbc/ehrd"/>
<sql:query var="entries" sql="SELECT NVL(brc.bbsread,0) AS bbsread, NVL(brc.bbswrite,0) AS bbswrite FROM wlg_adm_smc smc, (SELECT * FROM wlg_crs_brc WHERE leccode = ? AND usrcode = ? AND reqnumb = ?) brc WHERE smc.leccode=brc.leccode(+) AND smc.smccode=brc.smccode(+) AND smc.smcflag='Y' AND smc.smctype<>'0' AND smc.leccode = ? ORDER BY smc.smcalgn ASC">
<sql:param value="${leccode}"/>
<sql:param value="${item.userid}"/>
<sql:param value="${item.lecnumb}"/>
<sql:param value="${leccode}"/>
</sql:query>
          <c:forEach var="row" items="${entries.rows}">
              <td class="list" align=center>     
            <c:out value="${row.bbsread}"/>/<c:out value="${row.bbswrite}"/>
              </td>            
          </c:forEach>
           <td class="list" align=center>
           <input size=3 maxlength=3 name="${item.userid}_${item.reqnumb}_attscore" value="${item.attscore}" onKeyUp="checkFloat(this);" onkeyPress="if ((event.keyCode<48) || (event.keyCode>57)) event.returnValue=false;"><bean:message key="table.score.title"/>
           </td>
           <td class="list" align=center>
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td align="center"><a href="javascript:updateScore(document.myform);"><mfmt:button bundle="button" key="button.app.attscore"/></a>
