...
import web.egov.util.DateUtil;
import web.egov.util.StringUtil;
...
public class BoardController {
    ...
    public String boardList(ModelMap model, HttpServletRequest request) throws Exception {
        ...
        params.put("SEARCHBOARD_TYPE",CommonUtil.isNull(request.getParameter("SEARCHBOARD_TYPE"), ""));
        ...
        params.put("searchTeacher", CommonUtil.isNull(request.getParameter("searchTeacher"), ""));
        params.put("searchLearningCD", CommonUtil.isNull(request.getParameter("searchLearningCD"), ""));
        ...
        if("NOTICE_024".equals(params.get("BOARD_MNG_SEQ"))) {
            model.addAttribute("catSubject", subjectservice.findSubjectCategoryList(params));
            model.addAttribute("teacherList", teacherservice.findTeacherList(params));
            return "board/liveBoardList";
        } else {
    ...
    public String boardWrite(ModelMap model, HttpServletRequest request) throws Exception {
        ...
        model.addAttribute("catSubject", subjectservice.findSubjectCategoryList(params));
        model.addAttribute("teacherList", teacherservice.findTeacherList(params));
        model.addAttribute("rankList", boardService.getRankCode(params));
...
//        String subPath = "board/";
        ...
        if(null == params.get("SEARCHONOFFDIV") || "".equals(params.get("SEARCHONOFFDIV").toString())
            ...
            for (int i = 0; i < code_arr.length; i++) {
                ...
                boardTestClassMap.put("SUBJECT_CD", request.getParameter("SUBJECT_CD"));
                boardTestClassMap.put("PRF_ID", request.getParameter("PRF_ID"));
                boardTestClassMap.put("LEARNING_CD", request.getParameter("LEARNING_CD"));
                
                boardService.insertBoardCatInfo(boardTestClassMap);
            }
        }else if("T".equals(params.get("SEARCHONOFFDIV").toString())){
        	HashMap<String, Object> boardTestClassMap = new HashMap<String, Object>();
        	
        	boardTestClassMap.put("BOARD_MNG_SEQ", params.get("BOARD_MNG_SEQ"));
            boardTestClassMap.put("BOARD_SEQ", params.get("BOARD_SEQ"));
            boardTestClassMap.put("CATEGORY_CODE", "081");
                ...
                if(replayData.size()  > 0 ){
                    for (int k = 0; k < replayData.size(); k++) {
                        ...
                        for (int i = 0; i < code_arr.length; i++) {
                            ...
                            boardTestClassMap2.put("SUBJECT_CD", request.getParameter("SUBJECT_CD"));
                            boardTestClassMap2.put("PRF_ID", request.getParameter("PRF_ID"));
                            boardTestClassMap2.put("LEARNING_CD", request.getParameter("LEARNING_CD"));
...
//        String subPath = "board/";
	...
	*/
    ...
    private void setParam(HashMap params, HttpServletRequest request, ModelMap model) throws Exception {
        ...
        params.put("ARR_CATEGORY",CommonUtil.isNull(request.getParameter("ARR_CATEGORY"), "")); //학원자료게시판일때만
        if(!"".equals(params.get("ARR_CATEGORY").toString())){
        	String ARR_CATEGORY = "";
        	String[] APP_CATEGORYS = params.get("ARR_CATEGORY").toString().split(",");
        	for(int i=0;i<APP_CATEGORYS.length;i++){
        		if(i==0){
        			ARR_CATEGORY += "'"+APP_CATEGORYS[i]+"'";
        		}else{
        			ARR_CATEGORY += ",'"+APP_CATEGORYS[i]+"'";
        		}
        	}
        	params.put("ARR_CATEGORYS",ARR_CATEGORY);
        }
