...
import com.willbes.cmm.util.Util;
...
public class ProductOrderController {
	...
	// 전체 상품 주문 관리 리스트(0원)
	@RequestMapping(value="/list_s_0.do")
	public String list_s_0(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
	...
	// 전체 상품 주문 관리 리스트(0원)
	@RequestMapping(value="/list_0.do")
	public String list_0(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		List list = null;
		List list_second = null;
		List order_list = null;
		List payment_list = null;
		...
		//리스트
		list = productOrderService.getOrderListDB_0(searchMap);

		// 총 건수
		int listCount = productOrderService.getOrderListCount_0(searchMap);
	...
	// 무료강좌 주문관리(0원)
	@RequestMapping(value="/list_s_freelec.do")
	public String list_s_freelec(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
//		List list_second = null;
//		List order_list = null;
//		List payment_list = null;
		...
		String paycode = CommonUtil.isNull(req.getParameter("paycode"), "");
		String prodgbn = CommonUtil.isNull(req.getParameter("prodgbn"), "");
		String FREE_LEC = CommonUtil.isNull(req.getParameter("FREE_LEC"), "Y");
		String payall = "3";
		...
		searchMap.put("currentPage", currentPage);
		searchMap.put("pageRow", pageRow);
		searchMap.put("FREE_LEC", FREE_LEC);
...
//		output.addAttribute("payment_list", payment_list);
		...
		return "productOrder/Tbl_Order_Lst_freelec";
	}
	
	// 전체 상품 주문 관리 리스트(0원)
	@RequestMapping(value="/list_freelec.do")
	public String list_freelec(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		List list = null;
//		List list_second = null;
//		List order_list = null;
//		List payment_list = null;
...
//		payment_list = productOrderService.getPaymentList(searchMap);
		...
		//리스트
		list = productOrderService.getOrderListDB_freelec(searchMap);
	
		// 총 건수
		int listCount = productOrderService.getOrderListCount_freelec(searchMap);
...
//		output.addAttribute("payment_list", payment_list);
	...
	// 엑셀리스트
		@SuppressWarnings({ "rawtypes", "unchecked" })
		@RequestMapping(value="/tbl_order_freeleclist_excel.do")
		public View excelDownloadfreelec(ModelMap output, HttpServletRequest req) throws Exception {
			...
			params.put("sdate",sdate);
			params.put("edate",edate);
			params.put("IS_EXCEL", "Y");

			//엑셀 리스트
			List<HashMap<String, String>> exe_list = productOrderService.getOrderListDB_freelec(params);

			Date date = new Date();
	  	    SimpleDateFormat simpleDate = new SimpleDateFormat("yyyy-MM-dd");

			String excelName = "무료강의상품주문리스트(" + sdate + "~" + edate + ")_" + simpleDate.format(date);
			...
			for(HashMap<String, String> map : exe_list) {
				...
				newMap.put(i++, String.valueOf(dCount));
				newMap.put(i++, map.get("ORDERNO").toString());
				if(map.get("ORDERS_USERNAME")!=null && !"".equals(map.get("ORDERS_USERNAME").toString())){
					newMap.put(i++, map.get("ORDERS_USERNAME").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("ORDERS_REGDATE")!=null && !"".equals(map.get("ORDERS_REGDATE").toString())){
					newMap.put(i++, map.get("ORDERS_REGDATE").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("CATEGORY_NM")!=null && !"".equals(map.get("CATEGORY_NM").toString())){
					newMap.put(i++, map.get("CATEGORY_NM").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("SUBJECT_NM")!=null && !"".equals(map.get("SUBJECT_NM").toString())){
					newMap.put(i++, map.get("SUBJECT_NM").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("LECCODE")!=null && !"".equals(map.get("LECCODE").toString())
						&&map.get("BRIDGE_LECCODE")!=null && !"".equals(map.get("BRIDGE_LECCODE").toString())){
					newMap.put(i++, map.get("LECCODE").toString()+"|"+map.get("BRIDGE_LECCODE").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("SUBJECT_TEACHER_NM")!=null && !"".equals(map.get("SUBJECT_TEACHER_NM").toString())){
					newMap.put(i++, map.get("SUBJECT_TEACHER_NM").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("SUBJECT_TITLE")!=null && !"".equals(map.get("SUBJECT_TITLE").toString())){
					newMap.put(i++, map.get("SUBJECT_TITLE").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("PC_M_NM")!=null && !"".equals(map.get("PC_M_NM").toString())){
					newMap.put(i++, map.get("PC_M_NM").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("PLAYYN_NM")!=null && !"".equals(map.get("PLAYYN_NM").toString())){
					newMap.put(i++, map.get("PLAYYN_NM").toString());
				}else{
					newMap.put(i++, "");
				}
				if(map.get("FREE_CANCEL_DT")!=null && !"".equals(map.get("FREE_CANCEL_DT").toString())){
					newMap.put(i++, map.get("FREE_CANCEL_DT").toString());
				}else{
					newMap.put(i++, "");
				}
	...
	public String update(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		Map<String, Object> pay_list = (HashMap) list.get(0);
		...
		order_list2.put("USER_ID",map.get("USER_ID"));
	...
	public String plyer_gostop(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
	    ...
	    //20170504 jjc 일반경찰 프리패스강의인지 조회. package_no
	    String leccdoe_package = productOrderService.getMyleccodepackageSelectone(searchMap);
	    if(leccdoe_package != ""){
	    	searchMap.put("leccdoe_package", leccdoe_package);
	    	
		    //20170504 jjc 현재 듣고있는 강사 숫자 조회(원유철 ,오태진)
		    int subject_teacher_num = productOrderService.getMysubjectteacherNumSelectone(searchMap);//
		    
		    if(subject_teacher_num == 1){
			    //20170504 jjc 현재 듣고있는 강의 조회(원유철 ,오태진)
			    String subject_teacher = productOrderService.getMysubjectteacherSelectone(searchMap);//
			    searchMap.put("subject_teacher", subject_teacher);
		    }else if(subject_teacher_num == 2){
		    	searchMap.put("subject_teacher", "wc_two");
		    }
	    }else{
	    	searchMap.put("leccdoe_package", "");
	    }
...
//			searchMap.put("DanIntCount",map.get("DANINTCOUNT")); //단과할인 % 가져오기
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/LectureChangeMovie.json")
	@ResponseBody
	@Transactional(readOnly=false,rollbackFor=Exception.class)
	public Map<String, Object> LectureChangeMovie(ModelMap model, HttpServletRequest request) throws Exception {
		...
		String manager_id = sessionMap.get("USER_ID");
		if(manager_id != null){
			params.put("USER_ID",manager_id);
		}else{
			params.put("USER_ID","");
		}		

		// 변경전 선택된 교수님이 2명이었는지 
		String strISTWO = CommonUtil.isNull(request.getParameter("SUBJECT_TEACHER_ISTWO"),"");		
		
		params.put("ORDERNO", 			CommonUtil.isNull(request.getParameter("ORDERNO"),""));
		params.put("PACKAGE_NO", 		CommonUtil.isNull(request.getParameter("PACKAGE_NO"),""));
		params.put("SUBJECT_TEACHER", 	CommonUtil.isNull(request.getParameter("SUBJECT_TEACHER"),""));
		params.put("CHANGE_DAY", 	CommonUtil.isNull(request.getParameter("CHANGE_DAY"),""));
		
		int CHANGE_DAY = Integer.parseInt(params.get("CHANGE_DAY"));
		
		Map<String, Object> returnMap = new  HashMap<String, Object>();
		String returnMsg = "";
		
		if("".equals(params.get("USER_ID"))){
			returnMsg = "loginFail";		
		}else{ 	//선택된 강의를 변경
			
			String USER_PACKAGE_FLAG = "";
			
			if(strISTWO.equals("Y")){
				
				//TB_ORDER_YEARPACKAGE_LOG 등록이 안되어있어야 변경가능
				USER_PACKAGE_FLAG = CommonUtil.isNull(productOrderService.myuserpackageflagSelectone(params),"");
				
				if(USER_PACKAGE_FLAG != ""){					
					returnMsg = "beforeChangeMyTeacher"; //이미 1번 변경했음					
				}else{
					
					// 교수 삭제
					int subject_teacher_update = productOrderService.deleteYearpackage(params);
					returnMsg = "refresh"; // 교수 삭제 후 화면 갱신
				...
				calendar.add(calendar.DATE, -CHANGE_DAY);
	            java.util.Date date = calendar.getTime();
	            String today = (new SimpleDateFormat("yyyyMMdd").format(date));
	            int beforextoday =  Integer.parseInt(today);			
	            int YEARPACKAGE_REG_TIME = 0;
				//강의 등록일
				YEARPACKAGE_REG_TIME = Integer.parseInt(CommonUtil.isNull(productOrderService.myuserpackageregtimeSelectone(params),"")); 
				//강의 등록일에서 7일까지는 변경가능
				//if(beforextoday <= YEARPACKAGE_REG_TIME){	
					// String USER_PACKAGE_FLAG = "";				
					//TB_ORDER_YEARPACKAGE_LOG 등록이 안되어있어야 변경가능
					USER_PACKAGE_FLAG = CommonUtil.isNull(productOrderService.myuserpackageflagSelectone(params),""); 
					...
					if(USER_PACKAGE_FLAG != ""){
						...
						//교수 변경
						int subject_teacher_update = productOrderService.yearpackageUpdate(params);
						//USER_PACKAGE_FLAG  Y로 변경
						int mylecture_change = productOrderService.mylectureInsert(params);
						//수강중인 강의 제거
						int mylecture_delete = productOrderService.mylectureDelete(params);	
						returnMsg = "addSuccess"; //변경
					}
				//}else{
				//	returnMsg = "dayspast"; //x일이 이미 지남
				//}			
...
//					productOrderService.updateEndDateMyLecture(searchMap);
	...
	public String pop_refund(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
    	...
    	if(list.size() > 0 && list != null){
	    	...
	    	searchMap2.put("STATUS_NM",map.get("STATUS_NM"));
    	}
...
//		output.addAttribute("momo_list_allcount", momo_list_allcount);
	...
	public List MemoModify1(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		//사용자 팝업 메모 및 point 변경 수정
		productOrderService.updateMemberPoint(searchMap);
		if (!memo.equals("")) {
...
//				}
	...
	public String offererPrint(ModelMap output, HttpServletRequest req) throws Exception {
		...
		if(PRINT_STS.equals("3")) {
			list = productOrderService.getSubLec(searchMap);
		}else{
		...
		if(PRINT_STS.equals("3")) {
			return "productOrder/offererPrint_New";
		}else{
	...
	public List updatePrint(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		//수강증출력
		productOrderService.insertPrintOffOrders(searchMap);
...
//		lec_list = productOrderService.getVwMenuMstTree_lec(searchMap);
	...
	//종합반 강의 품목변경 팝업
	@RequestMapping(value="/pop_subject_smod.pop")
	@Transactional(readOnly=false, rollbackFor=Exception.class)
	public String pop_subject_smod(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		String Num = CommonUtil.isNull(req.getParameter("Num"), "");
		String LECCODE = CommonUtil.isNull(req.getParameter("LECCODE"), "");
		String SCH_GUBUN = CommonUtil.isNull(req.getParameter("SCH_GUBUN"), "");
		String SCH_SUBJECT = CommonUtil.isNull(req.getParameter("SCH_SUBJECT"), "");
		String SCH_ORDERNO = CommonUtil.isNull(req.getParameter("SCH_ORDERNO"), "");
		searchMap.put("Num", Num);
		searchMap.put("LECCODE", LECCODE);
		searchMap.put("SCH_ORDERNO", SCH_ORDERNO);
		searchMap.put("SCH_GUBUN", SCH_GUBUN);
		if("1".equals(SCH_GUBUN)){
			searchMap.put("SCH_SUBJECT", SCH_SUBJECT);
		}
		
		//강의선택 팝업  리스트
		List list = productOrderService.lectureOffSView(searchMap);
		output.addAttribute("searchMap", searchMap);
		output.addAttribute("list", list);
		return "productOrder/Tbl_Order_subejct_smod_pop";
	}
	...
	public String updatePage(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		HashMap<String, String> sessionMap = (HashMap)req.getSession().getAttribute("AdmUserInfo");
		String ADMINID = sessionMap.get("USER_ID");
	...
	public String orderUpdate(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
		...
		if (Long.parseLong(PRICE_CARD) > 0) {
			PAYCODE = "PAY110";
		}else if (Long.parseLong(PRICE_CARD) <= 0 && Long.parseLong(PRICE_CASH) > 0) {
			PAYCODE = "PAY140";
		}else if (Long.parseLong(PRICE_CARD) <= 0 && Long.parseLong(PRICE_CASH) <= 0 && Long.parseLong(PRICE_BANK) > 0) {
			PAYCODE = "PAY150";
		}else{
			PAYCODE = "PAY160";
		}
		...
		searchMap.put("STATUSCODE", "DLV105");
			// 결제 정보가 있으면 업데이트한다.
		 ...
		 if (PRD != null) {
			for (int i = 0; i < PRD.length; i++) {
				...
				REAL_PRICE = req.getParameter("REALPRICE_"+PRD[i]);
				 ...
				 } else{ //변경과목이 종합반의 부속 과목일 경우 가격만 업데이트 한다.(강의코드도 변경 가능 2016.10.11)
					 NEW_LECCODE = req.getParameter("NEW_LECTURE_NO_"+PRD[i]);
...
//					 searchMap.put("REFUND", "Y");
	...
	// 누락 주문건 등록 팝업
	@RequestMapping(value="/MissOrder.pop")
	public String MissOrder(ModelMap model, HttpServletRequest req) throws Exception {

		return "productOrder/MissOrder_Pop";
	}

	// 주문누락건 등록 프로세스 - 단과
	@RequestMapping(value="/insertMissOrder.json")
    @ResponseBody
	public String insertMissOrder(ModelMap model, HttpServletRequest req) throws Exception {

		HashMap<String, String> sessionMap = (HashMap)req.getSession().getAttribute("AdmUserInfo");
		String reg_id = sessionMap.get("USER_ID");
		...
		String leccode = CommonUtil.isNull(req.getParameter("leccode"), "");
		String price = CommonUtil.isNull(req.getParameter("price"), "");
		String pay_type = CommonUtil.isNull(req.getParameter("pay_type"), "");
		String tid = CommonUtil.isNull(req.getParameter("tid"), "");
		...
		productOrderService.insertMissOrder(params);
	...
	/**
	 * @Method Name : userLecMovieList
	 * @작성일 : 2017. 02.
	 * @Method 설명 : 사용자 단과별 수강내역 팝업
	 */
	@RequestMapping(value="/userLecMovieList.pop")
	public String userLecMovieList(ModelMap model, HttpServletRequest request) throws Exception {

		HashMap<String, String> params = new  HashMap<String, String>();

		params.put("ORDERNO", CommonUtil.isNull(request.getParameter("ORDERNO"), ""));
		params.put("PACKAGE_NO", CommonUtil.isNull(request.getParameter("PACKAGE_NO"), ""));
		params.put("LECTURE_NO", CommonUtil.isNull(request.getParameter("LECTURE_NO"), ""));
		
		List<HashMap<String, String>> list = productOrderService.getUserLecMovie(params);

		model.addAttribute("list", list);
		model.addAttribute("params", params);

		return "productOrder/userLecMovieList_pop";
	}
	 ...
	 * @Method 설명 : 사용자 단과별 수강내역 팝업
	 */
	@RequestMapping(value="/userLecMovieLog.pop")
	public String userLecMovieLog(ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<HashMap<String, String>> list = productOrderService.getUserLecMovieLog(params);

		model.addAttribute("list", list);
		model.addAttribute("params", params);

		return "productOrder/userLecMovieLog_pop";
	}
