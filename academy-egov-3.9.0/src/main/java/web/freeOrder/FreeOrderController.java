...
public class FreeOrderController {
    ...
    public String pop_add2(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
        ...
        if (log.isDebugEnabled()) {
            log.debug("***** 수강신청 등록 팝업 파라미터 ***** ");
            log.debug("***** keyword : " + keyword);
            log.debug("***** currentPage : " + currentPage);
            log.debug("***** pageRow : " + pageRow);

            log.debug("***** userid : " + userid);
            log.debug("***** package_no : " + package_no);
        ...
        if(leccode != null && !"".equals(leccode)) {
            ...
            if(list.size() > 0){
				...
				if(map.get("SUBJECT_OPTION").toString() != null || !"".equals(map.get("SUBJECT_OPTION").toString())) {
					Subject_Option = map.get("SUBJECT_OPTION").toString();
					String Subject_Option_Division[] = Subject_Option.split(",");

					for(int i = 0; i<Subject_Option_Division.length; i++){
						if(Subject_Option_Division[i].equals("1")){
							searchMap.put("subject_option","VOD");
						}else if(Subject_Option_Division[i].equals("2")){
							searchMap.put("subject_option","PMP");
						}else if(Subject_Option_Division[i].equals("3")){
							searchMap.put("subject_option","VOD+PMP");
						}else if(Subject_Option_Division[i].equals("AA")){
							searchMap.put("subject_option","MP4");
						}else if(Subject_Option_Division[i].equals("BB")){
							searchMap.put("subject_option","VOD+MP4");
						}
					}
				}else{
					searchMap.put("subject_option","");
				}*/
    ...
    public String pop_subject_add2(ModelMap output, HttpServletRequest req) throws UnsupportedEncodingException {
        ...
        if (log.isDebugEnabled()) {
            log.debug("***** 강의선택 팝업 파라미터 ***** ");
            log.debug("***** currentPage : " + currentPage);
            log.debug("***** pageRow : " + pageRow);

            log.debug("***** userid : " + userid);

            log.debug("***** s_cat_cd : " + s_cat_cd);
            log.debug("***** s_sjt_cd : " + s_sjt_cd);
            log.debug("***** s_menu_id : " + s_menu_id);
            log.debug("***** search_type : " + search_type);
