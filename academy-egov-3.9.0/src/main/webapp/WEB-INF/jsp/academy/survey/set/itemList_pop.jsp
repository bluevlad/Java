...
window.onload = function () {
  ...
  <!--content -->
	<h2>● 온라인설문 > <strong>설문문항</strong></h2>
	...
	<input type="hidden" id="SETID" name="SETID" value="${params.SETID }" />
	...
	<table class="table04">
    	<tr>
            ...
            <td width="50%">            
				...
				<input type="text" id="SEARCHTEXT" name="SEARCHTEXT" value="${params.SEARCHTEXT}" size="20" title="검색어" onkeypress="fn_checkEnter()">
    ...
    <table class="table04">
		...
		<tr>
            <td><input type="checkbox" name="v_queids" value="${list.QUEID}" <c:if test="${list.CHK > 0}">disabled</c:if>></td>
...
<!-- paginate-->
	...
	<ul class="boardBtns"><li><a href="javascript:fn_Reg();">등록</a></li></ul>
	</form> 
  <!--//content --> 
...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/survey/set/itemList.pop"/>').submit();
}

//등록
function fn_Reg() {
	$('#searchFrm').attr('action','<c:url value="/survey/set/itemInsert.do"/>').submit();
}
</script>
