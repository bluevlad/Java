              ...
              <c:if test="${not empty list}">
	              <c:forEach items="${list}" var="list" varStatus="status">
		                <tr>
		                  ...
		                  <td><a href="javascript:view('${list.SETID}');">${list.SETTITLE}</a></td>
		                  <td>${list.CNT}</td>
		                  <td>${list.ISUSE}</td>
		                </tr>
			        </c:forEach>
...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/survey/set/list.do"/>').submit();
}

//등록
function fn_Reg() {
	$('#searchFrm').attr('action','<c:url value="/survey/set/write.do"/>').submit();
}

//상세
function view(no) {
	$("#SETID").val(no);
	$('#searchFrm').attr('action','<c:url value="/survey/set/modify.do"/>').submit();
}
</script>
