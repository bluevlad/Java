...
<c:if test="${view[0].SCREEN_GUBUN == 'S' and fn:length(view[0].CATEGORY_CD) == 3}">
    ...
    <c:if test="${view[0].BANNER_NO == 23}">
    	if($("input[name='BANNER_GUBUN']:checkbox:checked").length < 1){
    		alert("배너구분을 선택해주세요.");
    		return;
    	}
    </c:if>
            ...
            <c:if test="${view[0].BANNER_NO == 23}">
	            <tr>
	                ...
	                <c:forEach items="${campus_gubun}" var="data" varStatus="status" >
	                	<input name="BANNER_GUBUN"  class="i_check" type="checkbox" value="${data.CODE_VAL}" ><label for="a1">${data.CODE_NM}</label>
	                </c:forEach>
	                </td>
	            </tr>
            </c:if>
