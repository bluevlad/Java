...
<script type="text/javascript">
	...
	function excel_onclick1() { //ck 추가
		$('#searchFrm').attr('action','<c:url value="/tm/tmPassBoardListExcel.do"/>').submit();
	}
	...
	function TM_delete(idx, vcode, id){
		$("#cmd").val('delete');
		$("#idx").val(idx);
		$("#vcode").val(vcode);
		$("#userid").val(id);
		$('#searchFrm').attr('action','<c:url value="/tm/tmPassBoardList.do"/>').submit();
	}
	...
	function TM_write(idx, userid, Reqname, content, dutycode, voccode){
		document.getElementById("divTmWrite").style.display = "block";
	}
...
<input type="hidden" id="userid" name="userid" />
    ...
    <!--버튼-->
    <ul class="boardBtns">
		<li><a href="javascript:javascrip:TM_write();">등록하기</a></li>
    </ul>
    <!--//버튼--> 
    ...
    <table class="table02">
        ...
        <tr>
			...
			<td ONMOUSEOVER="popHelp('${list.CONTENTS}','lightgreen','200','title1');" ONMOUSEOUT="kill();" >${list.CONTENTS}</td>
			<td>
                ...
                <font color="red">|</font>
                <input type="button" value="X" onclick="javascript:TM_delete('${list.IDX}','${list.VOCCODE}','${list.REQUSERID}');" title="삭제" ></input>
			</td>
		</tr>
		</c:forEach>
		</c:if>
	...
	<table style="width:100%;">
		<tr>
			<td width="20%" style="vertical-align:middle;">
	      	...
	      	<td><strong>고객명 : <span id="MName"></span></strong> (고객명은 수정되지 않습니다)</td>
		</tr>
		<tr>
	...
	<div id="divTmWrite" style="display:none;">
	<table style="width:100%;">
		<tr>
			<td width="20%" style="vertical-align:middle;">
	        	<select id="CDUTYCODE" name="CDUTYCODE" style="width:120;">
					<option value="">- 구분 -</option>
					<c:forEach items="${dutyCodelist}"  var="item">
					<option value="${item.CODE }">${item.NAME }</option>
					</c:forEach>
				...
				<select id="CVOCCODE" name="CVOCCODE" style="width:120;">
					<option value="">- 분류 -</option>
					<c:forEach items="${vocCodelist}"  var="item">
					<option value="${item.CODE_VAL}">${item.CODE_NM }</option>
					</c:forEach>
	      	...
	      	<th>수강생아이디</th>
	      	<td><input type="text" id="REQUSERID" name="REQUSERID" style="width:100px;"/></td>
	      	<th>수강생이름</th>
	      	<td><input type="text" id="REQUSERNAME" name="REQUSERNAME" style="width:100px;"/></td>
	      	<th>주문번호</th>
	      	<td><input type="text" id="REQORDERNO" name="REQORDERNO" style="width:100px;"/></td>
	      	<th>TM담당자아이디</th>
	      	<td><input type="text" id="REGUSERID" name="REGUSERID" value="${AdmUserInfo.USER_ID}" style="width:100px;"/>${AdmUserInfo.USER_ID}</td>
	      	<th>TM담당자이름</th>
	      	<td><input type="text" id="REGUSERNAME" name="REGUSERNAME" value="${AdmUserInfo.USER_NM}" style="width:100px;"/>${AdmUserInfo.USER_NM}</td>
		</tr>
		<tr>
			<td colspan="10" style="text-align:left;">
				<input id="CONTENTS" name="CONTENTS" type="text" style="width:60%;" title=""/> &nbsp;
				<input type="button" onclick="TM_BoardInsert();"  value="등록" /> &nbsp;&nbsp;
			</td>
		</tr>
	</table>
	</div>
