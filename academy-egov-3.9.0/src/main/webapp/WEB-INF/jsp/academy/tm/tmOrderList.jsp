...
<script type="text/javascript">
	...
	function goList(page) {
		...
		$('#searchFrm').attr('action','<c:url value="/tm/tmOrderList.do"/>').submit();
	}
	...
	function excel_onclick1() { //ck 추가
		...
		$('#searchFrm').attr('action','<c:url value="/tm/tmOrderList_excel.do"/>').submit();
	}
...
<title>TM관리 - TM 결제리스트</title>
</head>

<!--content -->
  <div id="content">
	<h2>● TM 관리 &gt; <strong>TM 결제리스트</strong></h2>
	...
	<table class="table01">
        <tr>
          ...
          <select name="SEARCHTYPE" id="SEARCHTYPE">
				...
				<option value="REQUSERID" <c:if test="${params.SEARCHTYPE == 'REQUSERID'}">selected</c:if>>회원ID</option>
			</select>
	...
	&nbsp;&nbsp;/&nbsp;- 전체 수강금액 : <fmt:formatNumber value="${resultMap.REALPRICESUM}" groupingUsed="true"/> 원 
	&nbsp;&nbsp;/&nbsp;- 전체 결제금액 : <fmt:formatNumber value="${resultMap.PAYPRICESUM}" groupingUsed="true"/> 원 / Page:(<c:out value="${currentPage}"/>/<c:out value="${totalPage}"/>)</span>
	</p>
