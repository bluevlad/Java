...
function goList(page) {	
	...
	$('#searchFrm').attr('action','<c:url value="/productOrder/TmCouponSubjectList.pop"/>').submit();
}
...
function fn_Add(){
	if($("input[name='DEL_ARR']:checked").length < 1){
		...
		$("#SUBJECT_NM", opener.document).val(appendStr);
		$("#SUBJECT", opener.document).val(curValStr);
  ...
  <div>
   ...
   <!--테이블--> 
    ...
    <!--버튼-->
	<ul class="boardBtns">
   		<li><a href="javascript:fn_Add();">강의추가</a></li>
    </ul>
    <!--//버튼-->
