...
.boardBtns2 {display:inline; margin-left:5px;}
.boardBtns2 a {display:inline-block; background-color:#0d73ae; padding:3px 10px; border:1px #03304a solid; color:#fff; font-weight:bold;}
.boardBtns2 a:hover {background-color:#333; border:1px #000 solid; color:#FF0;}
.boardBtns2 a.Btn_01 {background-color:#cc0000; border:1px #660000 solid;}
</style>
...
function lecview_pop(orderno, package_no, lecture_no) {
	window.open('<c:url value="/productOrder/userLecMovieList.pop"/>?ORDERNO=' + orderno + '&PACKAGE_NO=' + package_no + '&LECTURE_NO=' + lecture_no, '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1100,height=670');
}
...
//강의변경 20170504 JJC
function fn_lecChange(orderno, package_no, before_subject_teacher){
		
	var cnt;
	
	// 선택된 교수가 2명 일때
	if(before_subject_teacher == "wc_two"){		
		
		cnt =$('input:checkbox[id="chkTeacher"]:checked').length; // 체크된 체크박스 수
		
		if(cnt == 2) return; // 강사 변경이 없을 때
		
		if(cnt == 0){  // 교수님을 아무도 체크하지 않았을 때
			alert("변경할 교수님을 체크하여 주시기 바랍니다.");
			return;
		} 
	...
	var change_day = 7;
	var teacher_length = document.getElementsByName("TEACHER").length;
	var subject_teacher = "";	
	
	for(var i = 0; i < teacher_length; i++) {
		  if(document.getElementsByName("TEACHER")[i].checked){
			  subject_teacher = document.getElementsByName("TEACHER")[i].value;
			  break;
		  }
    }	  
	  
	//같은 강사로 강사변경하기 눌렀을시 
	if(before_subject_teacher == subject_teacher){
		return;
	}
	
	var teacher_nm;
	if(subject_teacher == "wc_007"){
		teacher_nm= "오태진";
	}else{
		teacher_nm= "원유철";
	}
	
	if(confirm(teacher_nm + "교수로 변경하시겠습니까?\n수강중인 강의는 삭제되고 단 한번만 변경됩니다.")){
		
		var param = "ORDERNO="+orderno;
		param += "&PACKAGE_NO="+package_no;
		param += "&CHANGE_DAY="+change_day;
		param += "&SUBJECT_TEACHER="+subject_teacher;
		if(before_subject_teacher == "wc_two"){	// 교수님이 2명 선택되있었을 때	 
			param += "&SUBJECT_TEACHER_ISTWO=Y";
		}else{
			param += "&SUBJECT_TEACHER_ISTWO=N";
		};		
		
		$.ajax({
			type: "GET",
			url : '<c:url value="/productOrder/LectureChangeMovie.json"/>?'+ param,
			dataType: "json",
			async : false,
			success: function(res) {
				if(res.returnMsg=="loginFail"){
					alert("로그인을 하신 후 이용하여 주시기 바랍니다.");
				}else if(res.returnMsg=="dayspast"){
					alert("등록을 한지 " +change_day + "일이 지났습니다.");						
				}else if(res.returnMsg=="beforeChangeMyTeacher"){
					alert("이미 교수를  1번 변경하였습니다.");	
				}else if(res.returnMsg=="addSuccess"){
					alert("교수가 변경되었습니다.");
				}else if(res.returnMsg=="refresh"){
					fn_refresh();  // 교수 삭제 후 화면 갱신
				}
			},error: function(){
				alert("실패!");
				return;
...
<table width="100%" border="0" cellspacing="0" cellpadding="12">
	<tr>
        ...
        <td><p>▣ 강의선택 ${searchMap.searchLeccode} </p></td>
        <td>
...
<table class="table02">
	...
	<tr>
    	<td style="text-align:left;">
			<a href="javascript:lecview_pop('${searchMap.orderno}', '${list.PACKAGE_NO}','${list.LECTURE_NO}')">${list.PACKAGE_NAME}</a>
		</td>
		<td>
			<input type="text" id="mylecture_start_date" name="mylecture_start_date" maxlength="10" class="i_text" value="${list.MYLECTURE_START_DATE }" readonly="readonly" style="width:90px;IME-MODE:disabled;" onChange="mylecture_start_date_onkeyup('${status.index}', '${list.LECTURE_NO}');" onKeyDown="onOnlyNumber(this);"/>
           	...
           	(<input type="text" name="mylecture_period" id="mylecture_period" value="${list.MYLECTURE_PERIOD}" style="width:30%;" onkeyup="mylecture_period_onkeyup('${status.index}', '${list.LECTURE_NO}');">
			<c:if test="${list.MYLECTURE_PAUSED_COUNT ne 0 }"> + ${list.MYLECTURE_PAUSED_PERIOD}</c:if>일)
			<c:if test="${list.MYLECTURE_PAUSED_COUNT ne 0 }"> + ${list.MYLECTURE_PAUSED_COUNT}회</c:if>
		</td>
		<td>
...
<!-- 강의변경 20170504 JJC 버튼 -->
<c:if test="${not empty searchMap.leccdoe_package}">	
	<c:if test="${searchMap.subject_teacher eq 'wc_005' || searchMap.subject_teacher eq 'wc_007' }">
		<ul class="boardBtns2" >
			<input type="radio" id="termTt1" name="TEACHER" value="wc_005" <c:if test="${fn:contains(searchMap.subject_teacher, 'wc_005') == true}"> checked</c:if>><label for="termTt1">원유철</label>
			<input type="radio" id="termTt2" name="TEACHER" value="wc_007" <c:if test="${fn:contains(searchMap.subject_teacher, 'wc_007') == true}"> checked</c:if>><label for="termTt2">오태진</label>
			<a href="javascript:fn_lecChange('${searchMap.orderno}','${searchMap.package_no}','${searchMap.subject_teacher}')">교수변경</a>
			<label for="termTt2">
				<c:choose> 							
							<c:when test="${searchMap.subject_teacher eq 'wc_005'}"><span style="color:blue;">* 원유철</span></c:when>
							<c:otherwise><span style="color:blue;">* 오태진</span></c:otherwise>							
				</c:choose>교수님 선택하셨습니다.
			</label>
		</ul>
	</c:if>
	<c:if test="${searchMap.subject_teacher eq 'wc_two'}">
		<ul class="boardBtns2" >
			<input type="checkbox" id="chkTeacher" name="TEACHER"  value="wc_005"  checked><label for="termTt1">원유철</label></input>
			<input type="checkbox" id="chkTeacher" name="TEACHER"  value="wc_007"  checked><label for="termTt2">오태진</label></input>
			<a href="javascript:fn_lecChange('${searchMap.orderno}','${searchMap.package_no}','${searchMap.subject_teacher}')">교수변경</a>
			<label for="termTt2"><span style="color:blue;">&nbsp* 원유철, 오태진 </span>  교수님 모두 선택하셨습니다.</label>
		</ul>
	</c:if>
</c:if>
...
<c:if test="${not empty pmp_list}">
	...
	<table class="table02">
        ...
        <tbody>              
        <tr>
        	<td>${pmp_list.MOVIE_ORDER1}회 ${pmp_list.MOVIE_ORDER2}강</td>
			<td>${pmp_list.MOVIE_NAME}</td>
			<td>${pmp_list.PMP_REGDATE}</td>
		</tr>
        </tbody>
	</table>        
    </c:forEach>
...
<c:if test="${not empty mobile_list}">
	...
	<table class="table02">
        ...
        <tbody>              
		<tr>
			<td>${mobile_list.MOVIE_ORDER1}회 ${mobile_list.MOVIE_ORDER2}강</td>
			<td>${mobile_list.MOVIE_NAME}</td>
			<td>${mobile_list.MOBILE_REGDATE}</td>
		</tr>
        </tbody>
    </table>
	</c:forEach>
