...
//종합반 강의 풍목변경 
function product_s_change(num,leccode,orderno,gubun,subject_cd) {
	window.open('<c:url value="/productOrder/pop_subject_smod.pop"/>?Num=' + num
			 + '&LECCODE=' + leccode+ '&SCH_ORDERNO=' + orderno + '&SCH_GUBUN=' + gubun + '&SCH_SUBJECT=' + subject_cd
			 + '&TOP_MENU_ID=' + $("#TOP_MENU_ID").val() + '&MENUTYPE=' + $("#MENUTYPE").val() + '&L_MENU_NM=' + $("#L_MENU_NM").val(), '_blank', 'location=no,resizable=yes,width=1200,height=550,top=0,left=0,location=no,scrollbars=yes');
}
		           ...
		           <div class="form_table" style="margin-top:10px; float:left; width:95%;">
			           <table class="tbl_type" style="margin-top:10px; float:left; width:95%;" border="1">
				         ...
				         <c:if test="${not empty sub_list}">
				            ...
				            <c:if test="${list.O_TYPE == 'S'}">
				            	<c:set var="price" value="${list.REALPRICE}"/><c:set var="colors" value="#FFFFFF"/>
				            	<input type="hidden" id="NEW_LECTURE_NO_${list.NUM}" name="NEW_LECTURE_NO_${list.NUM}" value="${list.LECTURE_NO}"/>
				            </c:if>
			             ...
			             <tr style="background-color:${colors}">
				            ...
				            <td>
					            ...
					            <c:if test="${list.LEC_TYPE_CHOICE == 'D' && list.O_TYPE == 'S'}">
					            <input type="button" onclick="javascript:product_s_change('${list.NUM}','${list.MGNTNO}','${searchMap.ORDERNO}','${list.GUBUN}','${list.SUBJECT_SJT_CD}');" value="품목변경" />
					            </c:if>
				            </td>
							<c:choose>
								<c:when test="${list.TICKET_PRINT_DT ne null}">
									<c:set var="colorx" value="red"/>
									<c:set var="p_date" value="${list.TICKET_PRINT_DT}"/>
								</c:when>
				            ...
				            <td>
								<c:if test="${(list.LEC_TYPE_CHOICE == 'D' && list.O_TYPE == 'M') || (list.LEC_TYPE_CHOICE != 'D')}">
								<input type="button" onclick='javascript:fn_Print_New("${searchMap.ORDERNO}","${list.MGNTNO}");' value="수강증출력" style="color:${colorx};" />
								</c:if>
				            </td>
				        </tr>
				      		</c:forEach>
						...
						<c:if test="${empty sub_list}">
			             <tr bgColor=#ffffff align=center>
							<td colspan="7" style="text-align:center;">등록된 신청 내역이 없습니다.</td>
						 </tr>
						 </c:if>
						 </tbody>
			              ...
			              <tr> 
							...
							<td style="text-align:right;"></td>
							<td style="text-align:center;"><c:out value="${p_date}"/></td>
						 </tr>
