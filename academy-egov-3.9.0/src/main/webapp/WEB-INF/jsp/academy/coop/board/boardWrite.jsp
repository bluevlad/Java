...
function paramCheck() {
	...
	if($("#COOP_AREA").val() == ""){
		...
		if(confirm("등록하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/CoopBoardMng/insertCoopboard.do' />");
			$("#frm").submit();			
		}
	}
}
	...
	<input type="hidden" id="MENU_NM" name="MENU_NM" value="${params.MENU_NM}" />
    		...
    		<th scope="col">지역</th>
   			<td scope="col" style="text-align:left;">
   				<div class="item" id="codeList">
					...
					<option value="${area_list.CODE_CD}" <c:if test="${searchMap.COOP_AREA == area_list.CODE_CD}">selected</c:if>>${area_list.CODE_NM }</option>
					</c:forEach>
		  			</select>
				</div>
   			</td>
    		...
    		<th scope="col">의료분과</th>
   			<td scope="col" style="text-align:left;">
   				<div class="item" id="codeList">
					...
					<option value="${hspt_list.CODE_CD}" <c:if test="${searchMap.COOP_CATE == hspt_list.CODE_CD}">selected</c:if>>${hspt_list.CODE_NM }</option>
					</c:forEach>
		  			</select>
				</div>
   			</td>
    	...
    	<tr>
    		<th scope="col">추천여부</th>
   			<td scope="col" style="text-align:left;">
   				<div class="item" id="codeList">
	   				<input type="checkbox" id="RECOMMEND" name="RECOMMEND" VALUE="Y"/>
				</div>
   			</td>
   		</tr>
   		<tr>
   		...
   		<tr>
   			...
   			<td scope="col" style="text-align:left;">
   				<div class="item">
   					<input type="text" id="CREATENAME" name="CREATENAME" class="i_text" title="레이블 텍스트" value="" style="width:200px;" />&nbsp;&nbsp;
   				</div>
   			</td>
   		</tr>
   		<tr>
		...
		<tr>
			...
			<td style="text-align:left;">
				<div class="item">
					<textarea id="CONTENT" name="CONTENT" cols="50" rows="10" class="i_text" title="레이블 텍스트" style="width:96%;"></textarea>
				</div>
			</td>
		</tr>
		<tr>
