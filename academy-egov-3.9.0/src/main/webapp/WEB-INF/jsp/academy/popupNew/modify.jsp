...
function paramCheck() {
	...
	if(codeStr==""){
		...
		if(confirm("수정하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/popupNew/update.do' />");
			$("#frm").submit();			
		}
	}
}
...
function fn_popupDelete(){
	if(confirm("삭제하시겠습니까?")){
		$("#frm").attr("action", "<c:url value='/popupNew/delete.do' />");
		$("#frm").submit();	
	}
}
//숫자만 입력
...
<input type="hidden" id="L_MENU_NM" name="L_MENU_NM" value="${L_MENU_NM}" />
    	...
    	<h2>● 팝업 관리  > <strong>팝업 관리</strong></h2>
    	<table class="table01">
    	<tr>
   		...
   		<tr>
    		<th scope="col">팝업유형</th>
   			<td scope="col" colspan="3" style="text-align:left;">
   				<div class="item">
	   				<input name="POPUP_TYPE"  class="i_check" value="L" type="radio"  <c:if test="${view.POPUP_TYPE == 'L' }">checked="checked"</c:if>><label for="a2">레이어 팝업</label>&nbsp;&nbsp;
	   				<input name="POPUP_TYPE"  class="i_check" value="W" type="radio"  <c:if test="${view.POPUP_TYPE == 'W' }">checked="checked"</c:if>><label for="a3">윈도우 팝업</label>
				</div>
   			</td>
   		</tr>	
   		<tr>
    		<th scope="col">팝업 사이즈</th>
   			<td scope="col" colspan="3" style="text-align:left;">
   				<div class="item">
  					<input type="text" id="POPUP_WIDTH" name="POPUP_WIDTH" value="${view.POPUP_WIDTH}" title="넓이" size="2" maxlength="4" style="ime-mode:disabled;" onKeyDown="return fn_OnlyNumber(event);" onkeyup="removeChar(event);"/> X
        			<input type="text" id="POPUP_HEIGHT" name="POPUP_HEIGHT" value="${view.POPUP_HEIGHT}" title="높이" size="2" maxlength="4" style="ime-mode:disabled;" onKeyDown="return fn_OnlyNumber(event);" onkeyup="removeChar(event);"/>
				</div>
   			</td>
   		</tr>	
   		<tr>
    		<th scope="col">팝업 위치</th>
   			<td scope="col" colspan="3" style="text-align:left;">
   				<div class="item">
  					<input type="text" id="POPUP_LEFT" name="POPUP_LEFT" value="${view.POPUP_LEFT}" title="x"  style="ime-mode:disabled;" onKeyDown="return fn_OnlyNumber(event);" onkeyup="removeChar(event);"/> X
        			<input type="text" id="POPUP_TOP" name="POPUP_TOP" value="${view.POPUP_TOP}" title="y" style="ime-mode:disabled;" onKeyDown="return fn_OnlyNumber(event);" onkeyup="removeChar(event);"/>
				</div>
   			</td>
   		</tr>
   		<tr>
