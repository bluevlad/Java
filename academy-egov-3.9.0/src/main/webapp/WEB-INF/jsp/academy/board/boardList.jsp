...
function fn_Write(){
	...
	$("#SEARCHKIND").val(searchKind);
	$("#SEARCHTEXT").val(searchText);
	$("#form").attr("action", "<c:url value='/board/boardWrite.pop' />");
	$("#form").submit();
}

function fn_Is_RE_Update(){
	var Ischk = document.getElementsByName('UPDATE_IS_ARR');
	var Rechk = document.getElementsByName('UPDATE_RE_ARR');
	var Openchk = document.getElementsByName('UPDATE_OPEN_ARR');
	
	if(confirm("이슈/추천/공개여부를 수정 하시겠습니까?")){
		
		
		$("#form").attr("action", "<c:url value='/board/Is_RE_Update.pop' />");
		for(i=0; i<Ischk.length; i++){
			if(Ischk[i].checked == true){
				Ischk[i].value = 'Y/'+ Ischk[i].value
			}else{
				Ischk[i].checked = true;
				Ischk[i].value = 'N/'+ Ischk[i].value
			}
		}
		
		for(i=0; i<Rechk.length; i++){
			if(Rechk[i].checked == true){
				Rechk[i].value = 'Y/'+ Rechk[i].value
			}else{
				Rechk[i].checked = true;
				Rechk[i].value = 'N/'+ Rechk[i].value
			}
		}
		
		for(i=0; i<Openchk.length; i++){
			if(Openchk[i].checked == true){
				Openchk[i].value = 'Y/'+ Openchk[i].value
			}else{
				Openchk[i].checked = true;
				Openchk[i].value = 'N/'+ Openchk[i].value
			}
		}
		$("#form").submit();
	}
	...
	<!--테이블-->
		...
		<tr>
			...
			<%-- 			<c:forEach items="${rankList}"  var="data" varStatus="status" >
                   ...
                   <c:if test="${params.BOARD_MNG_SEQ eq 'BOARD_002' 
					...
					or params.BOARD_MNG_SEQ eq 'BOARD_005' or params.BOARD_MNG_SEQ eq 'BOARD_006'}">
						<select class="sele" id="SEARCHBOARD_TYPE" name="SEARCHBOARD_TYPE">
						   	<option <c:if test="${params.SEARCHBOARD_TYPE == '' or empty params.SEARCHBOARD_TYPE }">selected="selected"</c:if> value="">-분류선택-</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'A' }">selected="selected"</c:if> value="A">강좌개강일</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'B' }">selected="selected"</c:if> value="B">시작일변경</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'C' }">selected="selected"</c:if> value="C">결제</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'D' }">selected="selected"</c:if> value="D">환불문의</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'E' }">selected="selected"</c:if> value="E">재수강,연장</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'F' }">selected="selected"</c:if> value="F">PMP</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'G' }">selected="selected"</c:if> value="G">동영상오류</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'H' }">selected="selected"</c:if> value="H">일시중지</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'I' }">selected="selected"</c:if> value="I">불만/건의</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'J' }">selected="selected"</c:if> value="J">교재 및 자료</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'K' }">selected="selected"</c:if> value="K">배송문의</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'L' }">selected="selected"</c:if> value="L">회원정보</option>
							<option <c:if test="${params.SEARCHBOARD_TYPE == 'Z' }">selected="selected"</c:if> value="Z">기타</option>
	                   </select>
					</c:if>
     ...
     <!--//테이블-->
		...
		<table class="table02">
			<tr>
				...
				<c:if test="${params.BOARD_MNG_SEQ eq 'BOARD_000'}">
					<th scope="col" width="10%">캠퍼스</th>
				</c:if>
				<th scope="col" width="10%">구분</th>
				<c:if test="${params.BOARD_MNG_SEQ eq 'BOARD_002' 
				or params.BOARD_MNG_SEQ eq 'BOARD_003' or params.BOARD_MNG_SEQ eq 'BOARD_004' 
				or params.BOARD_MNG_SEQ eq 'BOARD_005' or params.BOARD_MNG_SEQ eq 'BOARD_006'}">
					<th scope="col" width="10%">질문구분</th>
				</c:if>
				<c:if test="${params.BOARD_MNG_SEQ eq 'BOARD_000'}">
				...
				or params.BOARD_MNG_SEQ eq 'BOARD_003' or params.BOARD_MNG_SEQ eq 'BOARD_004' 
				or params.BOARD_MNG_SEQ eq 'BOARD_005' or params.BOARD_MNG_SEQ eq 'BOARD_006'}">
					<th scope="col" width="7%">답변상태</th>
				</c:if>
				<c:if test="${params.BOARD_MNG_TYPE ne 'BOARD'}">
				<th scope="col" width="7%">이슈</th>
				<th scope="col" width="7%">추천</th>
				</c:if>
			...
			<c:if test="${not empty list}">
		        ...
		        <tr>
					...
					<td>
						<a href="javascript:member_view('${data.UPD_ID }');">
							<c:choose>
								<c:when test="${params.BOARD_MNG_SEQ eq 'NOTICE_013'  or params.BOARD_MNG_SEQ eq 'BOARD_002' 
								or params.BOARD_MNG_SEQ eq 'BOARD_003' or params.BOARD_MNG_SEQ eq 'BOARD_004' 
								or params.BOARD_MNG_SEQ eq 'BOARD_005' or params.BOARD_MNG_SEQ eq 'BOARD_006'}">
					...
					<c:if test="${params.BOARD_MNG_TYPE ne 'BOARD'}">
					<td>						
						<input type="checkbox" value="${data.BOARD_SEQ}" name="UPDATE_IS_ARR" <c:if test="${data.ISSUE eq 'Y'}">checked="checked"</c:if>>						
					</td>
					<td>
						<input type="checkbox" value="${data.BOARD_SEQ}" name="UPDATE_RE_ARR" <c:if test="${data.RECOMMEND eq 'Y'}">checked="checked"</c:if>>						
					</td>
					</c:if>
					<td>						
		 ...
		 <ul class="boardBtns">
	   	  ...
	   	  <c:if test="${params.BOARD_MNG_TYPE ne 'BOARD'}">
	   	  <li><a href="javascript:fn_Is_RE_Update();">이슈/추천/공개 수정</a></li>
	   	  </c:if>
	    </ul>
		<!--//버튼-->
	<!--paging  -->
