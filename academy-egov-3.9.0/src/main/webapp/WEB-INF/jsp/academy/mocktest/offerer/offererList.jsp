    ...
    <table class="table02">
        ...
        <c:forEach items="${list}" var="item" varStatus="index">
            <tr>
                ...
                <td>${fn:substring(item.IDENTYID,8,13)}</td>
                <td>${item.PHONE_NO}</td>
                <td>
                ...
                <td><a href="javascript:fn_Modify('${item.IDENTYID}');" style="color:blue;">${item.MOCKCODE} - &nbsp;${item.MOCKNAME}</a></td>
                <td>${item.CLASSSERIESCODENM}</td>
                <td>${item.AREANM}</td>
                <td>
                    <c:if test="${item.PAYMENTSTATE ne 0}">
                        <fmt:formatNumber value="${item.PAYMENTAMOUNT}" pattern="##,###.##" />
                    </c:if>
                    <c:if test="${item.PAYMENTSTATE eq 2 }">
                        <br/>-<fmt:formatNumber value="${item.PAYMENTAMOUNT}" pattern="##,###.##" />
                    </c:if>
                </td>
                <td>
...
function goList(page) {
    ...
    $("#form").attr("action", "<c:url value='/offerer/offererList.do' />");
    $("#form").submit();
}
...
function fn_Reg(){
    $("#form").attr("action", "<c:url value='/offerer/offererWrite.do' />");
    $("#form").submit();
}
...
function fn_Excel() {
    $("#currentPage").val(1);
    $("#form").attr("action", "<c:url value='/offerer/offererListExcel.do' />");
    $("#form").submit();
}
</script>
