...
function goList(page) {	
	...
	$('#searchFrm').attr('action','<c:url value="/productevent/pop_subject_add.pop"/>').submit();
}

//강의명 클릭
function fn_AddLec()
{
	$('#regFrm').attr('action','<c:url value="/productevent/lec_insert.do"/>').submit();
}
  ...
  <div>
   ...
   <!--테이블--> 
    ...
    <!--버튼-->
	<ul class="boardBtns">
   		<li><a href="javascript:fn_AddLec();">강의추가</a></li>
    </ul>
    <!--//버튼-->
        ...
        <tr>
         ...
         <c:if test="${not empty list}">
          <c:forEach items="${list}" var="list" varStatus="status">
             <tr>
				<td><input name="v_leccode" type="checkbox" value="${list.LECCODE}"/></td>
