...
<input type="hidden" id="LECCODE" name="LECCODE" value="">
    ...
    <ul class="boardBtns">
    	...
    	<li><a href="javascript:fn_Delete2();">삭제</a></li>
    	<!-- 
    	<li><a href="#">삭제</a></li> 
        <li><a href="#">개설</a></li>
        <li><a href="#">폐강</a></li>
...
<p class="pInto01">&nbsp;<span>총${totalCount}건 (<c:out value="${params.currentPage}"/>/<c:out value="${totalPage}"/>)</span></p>    
	...
	<table class="table02">
      ...
      -->
		<tr>
	        <th width="110"><input type="checkbox" name="allCheck" id="allCheck" class="i_check" value="" onclick="fn_CheckAll('allCheck', 'DEL_ARR')" />No </th>
	        <th width="80">직종</th>
	        <th width="80">과목</th>
	        <th width="75">강의코드</th>
	        <th width="55">강사명</th>
		...
		<c:forEach items="${list}" var="item" varStatus="loop">
			...
			<tr id="TR_${item.BRIDGE_LECCODE}" <c:if test="${item.SUBJECT_ISUSE eq 'Y'}">class="vitality"</c:if>>
		        <td class="tdCenter"><input type="checkbox" name="DEL_ARR" class="i_check" value="${item.LECCODE}/${item.BRIDGE_LECCODE}" />${totalCount-((params.currentPage-1)*params.pageRow)-loop.index}</td>
...
// 등록폼
function fn_Reg(){
	$("#frm").attr("action", "<c:url value='/lecture/freewrite.do'/>");
	$("#frm").submit();
}
...
function fn_Delete2(){
	if($("input[name='DEL_ARR']:checked").length > 0){
		if(confirm("선택한 항목을 정말 삭제하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/lecture/FreeLec_listDelete.do' />");
			$("#frm").submit();
		}
	}else{
		alert("선택된 항목이 없습니다");
