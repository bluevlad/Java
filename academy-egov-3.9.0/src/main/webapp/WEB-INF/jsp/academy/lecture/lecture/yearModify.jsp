					...
					<li>
						<table class="tdTable" id="ADDAREA">
				        	<tr>
				                ...
				                <th width="15%">학습형태</th>
				                <th>강의명</th>
				                <c:if test="${params.LEC_TYPE_CHOICE eq 'Y'}">
				                <th width="5%">순서</th>
				                </c:if>
				                <th width="15%">강사</th>
				                <th width="10%">삭제</th>
	...
	<table class="table01">
		...
		<tr>
			...
			<td>
				...
				<%-- <c:if test="${params.SEARCHKIND == item.CODE }">selected="selected"</c:if> --%>
				<select name="SUBJECT_PERIOD" id="SUBJECT_PERIOD">
					<!-- <option value="">--직종검색--</option> -->
					<option value="1" <c:if test="${'1' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>1개월</option>
					<option value="2" <c:if test="${'2' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>2개월</option>					
					<option value="3" <c:if test="${'3' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>3개월</option>					
					<option value="6" <c:if test="${'6' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>6개월</option>
					<option value="7" <c:if test="${'7' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>7개월</option>					
					<option value="12" <c:if test="${'12' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>12개월</option>
					<option value="13" <c:if test="${'13' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>13개월</option>
					<option value="14" <c:if test="${'14' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>14개월</option>
					<option value="24" <c:if test="${'24' == view[0].SUBJECT_PERIOD }">selected="selected"</c:if>>24개월</option>
	            </select>
		...
		<tr>
			<th>선택강사(선택갯수)</th>
			<td><input type="text" id="TEACHERNO" name="TEACHERNO" value="${view[0].TEACHERNO}" size="10" maxlength="3" style="background:#FFECEC;IME-MODE:disabled;" onKeyDown="fn_OnlyNumber(this);"/> 개</td>
		</tr>		
		<tr>
        	<th>원가</th>
...
function fn_Submit(){
	...
	if(confirm("강의를 수정하시겠습니까?")){
		...
		$("#frm").attr("action","<c:url value='/lecture/yearUpdate.do' />");
		$("#frm").submit();
	}
}
...
function fn_Delete(){
	if("${lectureOrderCount}"!="0"){
		if(confirm("이 강의를 주문한 내역이 존재합니다. 정말 삭제하시겠습니까?")){
			$("#frm").attr("action","<c:url value='/lecture/yearDelete.do' />");
			$("#frm").submit();
		}		
	}else{
...
function fn_PayListPop(gubn, type, leccode){
	window.open('<c:url value="/lecture/yearPayList.pop"/>?SEARCHPAYYN=' + gubn + '&SEARCHOPENPAGE=&SEARCHPAYTYPE=' + type + '&LECCODE=' + leccode, '_blank', 'scrollbars=no,toolbar=no,resizable=yes,width=1040,height=670');
}
</script>
</body>
</html>