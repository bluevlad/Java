...
<input type="hidden" id="BRIDGE_MSTCODE" name="BRIDGE_MSTCODE" value="">
<input type="hidden" id="MSTCODE" name="MSTCODE" value="">
...
<p class="pInto01">&nbsp;<span>총${totalCount}건 (<c:out value="${params.currentPage}"/>/<c:out value="${totalPage}"/>)</span></p>    
	...
	<table class="table02">
		...
		<c:forEach items="${list}" var="item" varStatus="loop">
			...
			<tr id="TR_${item.BRIDGE_MSTCODE}" <c:if test="${item.SUBJECT_ISUSE eq 'Y'}">class="vitality"</c:if>>
		        ...
		        <td><a href="javascript:fn_DataView('${item.BRIDGE_MSTCODE}','${item.MSTCODE}');">${item.SUBJECT_NM}</a></td>
		        <td>${item.MSTCODE}<br>${item.BRIDGE_MSTCODE}</td>
		        ...
		        <%-- <td><a href="javascript:fn_PayListPop('Y','VOD','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.VODY}</a>/<a href="javascript:fn_PayListPop('N','VOD','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.VODN}</a> | <a href="javascript:fn_PayListPop('Y','PMP','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.PMPY}</a>/<a href="javascript:fn_PayListPop('N','PMP','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.PMPN}</a> | <a href="javascript:fn_PayListPop('Y','VOD+PMP','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.VODPMPY}</a>/<a href="javascript:fn_PayListPop('N','VOD+PMP','${item.MSTCODE}','${item.BRIDGE_MSTCODE}');">${item.VODPMPN}</a></td> --%>
		        <td>
		        ...
		        <%-- <td class="txt03"> <select onchange="javascript:Lec_ON_OFF('${item.BRIDGE_MSTCODE}','${item.SUBJECT_TITLE}');" id="ON_OFF_${item.BRIDGE_MSTCODE}"> <option value="ON" id="ON_${item.BRIDGE_MSTCODE}"<c:if test="${item.SUBJECT_ISUES eq 'Y' }">selected</c:if>>활성</option><option value="OFF" id="OFF_${item.BRIDGE_MSTCODE}"<c:if test="${item.SUBJECT_ISUSE ne 'Y'}">selected</c:if>>비활성</option></select></td> --%>
			</tr>
		</c:forEach>	
...
function goList(page) {
	...
	$("#frm").attr("action", "<c:url value='/lecturemst/list.do' />");
	$("#frm").submit();
}
...
function fn_Delete(){
	if($("input[name='DEL_ARR']:checked").length > 0){
		if(confirm("선택한 항목을 정말 삭제하시겠습니까?")){
			$("#frm").attr("action", "<c:url value='/lecturemst/listDelete.do' />");
			$("#frm").submit();
		}
	}else{
		alert("선택된 항목이 없습니다");
...
// 등록폼
function fn_Reg(){
	$("#frm").attr("action", "<c:url value='/lecturemst/write.do'/>");
	$("#frm").submit();
}
...
function fn_Modify(val1, val2){
	...
	$("#MSTCODE").val(val2);
	$("#frm").attr("action", "<c:url value='/lecturemst/modify.do' />");
	$("#frm").submit();
}

// 직종클릭시 팝업
function fn_DataView(val1, val2){
	window.open('<c:url value="/lecturemst/dataViewList.pop"/>?MSTCODE=' + val2 + '&BRIDGE_MSTCODE=' + val1, '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1100,height=670');
}
...
function fn_SeqPop(type){
	window.open('<c:url value="/lecturemst/seqList.pop"/>?LEC_TYPE_CHOICE=${params.LEC_TYPE_CHOICE}', '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1040,height=670');
}
...
//VOD,PMP,동영상 유/무 팝업
function fn_PayListPop(gubn, type, MSTCODE, BRIDGE_MSTCODE){
	window.open('<c:url value="/lecturemst/payList.pop"/>?SEARCHPAYYN=' + gubn + '&SEARCHOPENPAGE=L&SEARCHPAYTYPE=' + type + '&MSTCODE=' + MSTCODE + '&BRIDGE_MSTCODE=' + BRIDGE_MSTCODE , '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1040,height=670');
}

// VOD,PMP,동영상 클릭시 팝업
function fn_MovieDataViewPop(val1, val2, val3){
	window.open('<c:url value="/lecturemst/dataMovieViewList.pop"/>?MSTCODE=' + val2 + '&BRIDGE_MSTCODE=' + val1, '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1734,height=1032');
}
...
function Lec_ON_OFF(Rcode,LecNM){
	...
	if(confirm("개설 여부를 변경 하시겠습니까?")){
 		$.ajax({
			type: "GET",
			url: "<c:url value='/lecturemst/Modify_Lecture_On_Off.do' />"+"?Rcode="+Rcode + "&flag=" + flag + "&flag2=" + flag2,
			dataType: "text",
