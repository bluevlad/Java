...
function goList(page) {	
	...
	$('#searchFrm').attr('action','<c:url value="/freeOrder/pop_subject_add2.pop"/>').submit();
}

//강의명 클릭
function put_leccode(leccode, cat_nm, user_nm, menu_nm, title, real_price, price, subject_discount, period)
{
 window.opener.popFrm.leccode.value= window.opener.popFrm.leccode.value + leccode + "/";
 window.opener.popFrm.teacher_nm.value= user_nm;
 window.opener.popFrm.cat_nm.value= cat_nm;
 window.opener.popFrm.menu_nm.value= menu_nm;
 window.opener.popFrm.textfield5.value= title;
 window.opener.popFrm.subject_price_temp.value= real_price;
 window.opener.popFrm.price.value= price;
 window.opener.popFrm.subject_discount.value= subject_discount;
 window.opener.popFrm.period.value= period;
 ...
 window.opener.addLecture(title, leccode);
 //window.opener.save();
 //window.self.close();
}
         ...
         <c:if test="${not empty list}">
          <c:forEach items="${list}" var="list" varStatus="status">
             <tr>
				...
				<td><a href="javascript:put_leccode('${list.LECCODE}','${list.CAT_NM}','${list.USER_NM}', '${list.MENU_NM}', '${list.SUBJECT_TITLE}' , '${list.SUBJECT_MOVIE }', '${list.SUBJECT_PRICE }', '${list.SUBJECT_DISCOUNT }','${list.SUBJECT_PERIOD }')">${list.SUBJECT_TITLE}</a></td>
	            <td>${list.USER_NM}</td>
