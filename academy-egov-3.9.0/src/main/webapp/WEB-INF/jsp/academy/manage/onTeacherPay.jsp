...
function fn_detail(id, name) {
	...
	$("#form").attr("action", "<c:url value='/manage/onTeacherPayDetail.do' />");
	$("#form").submit();
}
    ...
    <table class="table02">
        ...
        <tbody>
			...
			<tr>
				...
				<td><font color="red"><fmt:formatNumber value="${totalRefund}" groupingUsed="true"/></font></td>
			...
			<tr id="${item.TEACHER_ID}" name="${item.TEACHER_ID}">
				<td>${loop.index + 1}</td>
				...
				<td><fmt:formatNumber value="${item.REFUND_PAY}" groupingUsed="true"/></td>
