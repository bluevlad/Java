...
function fn_excel() {
	...
	window.open('<c:url value="/manage/onReceivedExcel.do"/>', '_excel', 'scrollbars=yes,toolbar=no,resizable=yes,width=500,height=500');

	$("#form").attr("target", "_excel");
//	$("#form").attr("action", "<c:url value='/manage/onReceivedExcel.do' />");
	$("#form").submit();
	
/* 	$.ajax({
		type: "POST",
		url : '<c:url value="/manage/onReceivedExcel.do"/>',
		dataType: "text",		
...
function fn_excel_org() {
	$.ajax({
		type: "GET", 
		url : '<c:url value="/manage/onAdvanceReceived.json"/>?searchDate=' + $.trim($("#searchDate").val()),
		dataType: "text",		
		async : false,
		success: function(RES) {
		    if(confirm(RES)){
