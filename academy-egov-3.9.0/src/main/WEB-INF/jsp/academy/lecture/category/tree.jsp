        ...
        <div class="dtree">
            <script type="text/javascript">
                ...
                function menuUpdate(menu_id){
                    $(parent.document).find("#frmMenuEdit").attr("src" , '<c:url value="/category/detail.frm"/>?CODE='+menu_id+'&TYPE_CHOICE=${TYPE_CHOICE}&TOP_MENU_ID=${TOP_MENU_ID}&MENU_ID=${MENU_ID}&MENUTYPE=${MENUTYPE}');
                }   
                        
                var menuList = [];
                <c:forEach var="list" items="${menuList}" varStatus="status">
                    d.add("${list.CODE}","${list.P_CODE}","${list.NAME}[${list.CODE}]" ,"javascript:menuUpdate('${list.CODE}');");
                </c:forEach>
