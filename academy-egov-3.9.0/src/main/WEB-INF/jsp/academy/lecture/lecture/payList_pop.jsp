...
function fn_excel() { //ck 추가 
	...
	$('#frm').attr('action','<c:url value="/lecture/payList_excel.do"/>').submit();
}
...
<input type="hidden" id="SEARCHPAYTYPE" name="SEARCHPAYTYPE" value="${params.SEARCHPAYTYPE}"/>
...
<input type="hidden" id="SEARCHOPENPAGE" name="SEARCHOPENPAGE" value="${params.SEARCHOPENPAGE}"/>
    ...
    <table class="table02">
        ...
        <tbody>
	        <c:forEach items="${list}" var="item" varStatus="loop">
				<tr>
					<td>
						<input type="checkbox" name="smsname" id="${item.USERID}" value="${item.USER_NM}"
						<c:if test="${ item.USERID eq null || item.STATUSCODE eq 'DLV230'}">style="display: none;"</c:if>>
					</td>
			    	<td><a href="#" onclick="javascript:view('${item.USERID}')">
				           ${item.USER_NM}
				    	</a>
