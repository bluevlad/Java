...
<input type="hidden" id="currentPage" name="currentPage" value="${params.currentPage}">
	...
	<h2>● 강의 관리 > 직종관리 > <strong>순서변경</strong></h2>
    ...
    <!--버튼-->
	<ul class="boardBtns">
	    <li><a href="javascript:fn_SeqUpdate();">순서저장</a></li>
	</ul>
    <!--//버튼--> 
    ...
    <table class="table02">
		<tr>
	        <th width="105"><input type="checkbox" name="allCheck" id="allCheck" class="i_check" value="" onclick="fn_CheckAll('allCheck', 'DEL_ARR')" />No</th>
	        <th>코드</th>
	        <th>직종명</th>
	        <th>상태</th>
	        <th>등록일</th>
	        ...
	        <c:forEach items="${list}" var="item" varStatus="loop">
				<tr>
			    	<td>
			    		...
			    		<input type="checkbox" name="DEL_ARR" class="i_check" value="${item.CODE}" />

	                    	<input type="text" size="2" name="Num" value="${loop.index+1}" readonly="readonly" />
	                        <input type="hidden" name="SEQ" value="${item.SEQ}"/>
	                        <input type="hidden" name="CODE_NO" value="${item.CODE}"/>
	                        <input type="hidden" name="CODE_NM" id="CODE_NM" value="${item.NAME}"/>
    ...
    if($("input[name='DEL_ARR']:checked").length > 1){
        ...
        if(dir=='U'){	
            ...
            if((str-1)>0){//맨윗순서까지 이동을 안했을경우에만 실행을 해라 
                ...
                var e = document.frm.CODE_NO[(str-1)].value; // 현재 select한 값의 PROFESSOR_USER_ID값
                var f = document.frm.CODE_NO[(str-2)].value; // 바꾸려는 위치의 PROFESSOR_USER_ID값
                ...
                if(str > 1 ){
                    ...
                    document.frm.CODE_NO[(str-1)].value = e; // PROFESSOR_USER_ID값을 서로 맞바꾼다 
                    document.frm.CODE_NO[(str-2)].value = f; // PROFESSOR_USER_ID값을 서로 맞바꾼다                    
            ...
            if(str < (document.frm.DEL_ARR.length-1) ){ // 맨아랫순서까지 이동을 했을경우가 아니라면 실행해라 
                ...
                var e = document.frm.CODE_NO[(str-1)].value; //  현재 select한 값의 PROFESSOR_USER_ID값
                var f = document.frm.CODE_NO[(str)].value; // 바꾸려는 위치의 PROFESSOR_USER_ID값                
                if(str<(document.frm.DEL_ARR.length-1)){
                    ...
                    document.frm.CODE_NO[str-1].value = e; // PROFESSOR_USER_ID값을 서로 맞바꾼다 
                    document.frm.CODE_NO[str].value= f ;// PROFESSOR_USER_ID값을 서로 맞바꾼다                    
...
function fn_SeqUpdate(){
	if(confirm("순서를 변경하시겠습니까?")){
		$("#frm").attr("action", "<c:url value='/kind/seqUpdate.do' />");
		$("#frm").submit();	
	}
}
</script>
