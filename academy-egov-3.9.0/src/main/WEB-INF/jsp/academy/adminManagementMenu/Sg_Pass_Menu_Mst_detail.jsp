...
function add(form) {
    $("#form").attr("action", "<c:url value='/adminManagementMenu/pass_menuAdd.frm' />");
    $("#form").submit();
}
...
function del(form) {
    ...
    if(confirm("☞ 선택한 항목을 삭제하겠습니까?") )  {
        ...
        $.ajax({
            type : 'POST' , 
            url : '<c:url value="/adminManagementMenu/passmenuDeleteProcess.json" />',
            data : dataString ,
            async : false,
            success : function(result){
                if(result.isDelete == 1) {
                    alert("삭제 되었습니다.");
                    window.parent.location.href = '<c:url value="/adminManagementMenu/passMenuMain.do" />?' 
...
function modify(form) {
   ...
   /*  if(form.menu_nm.value=='') {
    ...
    //$("#form").attr("action", "<c:url value='/adminManagementMenu/PassmenuUpdateProcess.frm' />");
    //$("#form").submit();
    var dataString = $("#form").serialize();
    $.ajax({
        type : 'POST' , 
        url : '<c:url value="/adminManagementMenu/passmenuUpdateProcess.json" />',
        data : dataString ,
        async : false,
        success : function(result){
            if(result.isUpdate == 1) {
                alert("수정 되었습니다.");
                window.parent.location.href = '<c:url value="/adminManagementMenu/passMenuMain.do" />?VWMENU_ID='+result.VWMENU_ID
                                            + '&TOP_MENU_ID=${TOP_MENU_ID}&MENUTYPE=${MENUTYPE}';
            } else {
                alert("수정이 안되었습니다.");
            }
...
<table class="table01" style="font-size: 9pt; min-width: 700px; width: 55%" >
    ...
    <input type="hidden" id="MENUTYPE" name="MENUTYPE" value="<c:out value='${MENUTYPE}'/>" />
        ...
        <tr>
            <th >순서1</th>
            <td ><c:out value='${detail.MENU_SEQ}'/><input type="hidden" name="menu_seq" size="5"  onkeyup="checkNumber(this)" value="<c:out value='${detail.MENU_SEQ}'/>"></td>
        </tr>
        <tr>
            <th>메뉴URL</th>
        ...
        <tr>
            ...
            <td colspan=3 >
                <input type=radio name="TARGET" value="_blank" <c:if test="${detail.TARGET == '_blank'}">checked</c:if> >새창
                <input type=radio name="TARGET" value="" <c:if test="${detail.TARGET != '_blank'}">checked</c:if>>현재창
            </td>
        </tr>
        <tr>
            <th>직렬코드</th>
            <td colspan=3 ><input type="text" name="CODE" size="10"  value="<c:out value='${detail.CODE}'/>"></td>
        </tr>
        <tr>
            <th>상단메뉴이미지URL</th>
            <td colspan=3 ><input type="text" id="TOP_IMG_URL" name="TOP_IMG_URL" size="60" maxlength="200" value="<c:out value='${detail.TOP_IMG_URL}'/>"></td>
        </tr>
        <tr>
            <th>왼쪽메뉴이미지URL</th>
            <td colspan=3 ><input type="text" id="LEFT_IMG_URL" name="LEFT_IMG_URL" size="60" maxlength="200" value="<c:out value='${detail.LEFT_IMG_URL}'/>"></td>
        </tr>
        <tr>
            <th>타이틀이미지URL</th>
            <td colspan=3 ><input type="text" id="TITL_IMG_URL" name="TITL_IMG_URL" size="60" maxlength="200" value="<c:out value='${detail.TITL_IMG_URL}'/>"></td>
        </tr>
        <tr>
            <th>부타이틀이미지URL</th>
            <td colspan=3 ><input type="text" id="SUBTITL_IMG_URL" name="SUBTITL_IMG_URL" size="60" maxlength="200" value="<c:out value='${detail.SUBTITL_IMG_URL}'/>"></td>
        </tr>
        <tr>
            <th>메뉴설명</th>
            <td colspan=3 >
