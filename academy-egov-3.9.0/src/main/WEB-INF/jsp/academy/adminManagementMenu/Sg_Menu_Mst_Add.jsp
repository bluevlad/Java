 ...
 //       alert("☞ 고유 메뉴ID를 입력해주세요");
  ...
  //  }
   ...
   // }
		...
		$.ajax({
			type : 'POST' , 
			url : '<c:url value="/adminManagementMenu/menuInsertProcess.json" />',
			data : dataString ,
			async : false,
...
<table class="table01" style="font-size: 9pt; min-width: 700px; width: 55%" >
	...
	<input type="hidden" id="MENUTYPE" name="MENUTYPE" value="<c:out value='${MENUTYPE}'/>" />
	   ...
	   <!--  <tr>
			...
			<td colspan=3  ><input type="text" name="target" size="15" >
			<br><span>※ 현재선택한 메뉴의 표시 창(프레임)<br>(_self : 현재창 , _blank : 새창, 특정이름 : 해당명의 창, window:popup)</span>
			</td>
		</tr> -->
		<tr>
			<th height="25" >상태</th>
