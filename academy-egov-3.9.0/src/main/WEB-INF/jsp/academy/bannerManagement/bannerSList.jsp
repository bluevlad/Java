...
function fn_RowNumCheck(input) {
  ...
  <div id="content">
    ...
    <input type="hidden" id="NEW_BANNER" name="NEW_BANNER" value= "${params.NEW_BANNER}" /> 
    <table class="table01">
        <tr>
          <th width="10%">배너명</th>
          <td width="30%">
        ...
        <tr>
        	<th>적용구분</th>
        	<c:choose>
        		<c:when test="${view[0].BANNER_NO == 23}"><td colspan="2"></c:when>
        		<c:otherwise><td colspan="5"></c:otherwise>
        	</c:choose>
            ...
            <c:if test="${view[0].BANNER_NO == 23}">
	            <th>학원구분</th>
	        	<td colspan="2">
		        	<select name="SEARCH_BANNER_GUBUN" id="SEARCH_BANNER_GUBUN">
	                <option value="">학원선택</option>
	                <c:forEach items="${campus_gubun}" var="data" varStatus="status" >
	                	<option value="${data.CODE_VAL}" <c:if test="${params.SEARCH_BANNER_GUBUN == data.CODE_VAL}">selected = "selected"</c:if>>${data.CODE_NM}</option>
	                </c:forEach>
	            </select>
	            </td>
            </c:if>
