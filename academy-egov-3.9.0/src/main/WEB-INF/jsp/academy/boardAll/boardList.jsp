...
function goBoardView(boardSeq , boardParentSeq , codeName){
	...
	if(boardParentSeq == 0){
	//	alert("원본글");
		$("#form").attr("action", "<c:url value='/allboard/boardView.do' />");
		$("#form").submit();
	}
	else{
	//	alert("답변글");
...
function fn_Write(){
	...
	$("#SEARCHKIND").val(searchKind);
	$("#SEARCHTEXT").val(searchText);
	$("#form").attr("action", "<c:url value='/allboard/boardWrite.do' />");
	$("#form").submit();
}
...
function fn_Is_RE_Update(){
	...
	if(confirm("이슈/추천/공개여부를 수정 하시겠습니까?")){
		...
		$("#form").attr("action", "<c:url value='/allboard/Is_RE_Update.do' />");
...
function member_view(userid){
  ...
  <!--content -->
  <div id="content">
	<h2>● 전체게시판 > <strong>${params.BOARD_MNG_NAME}</strong></h2>
...
<%-- 				</c:if> --%>
			...
			<c:if test="${not empty list}">
		        ...
		        <tr>
					...
					<td>
							<a href="javascript:member_view('${data.UPD_ID }');"><c:out value="${data.USER_NM}"/></a>
					</td>
<%-- 					<c:if test="${params.BOARD_MNG_TYPE ne 'BOARD'}"> --%>
<!-- 					<td>						 -->
