...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/popupNew/list.do"/>').submit();
}

//등록
function fn_add() {
	$('#searchFrm').attr('action','<c:url value="/popupNew/write.do"/>').submit();
}
...
function checkDelete() {
	...
	if(confirm("삭제하시겠습니까?")) {		
		$('#searchFrm').attr('action','<c:url value="/popupNew/allDelete.do"/>').submit();
	}
}
//전체적용
function checkOPENY() {
	...
	if(confirm("적용하시겠습니까?")) {		
		...
		$("#OPEN_YN_ARR").val(tmp);
		$('#searchFrm').attr('action','<c:url value="/popupNew/openY.do"/>').submit();
	}
}
//전체미적용
function checkOPENN() {
	...
	if(confirm("미적용하시겠습니까?")) {		
		...
		tmp = tmp.substr(0,last);
		$("#OPEN_YN_ARR").val(tmp);
		$('#searchFrm').attr('action','<c:url value="/popupNew/openN.do"/>').submit();
	}
}
...
function view(no) {
	$("#NO").val(no);
	$('#searchFrm').attr('action','<c:url value="/popupNew/modify.do"/>').submit();
}
</script>
</head>
