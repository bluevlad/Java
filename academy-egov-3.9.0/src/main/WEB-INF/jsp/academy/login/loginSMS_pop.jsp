...
function doCheck() {
    $.ajax({
        type : "GET", 
        url : '<c:url value="/login/loginPhoneCheck"/>?smsCode=' + ($("#smsCode").val()),
        //dataType : "json", <!-- 단순 문자나 숫자값이 리턴될 경우 json parser error 발생할 수 있음.-->      
        cache : false,
        success : function(data, textStatus, jqXHR) {
            if(data=="Y"){
				alert('인증번호가 확인되었습니다. \n관리자화면으로 이동합니다.');
				window.opener.document.location.href="<c:url value='/login/loginProc.adm' />";
				window.close();
              return;
            } else if(data =="N"){
                alert('인증번호가 맞지 않습니다. \n확인후 정확하게 입력해 주십시요.');
		...
		<div class="cf-popcontent">         
            ...
            <div class="cfBox">                
                ...
                <form name="frm" id="frm" method="post" action="">
                    <input type="text" id="smsCode" name="smsCode" value="" /> <a onClick="doCheck();" />확인</a>
                </form>                         
            </div><!--cfBox//-->
        </div><!--cf-popcontent//-->
	</div><!--cf-pop//-->
