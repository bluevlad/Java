...
function fn_select(brd_mng_seq, brd_seq, brd_title){
    $(top.opener.document).find("#${params.SRCHCODE}").val(brd_seq);
    $(top.opener.document).find("#${params.SRCHTXT}").val(brd_title);
    ...
    <c:if test="${not empty list}">
        ...
        <c:choose>    
          ...
          <c:otherwise>
            ...
            <td><input type="button" name="lecbtn" value="선택" onClick="fn_select('${params.BOARD_MNG_SEQ}','${data.BOARD_SEQ}','<c:out value="${data.SUBJECT}"/>');"></td>
        </tr>
        </c:forEach>
    </c:if>
