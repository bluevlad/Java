 ...
 --%>
	...
	<!--테이블-->
		...
		<tr>
            <th width="15%">검색</th>
			<td>
				<select style="width:80px;" name="searchCategory" id="searchCategory" onchange="goList('1')">
				...
				<select style="width:100px;" name="SEARCHSBJTCD" id="SEARCHSBJTCD" onchange="goList('1')">
						<option value="" >과목</option>
					<c:forEach items="${catSubject}"  var="data" varStatus="status" >
						 <c:set var="vChecked">
								   <c:choose>
								    <c:when test="${data.SUBJECT_CD == params.SEARCHSBJTCD}">selected="selected"</c:when>
								    <c:otherwise></c:otherwise>
								   </c:choose>
						 </c:set>
				...
				<select style="width:100px;" name="searchTeacher" id="searchTeacher" onchange="goList('1')">
						<option value="" >강사</option>
					<c:forEach items="${teacherList}"  var="data" varStatus="status" >
						 <c:set var="vChecked">
								   <c:choose>
								    <c:when test="${data.USER_ID == params.searchTeacher}">selected="selected"</c:when>
								    <c:otherwise></c:otherwise>
								   </c:choose>
						 </c:set>
				...
				<select style="width:80px;" name="searchLearningCD" id="searchLearningCD" onchange="goList('1')">
					<option value="" >단계</option>
					<option value="M0101" <c:if test="${params.searchLearningCD == 'M0101'}">selected="selected"</c:if>>이론</option>
					<option value="M0102" <c:if test="${params.searchLearningCD == 'M0102'}">selected="selected"</c:if>>문제풀이</option>
					<option value="M0103" <c:if test="${params.searchLearningCD == 'M0103'}">selected="selected"</c:if>>특강</option>
				</select>
				<select style="width:80px;" name="searchKind" id="searchKind">
            	...
            	<input class="i_text" title="레이블 텍스트" type="text" name="searchText" id="searchText"  style="width:150px;" value="${params.SEARCHTEXT}" onKeyPress="fn_checkEnter()" />
            </td>
     ...
     <!--//테이블-->
		...
		<table class="table02">
			<tr>
				...
				<th scope="col" width="7%">구분</th>
				<th scope="col" width="7%">과목</th>
				<th scope="col" width="7%">강사</th>
				<th scope="col" width="7%">단계</th>
		 ...
		 <ul class="boardBtns">
	   	  ...
	   	  <li><a href="javascript:fn_Is_RE_Update();">공개수정</a></li>
	   	  </c:if>
	    </ul>
		<!--//버튼-->
	<!--paging  -->
