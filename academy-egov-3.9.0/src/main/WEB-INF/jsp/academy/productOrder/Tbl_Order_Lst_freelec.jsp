...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/productOrder/list_freelec.do"/>').submit();
}
...
function excel_onclick1() { //ck 추가
	...
	//alert($('#payall').val());
	$('#searchFrm').attr('action','<c:url value="/productOrder/tbl_order_freeleclist_excel.do"/>').submit();
}
...
<!--content -->
  <div id="content">
	...
	<strong>
		<c:choose>
			<c:when test="${searchMap.FREE_LEC eq 'Y'}">
				 무료강좌 신청확인
			</c:when>
			<c:otherwise>
				전체상품주문<c:if test="${searchMap.payall eq '3' }">(0원)</c:if>
			</c:otherwise>
		</c:choose>
	</strong></h2>
	...
	<table class="table01">
        <tr>
          ...
          <select id="orderstatuscode" name="orderstatuscode" onchange="goList(1)" style="width:120;">
				...
				<option value="Y" <c:if test="${searchMap.orderstatuscode == 'Y'}">selected</c:if>>신청완료</option>
				<option value="N" <c:if test="${searchMap.orderstatuscode == 'N'}">selected</c:if>>신청취소</option>
		  </select>
...
<!-- 						</a> -->
				...
				<td>${list.ORDERS_REGDATE}</td>
				<td>${list.CATEGORY_NM}</td>
				<td>${list.SUBJECT_NM}</td>
				<td>${list.LECCODE}<br/>${list.BRIDGE_LECCODE}</td>
				<td>${list.SUBJECT_TEACHER_NM}</td>
				<td style="text-align:left;">
					<a href="javascript:plyer_gostop('${list.ORDERS_USERID}','${list.ORDERNO}','${list.LECCODE}');">
				    	${list.SUBJECT_TITLE}
				    </a>
				</td>
<%-- 				<td><input type="checkbox" name="orderck" id="checkbox" value="${list.ORDERNO}#${list_second.MGNTNO}#${list_second.CONFIRMDATE}#${total_count + 1}#${status2.index}"></td> --%>
				<td>${list.PC_M_NM}</td>
				<td>${list.PLAYYN_NM}</td>
				<td>${list.FREE_CANCEL_DT}</td>
