...
function fn_excel_down() {
    $("#frm").attr("action", "<c:url value='/gosi/excel3.do' />");
    $("#frm").submit();
}
...
<input type="hidden" id="currentPage" name="currentPage" value="${params.currentPage}"/>
	<h2>● 경찰 고시 > <strong> 필기합격자인증</strong></h2>
    	...
    	<tr>          
            ...
            <td COLSPAN="3">            
				...
				<select name="SEARCHTYPE" id="SEARCHTYPE">
                    ...
                    <option value="GOSI_AREA_FULL_NM"  <c:if test="${params.SEARCHTYPE == 'GOSI_AREA_FULL_NM'}">selected</c:if>>시험직렬</option>
        ...
        <tbody>
	        <c:forEach items="${list}" var="item" varStatus="loop">
				<tr>
	                ...
	                <td>${item.RST_NO}</td>
	                <td>
	                	<a class="show_pop" href="/imgFileView.do?path=/${item.FILE_PATH1}">파일1</a>
	                	<c:if test="${not empty item.FILE_PATH2 and item.FILE_PATH2!=''}">
	                		<a class="show_pop" href="/imgFileView.do?path=/${item.FILE_PATH2}">파일2</a>
	                	</c:if>
	                </td>
	                <td>${item.REG_DT}</td>	                           
				</tr>
...
function fn_Search() {
	$("#currentPage").val(1);
	$("#frm").attr("action", "<c:url value='/gosi/Gosi_event3_list.do' />");
	$("#frm").submit();
}
