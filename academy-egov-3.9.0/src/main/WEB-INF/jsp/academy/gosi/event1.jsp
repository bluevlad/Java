...
function fn_excel_down() {
    $("#frm").attr("action", "<c:url value='/gosi/excel.do' />");
    $("#frm").submit();
}
...
<input type="hidden" id="currentPage" name="currentPage" value="${params.currentPage}"/>
	<h2>● 경찰 고시 > <strong> 응시표 인증하기</strong></h2>
        ...
        <tbody>
	        <c:forEach items="${list}" var="item" varStatus="loop">
				<tr>
	                ...
	                <td>${item.RST_NO}</td>
	                <td><a class="show_pop" href="/imgFileView.do?path=/${item.FILE_PATH}">파일</a></td>
	                <td>${item.REG_DD}</td>	                           
				</tr>
			</c:forEach>
...
function fn_Search() {
	$("#currentPage").val(1);
	$("#frm").attr("action", "<c:url value='/gosi/Gosi_event1_list.do' />");
	$("#frm").submit();
}
