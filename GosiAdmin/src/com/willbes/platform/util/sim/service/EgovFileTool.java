...
import com.willbes.platform.util.fcc.service.EgovStringUtil;
...
public class EgovFileTool {
	...
	for (int i = 0; i < fileArray.length; i++) {
		...
		if (Globals.OS_TYPE.equals("UNIX")) {
		    String[] command = { MirProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryByOwner"),
	...
	try {
		...
		if (Globals.OS_TYPE.equals("UNIX")) {
		    Process p = null;
		    String[] command = { MirProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getMountLc"), src, "/" };
		    p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	try {
		...
		Process p = null;
		String cmdStr = MirProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryOwner");
	...
	try {
		...
		Process p = null;
		if (Globals.OS_TYPE.equals("UNIX")) {
		    String[] command = { MirProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryAccess"),
			    parentPath, fname };
		    p = Runtime.getRuntime().exec(command);
		    p.waitFor();
		    //프로세스 에러시 종료
	...
	try {
		...
		if (strArr.length >= 2) {
		    ...
		    type = MirProperties.getProperty(Globals.FILE_FORMAT_PATH, format);
		}
	...
	try {
		...
		if (targetDir.exists()) {
		    ...
		    String cmdStr = MirProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".moveDrctry");
		    String[] command = { cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
