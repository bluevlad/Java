...
public class TeacherExcelDownload extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		String excelName = searchTeacherName + "_강사님의 강사료 정산 내역_" + CommonUtil.getCurrentDate();
		HSSFSheet sheet = workbook.createSheet("강사료");
		...
		for(Map<String, Object> item : list) {
			...
			cell = row.createCell(4);
			cell.setCellValue(MafUtil.parseDouble(String.valueOf(item.get("TOTAL_PAY"))));
			...
			pay110 = pay110 + MafUtil.parseDouble(String.valueOf(item.get("PAY110_PRICE")));
			pay120 = pay120 + MafUtil.parseDouble(String.valueOf(item.get("PAY120_PRICE")));
			pay130 = pay130 + MafUtil.parseDouble(String.valueOf(item.get("PAY130_PRICE")));
			pay100 = pay100 + MafUtil.parseDouble(String.valueOf(item.get("PAY100_PRICE")));
	        
        	pay110_su = pay110_su + MafUtil.parseDouble(String.valueOf(item.get("PAY110_SUSU")));
        	pay120_su = pay120_su + MafUtil.parseDouble(String.valueOf(item.get("PAY120_SUSU")));
        	pay130_su = pay130_su + MafUtil.parseDouble(String.valueOf(item.get("PAY130_SUSU")));

        	total_price = total_price + MafUtil.parseDouble(String.valueOf(item.get("TOTAL_PAY")));
        	if (item.get("TEACHER_PAY") != null) {
        		price = price + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY")));
        	}
        	if (item.get("TEACHER_PAY_TEMP1") != null) {
        		etc1 = etc1 + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY_TEMP1")));
        	}
        	if (item.get("TEACHER_PAY_TEMP2") != null) {
        		etc2 = etc2 + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY_TEMP2")));
        	}
		}
		row = sheet.createRow(++rowIndex);
		cell = row.createCell(4);
		...
		for(Map<String, Object> item : list2) {
			...
			cell = row.createCell(5);
			cell.setCellValue(String.valueOf(MafUtil.nvl(item.get("PAYCODE"),"")));
			cell.setCellStyle(style);
			
			r_total_price = r_total_price + MafUtil.parseDouble(String.valueOf(item.get("TOTAL_PAY")));
			r_price = r_price + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY")));
			r_etc1 = r_etc1 + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY_TEMP1")));
			r_etc2 = r_etc2 + MafUtil.parseDouble(String.valueOf(item.get("TEACHER_PAY_TEMP2")));
		}
		row = sheet.createRow(++rowIndex);
		cell = row.createCell(4);
		cell.setCellValue("소계");
