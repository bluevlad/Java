...
public class TeacherController {
	...
	public String write(ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<HashMap<String, String>> kindlist = teacherservice.getKindList(params);
		List<HashMap<String, String>> subjectlist = teacherservice.getSubjectList(params);
		params.put("ONOFF_DIV", "O");
    ...
    public HashMap<String, String> lecFileProcess(HashMap params, MultipartHttpServletRequest multipartRequest) throws Exception {
        ...
        if(PIC6 != null && PIC6.isEmpty() == false) {
        	/*int PIC6X = 32;
            if(null != params.get("PIC6X") && !"".equals(params.get("PIC6X").toString())) {
                PIC6X = Integer.parseInt(params.get("PIC6X").toString());
            }
            int PIC6Y = 32;
            if(null != params.get("PIC6Y") && !"".equals(params.get("PIC6Y").toString())) {
                PIC6Y = Integer.parseInt(params.get("PIC6Y").toString());
            }
            HashMap<String, Object> fileMap6 = fileUtil.uploadFile(PIC6, rootPath, subPath, PIC6X , PIC6Y, "");
        ...
        if(OFF_PIC6 != null && OFF_PIC6.isEmpty() == false) {
        	/* int OFF_PIC6X = 32;
            if(null != params.get("OFF_PIC6X") && !"".equals(params.get("OFF_PIC6X").toString())) {
                OFF_PIC6X = Integer.parseInt(params.get("OFF_PIC6X").toString());
            }
            int OFF_PIC6Y = 32;
            if(null != params.get("OFF_PIC6Y") && !"".equals(params.get("OFF_PIC6Y").toString())) {
                OFF_PIC6Y = Integer.parseInt(params.get("OFF_PIC6Y").toString());
            }
            HashMap<String, Object> fileMap6 = fileUtil.uploadFile(OFF_PIC6, rootPath, subPath, OFF_PIC6X, OFF_PIC6Y, "");
	...
	/**
	 * @Method Name : etcProcess
	 * @작성일 : 2013. 10.
	 * @Method 설명 : 카테고리 & 과목 등록프로세스
	 * @param params
	 * @param request
	 * @return HashMap
	 * @throws Exception
	 */
    /*public void etcProcess(HashMap<String, String> params, HttpServletRequest request) throws Exception {
		String[] ORI_CATEGORY_CODE = request.getParameterValues("ORI_CATEGORY_CODE");
		String[] CETCARR = request.getParameterValues("CATEGORY_CODE");
		...
		if(ORI_CATEGORY_CODE == null) {	// 등록
			...
			for(int i=0; i<ORI_CATEGORY_CODE.length;i++){
				...
				if(isDel) {
					params.put("DEL_CATEGORY_CODE", ORI_CATEGORY_CODE[i]);
					teacherservice.teacherCategoryDelete(params);
				}
			}
...
//		}
		String[] SETCARR = request.getParameterValues("SUBJECT_CD");
		teacherservice.teacherSubjectDelete(params);
		teacherservice.teacherMain_Category_Delete(params);
		if(SETCARR != null){
			for(int i=0; i<SETCARR.length; i++){
				params.put("SUBJECT_CD", SETCARR[i]);
				int teacherMain_Category_Subject = teacherservice.teacherMain_Category_Subject(params); // 등록
				teacherservice.teacherSubjectInsert(params);

				if(ORI_CATEGORY_CODE == null) {
					teacherservice.teacherMain_Category_Insert(params); // 등록
				}else{

					if(teacherMain_Category_Subject == 0){

						teacherservice.teacherMain_Category_Insert(params); // 수정
					}else{
