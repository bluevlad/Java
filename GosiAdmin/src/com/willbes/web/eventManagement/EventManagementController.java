...
public class EventManagementController {
	...
	*/
	...
	public String eventInsertProcess(ModelMap model, HttpServletRequest request, MultipartHttpServletRequest multipartRequest) throws Exception {
		...
		params.put("OPTION4_YN", CommonUtil.isNull(request.getParameter("OPTION4"), "N"));
		
		params.put("MESSAGEBOX", CommonUtil.isNull(request.getParameter("MESSAGEBOX"), ""));		
		params.put("SMSNUM", CommonUtil.isNull(request.getParameter("SMSNUM"), "1544-0330"));
		params.put("POPUP_TITLE", CommonUtil.isNull(request.getParameter("POPUP_TITLE"), ""));
		params.put("POPUP_LINK", CommonUtil.isNull(request.getParameter("POPUP_LINK"), ""));
		...
		params.put("OPTION2_EVENT", CommonUtil.isNull(request.getParameter("OPTION2_EVENT"), "N"));
		params.put("OPTION2_POPUP", CommonUtil.isNull(request.getParameter("OPTION2_POPUP"), "N"));
		...
		if(CATEGORY_CODE_ARR != null){
			for(int i=0; i<CATEGORY_CODE_ARR.length; i++){
				...
				if("Y".equals(params.get("OPTION3_YN"))){
					//옵션3 테이블 저장
						params.put("EVENT_NO", params.get("eventNo"));
						params.put("MESSAGEBOX", params.get("MESSAGEBOX"));
						params.put("SMSNUM", params.get("SMSNUM"));
						eventManagementService.insertEventOption3(params);
				}
				
				if("Y".equals(params.get("OPTION4_YN"))){
					//옵션4 테이블 저장
						params.put("EVENT_NO", params.get("eventNo"));
						params.put("POPUP_TITLE", params.get("POPUP_TITLE"));
						params.put("POPUP_LINK", params.get("POPUP_LINK"));
						eventManagementService.insertEventOption4(params);
				}
			}
		}
...
//			HashMap<String, Object> fileMap1 = fileUtil.uploadFile(img, rootPath, subPath, width, height, "");
		...
		if("Y".equals(params.get("OPTION4_YN"))){
			eventManagementService.deleteEventOption4(params);
	...
	*/
	...
	public View eventResultListExcel(ModelMap model, HttpServletRequest request) throws Exception {
		...
		for(HashMap<String, String> map : exe_list) {
			...
			if(null != map.get("CATEGORY_INFO") && !"".equals(map.get("CATEGORY_INFO"))) {
                newMap.put(i++, map.get("CATEGORY_INFO").toString());
            } else {
                newMap.put(i++, "");
            }
