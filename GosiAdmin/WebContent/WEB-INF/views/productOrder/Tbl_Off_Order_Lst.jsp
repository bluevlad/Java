        ...
        <tr>
		  ...
		  <select id="orderpaycode" name="orderpaycode" onchange="goList(1)" style="width:120;">
			...
			<option value="PAY160" <c:if test="${searchMap.orderpaycode == 'PAY160'}">selected</c:if>>전액할인</option>
          	...
          	<select id="s_cat_cd" name="s_cat_cd" onchange="goList(1)" >
         ...
         <c:if test="${not empty list}">
          <c:forEach items="${list}" var="list" varStatus="status">
             <tr>
				...
				<td>
					<c:if test="${list.STATUSCODE_NM ne '대기중'}">
					<input type="button" onclick='javascript:fn_Print("${list.ORDERNO}","${list.MGNTNO}");' value="수강증출력" style="color:${colorx};" />
					</c:if> 
				</td>
	        </tr>
      </c:forEach>
