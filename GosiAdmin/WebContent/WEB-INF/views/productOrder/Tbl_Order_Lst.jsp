					...
					<c:choose>
						...
						<c:otherwise>
							<c:if test="${not empty list.SENDDATE}">
								<c:set var="senddate1" value="${fn:replace(list.SEND_DT,' ', '')}"/>
	  				...
	  				<c:forEach items="${requestScope[list_second_idx]}" var="list_second" varStatus="status2">
						...
						<table style="width:100%">
							<tr>
								...
								<td style="width:38px;"><input type="checkbox" name="orderck" id="checkbox" value="${list.ORDERNO}#${list_second.MGNTNO}#${list_second.CONFIRM_DT}#${total_count + 1}#${status2.index}"></td>
								<c:set var="confirmdate_old" value="${fn:replace(list_second.CONFIRM_DT,'-', '')}"/>
