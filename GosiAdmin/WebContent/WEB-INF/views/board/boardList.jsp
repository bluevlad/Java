...
function fn_Is_RE_Update(){
	...
	var Mainchk = document.getElementsByName('UPDATE_MAIN_ARR');
	...
	if(confirm("이슈/추천/공개여부를 수정 하시겠습니까?")){
		...
		for(i=0; i<Mainchk.length; i++){
			if(Mainchk[i].checked == true){
				Mainchk[i].value = 'Y/'+ Mainchk[i].value
			}else{
				Mainchk[i].checked = true;
				Mainchk[i].value = 'N/'+ Mainchk[i].value
			}
		}
		$("#form").submit();
	}
		...
		<tr>
			...
			<%-- 			<c:forEach items="${rankList}"  var="data" varStatus="status" >
					...
					<c:if test="${params.BOARD_MNG_SEQ == 'NOTICE_008'}">
						<select style="width:100px;" name="SCH_EXAM_TYPE" id="SCH_EXAM_TYPE">
							<option value="">-공고유형-</option>
							<c:forEach items="${pub_gubun}" var="item" varStatus="loop">
								<option value="${item.CODE_VAL}" <c:if test="${item.CODE_VAL == params.SCH_EXAM_TYPE}">selected="selected"</c:if>>${item.CODE_NM}</option>					
							</c:forEach>
						</select>
						<select style="width:100px;" name="SCH_EXAM_AREA" id="SCH_EXAM_AREA">
							<option value="">-지역구분-</option>
							<c:forEach items="${pub_area}" var="item" varStatus="loop">
								<option value="${item.CODE_VAL}" <c:if test="${item.CODE_VAL == params.SCH_EXAM_AREA}">selected="selected"</c:if>>${item.CODE_NM}</option>					
							</c:forEach>
						</select>
					</c:if>
     ...
     <!--//테이블-->
		...
		<table class="table02">
			<tr>
				...
				<c:if test="${params.BOARD_MNG_SEQ == 'NOTICE_008'}">
					<th scope="col" width="10%">공고유형</th>
					<th scope="col" width="10%">지역구분</th>
				</c:if>
				<th scope="col">제목${params.BOARD_MNG_TYPE}</th>
				<c:if test="${params.BOARD_MNG_SEQ == 'NOTICE_000'}">
					<th scope="col" width="10%">메인노출</th>
				</c:if>
			...
			<c:if test="${not empty list}">
		        ...
		        <tr>
		            ...
		            <td>${data.CODENAME}</td>
		            <c:if test="${params.BOARD_MNG_SEQ == 'NOTICE_008'}">
		            	<td>${data.EXAM_TYPE_NM}</td>
		            	<td>${data.EXAM_AREA_NM}</td>
		            </c:if>
		            ...
		            <c:if test="${params.BOARD_MNG_SEQ == 'NOTICE_000'}">
						<td>						
							<input type="checkbox" value="${data.BOARD_SEQ}" name="UPDATE_MAIN_ARR" <c:if test="${data.CATE_CODE eq 'Y'}">checked="checked"</c:if>>						
						</td>
					</c:if>
		 ...
		 <ul class="boardBtns">
	   	  ...
	   	  <c:if test="${params.BOARD_MNG_TYPE ne 'BOARD'}">
	   	  <li><a href="javascript:fn_Is_RE_Update();">메인/이슈/추천/공개 수정</a></li>
	   	  </c:if>
	    </ul>
		<!--//버튼-->
	<!--paging  -->
