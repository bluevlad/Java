  ...
  <div id="">
    ...
    <tr>
    	...
    	<td width="94%">
		    ...
		    <div align=right><font size=2>${orderinf_list0.to_date}</font></div>
			
			<c:forEach var="i" begin="0" end="${searchMap.cnt - 1}">
			
			<table style="width:100%;">
		      <tr>
		        <td align="left" bgcolor="#FFFFFF"><p>▣ 주문자정보(${i + 1})</p></td>
		      </tr>
		    </table>
		    
		    <c:set var="orderinf_list" value="orderinf_list${i}"/>
		    <c:set var="orderinf_list" value="${requestScope[orderinf_list]}"/>
		    ...
		    <c:set var="inf_list" value="inf_list${i}"/>
		    <c:set var="inf_list" value="${requestScope[inf_list]}"/>
		    ...
		    <table class="table05">
		        ...
		        <c:set var="list_idx" value="list${i}"/>
		        
		        <c:if test="${not empty list0}">
		         <c:forEach items="${requestScope[list_idx]}" var="list" varStatus="status">
				...
				<c:choose>
					<c:when test="${tot_sum < 50000}">
						<c:set var="addprice" value="3000"/>
					</c:when>
