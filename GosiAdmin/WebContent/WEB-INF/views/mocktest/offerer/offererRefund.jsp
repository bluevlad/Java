...
<form name="frm" id="frm" class="form form-horizontal" method="post" action="<c:url value="/offerer/offererSave"/>">
...
<input type="hidden" id="SEARCHTEXT" name="SEARCHTEXT" value="${params.SEARCHTEXT}"/>
    ...
    <c:if test="${view[0].PAYMENTSTATE == 1}">
      <c:if test="${view[0].PAYMENTTYPE == 0}">
        ...
        <li><a href="javascript:fn_RefundSave();">확인</a></li>
    </c:if>
...
function fn_CardRefundSave() {
    if(confirm("정말 환불처리하시겠습니까?\n카드결제 내역도 모두 삭제됩니다")){
        $("#frm").attr("action","<c:url value='/offerer/offererCardPaymentDelete.do' />");
        $("#frm").submit();
    }
}
...
function fn_RefundSave() {
    ...
    if(confirm("정말 환불처리하시겠습니까?")){
        $("#frm").attr("action","<c:url value='/offerer/offererRefundSave.do' />");
        $("#frm").submit();
    }
}
...
function fn_RefundCancel() {
    if(confirm("정말 환불취소하시겠습니까?")){
        $("#frm").attr("action","<c:url value='/offerer/offererRefundCancel.do' />");
        $("#frm").submit();
    }
}
