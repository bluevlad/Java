			...
			<td>
				...
				<c:choose>
					<c:when test="${params.LEC_TYPE_CHOICE eq 'F'}">
						<li>입문특강 <span><input name="LEARNING_CD" type="radio" value="TAB_001" <c:if test="${view[0].FREE_TAB eq 'TAB_001'}">checked='checked'</c:if></input></span></li>
						<li>테마별특강 <span><input name="LEARNING_CD" type="radio" value="TAB_002" <c:if test="${view[0].FREE_TAB eq 'TAB_002'}">checked='checked'</c:if></input></span></li>
						<li>마무리특강 <span><input name="LEARNING_CD" type="radio" value="TAB_003" <c:if test="${view[0].FREE_TAB eq 'TAB_003'}">checked='checked'</c:if></input></span></li>
						<li>모의고사해설 <span><input name="LEARNING_CD" type="radio" value="TAB_004" <c:if test="${view[0].FREE_TAB eq 'TAB_004'}">checked='checked'</c:if></input></span></li>
						<li>기출해설특강 <span><input name="LEARNING_CD" type="radio" value="TAB_005" <c:if test="${view[0].FREE_TAB eq 'TAB_005'}">checked='checked'</c:if></input></span></li>
						<li>합격설명회 <span><input name="LEARNING_CD" type="radio" value="TAB_006" <c:if test="${view[0].FREE_TAB eq 'TAB_006'}">checked='checked'</c:if></input></span></li>
						<li>보강동영상 <span><input name="LEARNING_CD" type="radio" value="TAB_007" <c:if test="${view[0].FREE_TAB eq 'TAB_007'}">checked='checked'</c:if></input></span></li>				
					</c:when>
					<c:otherwise>
...
<%-- 		<tr>
		...
		<tr>
        	...
        	<c:if test="${item.CODE eq view[0].CATEGORY_CD}"><c:set var="CHECKSET" value="checked='checked'" /></c:if>			
	        	...
	        	<input name="SUBJECT_OPTION_ARR" type="checkbox" value="4" ${SOACHECKSET6} /><span class="txt05">동영상개설(500k)</span>
...
function fn_Submit(){
	...
	if(confirm("강의를 등록하시겠습니까?")){
		...
		$("#frm").attr("action","<c:url value='/lecture/copySave.do' />");
		$("#frm").submit();
	}
}
