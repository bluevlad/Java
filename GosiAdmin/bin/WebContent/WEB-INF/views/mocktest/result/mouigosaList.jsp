...
function goList(page) {
    ...
    $('#searchFrm').attr('action','<c:url value="/mock/result/mouigosaList.do"/>').submit();
}

function fn_View(val){
    $("#MOCKCODE").val(val);
    $("#searchFrm").attr("action", "<c:url value='/mock/result/mockResultView.do' />");
    $("#searchFrm").submit();
}
</script>
</head>
...
<form id="searchFrm" name="searchFrm" method="get" onsubmit="fn_checkEnter()">
    ...
    <table class="table02">
        ...
        <tr>
            <th scope="col">
            <div class="item">
              <input id="allCheck" class="i_check" value="" type="checkbox" onclick="checkAll('allCheck', 'MOCKCODE')" />
            </div>
            </th>
            <th scope="col">No</th>
            <th scope="col">년</th>
        ...
        <c:if test="${not empty list}">
          <c:forEach items="${list}" var="list" varStatus="status">
            <tr>
                <td><div class="item">
                <input class="i_check" name="MOCKCODE"  value="${list.MOCKCODE }" type="checkbox" />
                </div></td>
                ...
                <td>${list.NAME}</td>
                <td style="text-align:left;"><a href="javascript:fn_View('${list.MOCKCODE}')">${list.MOCKNAME}</a></td>
                <td>
