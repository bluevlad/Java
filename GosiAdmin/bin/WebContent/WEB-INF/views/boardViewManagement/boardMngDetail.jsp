...
<input type="hidden" id="BOARD_TYPE" name="BOARD_TYPE" value="${detail.BOARD_TYPE}" />	
<input type="hidden" id="DETAIL_BOARD_SEQ" name="DETAIL_BOARD_SEQ" value="${detail.BOARD_SEQ}" />	
...
<!--//테이블--> 
    	...
    	<tr>
    		<th >게시판명</th>
   			<td>
   				<div class="item" >
	   				<input type="text" id="BOARD_NAME" name="BOARD_NAME"  title="레이블 텍스트" value="${detail.BOARD_NAME}" size="20"  maxlength="20" style="width:28% ; background:#FFECEC;" />
				</div>
   			</td>
   		</tr>
   		<tr>
   			<th>게시판 타입</th>
  			<td>
				${detail.BOARD_TYPE_NM}
  			</td>
  		</tr>
  		<c:if test="${detail.BOARD_TYPE =='BOARD'}">
  			<tr id="display_1">
	   			<th>공개,비공개 기능</th>
	   			<td>
						...
						<span>(☞ 게시물 등록시 [공개,비공개] 활성 여부에 대한 설정 입니다.)</span> 
	   			</td>
	   		</tr>
	   		<tr id="display_2">
	   			<th>답변여부</th>
	   			<td>
	   					...
	   					<select  id="REPLY_YN" name="REPLY_YN">
							<option value="Y" <c:if test="${detail.OPEN_YN == 'Y'}">selected</c:if> >활성</option>
							<option value="N" <c:if test="${detail.OPEN_YN == 'N'}">selected</c:if>>비활성</option>
						</select>
   		...
   		<!-- 게시판 타입이 NOTICE 일때 -->
   		<c:if test="${detail.BOARD_TYPE =='NOTICE' or detail.BOARD_TYPE =='FAQ'}">
	   		<tr id="display_3">
	   			<th>첨부파일여부</th>
	   			<td>
	   					<label for="ATTACH_FILE_YN"></label>
