...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/room/roomOrderList.do"/>').submit();
}

//엑셀
function excel_onclick1() { //ck 추가 	
	$('#searchFrm').attr('action','<c:url value="/room/roomOrderList_excel.do"/>').submit();
}
...
function fn_view(orderNo){
	...
	$("#searchFrm").attr("action","<c:url value='/room/roomOrderView.do' />");
	$("#searchFrm").submit();
}
...
<!--content -->
  <div id="content">
	<h2>● 독서실관리 &lt; <strong>독서실신청</strong></h2>
        ...
        <tr>
		  ...
		  <select id="ORDERSTATUSCODE" name="ORDERSTATUSCODE" onchange="goList(1)" style="width:120;">
           ...
           <select name="SEARCHTYPE" id="SEARCHTYPE">
			...
			<option value="TOO.PHONE_NO" <c:if test="${params.SEARCHTYPE == 'TOO.PHONE_NO'}">selected</c:if>>핸드폰번호</option>
		  </select>
         ...
         <c:if test="${not empty list}">
          <c:forEach items="${list}" var="list" varStatus="status">
             <tr>
				...
				<td style="text-align:left;"><a href="javascript:fn_view('${list.ORDERNO}')">${list.ROOM_NM} ${list.ROOM_NUM}번 독서실 주문</a></td>				
				<td>${list.PRICE}</td>
