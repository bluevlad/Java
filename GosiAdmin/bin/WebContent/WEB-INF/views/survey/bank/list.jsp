...
<c:import url="/WEB-INF/views/survey/tab.jsp" />
    ...
    <table class="table02">
		...
		<tr>
		    ...
		    <td><a href="javascript:view('${list.QUEID}');">${list.QUETITLE}</a></td>
		    <td>${list.QUENM}</td>
		    <td>${list.QUECOUNT}</td>
		    <td>${list.ISUSE}</td>
		</tr>
...
function goList(page) {
	...
	$('#searchFrm').attr('action','<c:url value="/survey/bank/list.do"/>').submit();
}

//등록
function fn_Reg() {
	$('#searchFrm').attr('action','<c:url value="/survey/bank/write.do"/>').submit();
}

//상세
function view(no) {
	$("#QUEID").val(no);
	$('#searchFrm').attr('action','<c:url value="/survey/bank/modify.do"/>').submit();
}
</script>
