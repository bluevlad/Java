...
function plus_subject_pop() {
	...
	window.open('<c:url value="/freeOrder/pop_subject_add.pop"/>?userid=' + $("#userid").val() + '&keyword=' + escape(encodeURIComponent(keyword))
...
function save(){
	...
	if ($('#package_no').val() == "" || $('#package_no').val() == null) {
		//alert("1");
    	$('#popFrm').attr('action','<c:url value="/freeOrder/pop_add.pop"/>').submit();
	}else{
		//alert("2");
...
function save_submit(){
	...
	if ($('#isChk').val() == "Y") { //이미 인증을 되었을경우
		if ($('#package_no').val() == "" || $('#package_no').val() == null) {
			...
			$('#popFrm').attr('action','<c:url value="/freeOrder/pop_subject_add_insert.do"/>').submit();
		}else{
        ...
        $.ajax({
            type : "GET", 
            url : '<c:url value="/freeOrder/chkIP"/>',
            cache : false,
            success : function(data, textStatus, jqXHR) {
                if(data=="Y"){
        			$('#popFrm').attr('action','<c:url value="/freeOrder/pop_subject_add_insert.do"/>').submit();
                } else if(data =="N"){
                    alert("새로운 아이피로 접속되었습니다. 개인정보 인증을 진행해주세요.");
                	fn_sms();
                    return;
                }
            },
...
function fn_sms(){
   	window.open('<c:url value="/freeOrder/sendSMS"/>', '_loginFrm', 'location=no,resizable=yes,width=400,height=200,top=0,left=0,scrollbars=yes,location=no');
}
	 ...
	 <table class="table05">
    	<tr>
   		...
   		<tr>
    		<th rowspan="3">강의명</th>
   			<td class="tdLeft">
				강의번호 : <input type="text" id="leccode" name="leccode"  title="레이블 텍스트"  style="width:30% ;background:#FFECEC;" value="${searchMap.leccode}" readonly="readonly"/>
   			</td>
   		</tr>

   		<tr>
   			<td class="tdLeft">
				<input type="text" id="textfield5" name="textfield5"  title="레이블 텍스트" style="width:30% ;background:#FFECEC;" value="${searchMap.subject_title}" readonly="readonly"/>
   		...
   		<tr>
    		<th rowspan="3">금액</th>
   			<td class="tdLeft">
				실 수강료  : <fmt:formatNumber value="${searchMap.subject_price}" groupingUsed="true" /> 원
   			</td>
   		</tr>

   		<tr>
   			<td class="tdLeft">
				할인율  :  <input type="text" id="subject_discount" name="subject_discount"  title="레이블 텍스트"  style="width:10% ;background:#FFECEC;" value="${searchMap.subject_discount}"/>%
   			</td>
   		</tr>

   		<tr>
   			<td class="tdLeft">
				할인가  :  <input type="text" id="subject_price_temp" name="subject_price_temp"  title="레이블 텍스트"  style="width:10% ;background:#FFECEC;" value="${searchMap.subject_movie}"/>원
   			</td>
   		</tr>
   		...
   		<tr>
    		<th rowspan="5">변경옵션</th>
   			<td class="tdLeft">
	   			<c:if test="${searchMap.leccode ne ''}">
					<select style="width:100px;"   id="wmv_check" name="wmv_check">
						<option value="VOD" <c:if test="${searchMap.subject_option == 'VOD' or searchMap.subject_option == ''}">selected</c:if>>VOD</option>
						<option value="PMP" <c:if test="${searchMap.subject_option == 'PMP'}">selected</c:if>>PMP</option>
						<option value="VOD+PMP" <c:if test="${searchMap.subject_option == 'VOD+PMP'}">selected</c:if>>VOD+PMP</option>
						<option value="MP4" <c:if test="${searchMap.subject_option == 'MP4'}">selected</c:if>>MP4</option>
