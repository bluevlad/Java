...
<input type="hidden" id="LEC_PROCESS" name="LEC_PROCESS" value="">
    ...
    <table class="table01">
		<tr>
			...
			<td>
	            ...
	            <label for="SEARCH_SUBJECT_ISUSE"></label>
	           	<select name="SEARCH_SUBJECT_ISUSE" id="SEARCH_SUBJECT_ISUSE">
					<option value="">--개설여부--</option>
					<option value="Y" <c:if test="${params.SEARCH_SUBJECT_ISUSE == 'Y'}">selected="selected"</c:if>>개설</option>					
					<option value="N" <c:if test="${params.SEARCH_SUBJECT_ISUSE == 'N'}">selected="selected"</c:if>>폐강</option>					
				</select>
	            <label for="SEARCH_LEC_PROCESS"></label>
	           	<select name="SEARCH_LEC_PROCESS" id="SEARCH_LEC_PROCESS">
					<option value="">--접수상태--</option>
					<option value="Y" <c:if test="${params.SEARCH_LEC_PROCESS == 'Y'}">selected="selected"</c:if>>접수중</option>					
					<option value="N" <c:if test="${params.SEARCH_LEC_PROCESS == 'N'}">selected="selected"</c:if>>접수마감</option>					
				</select>
			</td>
			<th>강의개강일</th>
			<td>
    ...
    <ul class="lecArrary">
      ...
      <li><input id="SEARCHYEAR" name="SEARCHYEAR" type="checkbox" value="Y" <c:if test="${params.SEARCHYEAR == 'Y' }">checked="checked"</c:if>>2011년도 이후강의만 보기 / 접수관리 사용자 화면은 차후 적용예정입니다.</li>
    </ul>
    
    <ul class="boardBtns">
	    <li><a href="javascript:fn_SelPayListPop();">선택된 강의 수강인원</a></li>
        <li><a href="javascript:fn_OpenClose('Y');" class="Btn_02">개설</a></li>
        <li><a href="javascript:fn_OpenClose('N');" class="Btn_02">폐강</a></li>
        <li><a href="javascript:fn_LecProcess('Y');" class="Btn_03">접수중</a></li>
        <li><a href="javascript:fn_LecProcess('N');" class="Btn_03">접수마감</a></li>
    ...
    <ul class="boardBtns">
        ...
        <li><a href="javascript:fn_LecProcess('Y');">접수중</a></li>
        <li><a href="javascript:fn_LecProcess('N');">접수마감</a></li>
   	  	<li><a href="javascript:fn_Reg();">등록</a></li>
   	  	<li><a href="javascript:fn_Excel();">Excel</a></li>
    </ul>
...
function fn_LecProcess(param){
	...
	$('#rd-popup').bPopup();
	//체크 안된건 value 값 삭제
	$("#LEC_PROCESS").val(param);
	$("#frm").attr("action", "<c:url value='/lectureOff/multiUpdateProcess.do'/>");
	$("#frm").submit();
}
...
function fn_SeqPop(type){
	window.open('<c:url value="/lectureOff/seqList.pop"/>?LEC_TYPE_CHOICE=${params.LEC_TYPE_CHOICE}', '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1040,height=670');
}
...
<div id="rd-popup" style="visibility:hidden;"></div>