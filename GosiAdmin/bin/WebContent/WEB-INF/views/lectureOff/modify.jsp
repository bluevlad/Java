...
<!--content -->
<div id="content">
<form name="frm" id="frm" method="post" enctype="multipart/form-data" saction="">
...
<input type="hidden" id="COPENDATE" name="COPENDATE" value="${view[0].SUBJECT_OPEN_DATE}"/>
<input type="hidden" id="CLECS" name="CLECS" value="${view[0].LEC_SCHEDULE}"/>

<c:set var="WEEKCHK" value="" />
<c:if test="${view[0].WEEK1 eq 'Y' }"><c:set var="WEEKCHK" value="Y," /></c:if>
<c:if test="${view[0].WEEK1 ne 'Y' }"><c:set var="WEEKCHK" value="," /></c:if>

<c:if test="${view[0].WEEK2 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y," /></c:if>
<c:if test="${view[0].WEEK2 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}," /></c:if>

<c:if test="${view[0].WEEK3 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y," /></c:if>
<c:if test="${view[0].WEEK3 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}," /></c:if>

<c:if test="${view[0].WEEK4 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y," /></c:if>
<c:if test="${view[0].WEEK4 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}," /></c:if>

<c:if test="${view[0].WEEK5 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y," /></c:if>
<c:if test="${view[0].WEEK5 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}," /></c:if>

<c:if test="${view[0].WEEK6 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y," /></c:if>
<c:if test="${view[0].WEEK6 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}," /></c:if>

<c:if test="${view[0].WEEK7 eq 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}Y" /></c:if>
<c:if test="${view[0].WEEK7 ne 'Y' }"><c:set var="WEEKCHK" value="${WEEKCHK}" /></c:if>
...
<input type="hidden" id="CDATE" name="CDATE" value="${WEEKCHK}"/>
    ...
    <table class="table02">
		...
		<c:forEach items="${viewlist}" var="item" varStatus="loop">
			<tr <c:if test="${item.LECCODE eq view[0].LECCODE}">style='background:#FFECEC;'</c:if>>
		        ...
		        <td><a href="javascript:fn_PayListPop('${item.LECCODE}','${item.BRIDGE_LECCODE}');">${item.OFFERERCNT}</a></td>        
		        ...
		        <td>
					<c:if test="${item.LEC_PROCESS == 'Y'}">접수중</c:if>
					<c:if test="${item.LEC_PROCESS == 'N'}">접수마감</c:if>
		        </td>
			</tr>
		</c:forEach>		 	
	</table>      
			...
			<td>
          	...
          	<td>
				...
				<c:forEach items="${subjectteacherlist}" var="item" varStatus="loop">
					...
					<li>${item.SUBJECT_NM}(${item.USER_NM}) <span><input name="SUBJECT_INFO_ARR" type="radio" value="${item.SUBJECT_CD}#${item.USER_ID}#${item.OFF_PAYMENT}" ${CHECKSET}/></span></li>
				</c:forEach>
          		</ul>
          	</td>
        </tr>
	...
	<table class="table01">
		...
		<tr>
			<th width="160">강의명</th>
        	<td><input type="text" id="SUBJECT_TITLE" name="SUBJECT_TITLE" value="${view[0].SUBJECT_TITLE}" size="120" maxlength="333" style="background:#FFECEC;"/> "<strong class="txt02">강의명</strong>"에 특수문자 5개는 입력하시수 없습니다.[<span class="txt04"> : + % \ ' </span>]</td>
		</tr>
		<tr>
        	<th>강사료지급률</th>
					...
					<c:forEach items="${viewbooklist}" var="item" varStatus="loop">
						...
						<c:set var="CHECKBOOKSET" value="Y" />
							<tr>
								...
								<td>${item.BOOK_NM}</td>
	                            <td>
								<textarea id="SUBJECT_BOOK_MEMO_${item.RSC_ID}" name="SUBJECT_BOOK_MEMO_${item.RSC_ID}" rows="2" maxlength="1000" style="width:95%;background:#FFECEC;">${item.SUBJECT_BOOK_MEMO}</textarea>
 	                           </td>
		...
		<tr>
			<th>실강료</th>
			<td><input type="text" id="SUBJECT_REAL_PRICE" name="SUBJECT_REAL_PRICE" value="${view[0].SUBJECT_REAL_PRICE}" size="10" maxlength="7" style="background:#FFECEC;IME-MODE:disabled;" onKeyDown="fn_OnlyNumber(this);"/> 원</td>
		</tr>				
		<tr>
			<th>사진</th>
			<td>
		...
		<tr>
			<th>회차</th>
			<td>
				총 <input type="text" id="LEC_SCHEDULE" name="LEC_SCHEDULE" value="${view[0].LEC_SCHEDULE}" size="10" maxlength="3" style="background:#FFECEC;IME-MODE:disabled;" onKeyDown="fn_OnlyNumber(this);"/> 회 강의입니다.
				&nbsp;&nbsp;[수강생 화면 제공 회차] <input type="text" id="LEC_COUNT" name="LEC_COUNT" value="${view[0].LEC_COUNT}" size="10" maxlength="3" style="background:#FFECEC;IME-MODE:disabled;" onKeyDown="fn_OnlyNumber(this);"/> 회
				<span class="txt03"># 사용자 수강신청 강의리스트에 별로 표기될 정보를 입력해 주세요.</span>
			</td>
		</tr>
		...
		<tr>
        	...
        	<td>
        		<input type="radio" name="SUBJECT_ISUSE" value="Y" <c:if test="${view[0].SUBJECT_ISUSE eq 'Y' }">checked="checked"</c:if> /><span class="txt03">개설</span>
          		<input type="radio" name="SUBJECT_ISUSE" value="N" <c:if test="${view[0].SUBJECT_ISUSE eq 'N' }">checked="checked"</c:if> /><span class="txt04">폐강</span>
          	</td>
      </tr>
		<tr>
			<th>접수상태</th>
        	<td>
				<input type="radio" name="LEC_PROCESS" value="Y" <c:if test="${view[0].LEC_PROCESS eq 'Y' }">checked="checked"</c:if> /><span class="txt03">접수중</span>
          		<input type="radio" name="LEC_PROCESS" value="N" <c:if test="${view[0].LEC_PROCESS eq 'N' }">checked="checked"</c:if> /><span class="txt04">접수마감</span>
          	</td>
		</tr>
      <tr>
...
function fn_Submit(flag){
	...
	if(confirm(MSG)){
		...
		$("#frm").attr("action","<c:url value='/lectureOff/update.do' />");
		$("#frm").submit();
	}
}
...
function fn_Delete(){
	if("${lectureOrderCount}"!="0"){
		if(confirm("이 강의를 주문한 내역이 존재합니다. 정말 삭제하시겠습니까?")){
			$("#frm").attr("action","<c:url value='/lectureOff/delete.do' />");
			$("#frm").submit();
		}		
	}else{
...
// 복제
function fn_Copy(){
	$("#frm").attr("action","<c:url value='/lectureOff/copy.do' />");
	$("#frm").submit();
}

//신규복제
function fn_NewCopy(){
	$("#frm").attr("action","<c:url value='/lectureOff/newCopy.do' />");
	$("#frm").submit();
}

function fn_PayListPop(leccode, bridge_leccode){
	window.open('<c:url value="/lectureOff/payList.pop"/>?SEARCHOPENPAGE=L&LECCODE=' + leccode + '&BRIDGE_LECCODE=' + bridge_leccode , '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1040,height=670');
}
