...
function goList(page) {
	...
	$("#frm").attr("action", "<c:url value='/lectureOff/bookList.pop' />");
	$("#frm").submit();
}
	...
	if($("input[name='ADD_ARR']:checked").length < 1){
		...
		$("input[name='ADD_ARR']:checked").each(function(idx,el){
			...
			if(checkVal == "Y"){
				...
				appendStr += "<td><textarea id=\"SUBJECT_BOOK_MEMO_"+$(this).next().val()+"\" name=\"SUBJECT_BOOK_MEMO_"+$(this).next().val()+"\" rows=\"2\" maxlength=\"1000\" style=\"width:95%;background:#FFECEC;\"></textarea></td>";
