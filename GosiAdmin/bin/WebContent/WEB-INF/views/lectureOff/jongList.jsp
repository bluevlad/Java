	...
	<table class="table02">
		...
		<c:forEach items="${list}" var="item" varStatus="loop">
			<tr class="vitality">
		        ...
		        <td><a href="javascript:fn_PayListPop('${item.LECCODE}');">${item.USR_CNT}</a></td>
...
function fn_SeqPop(type){
	window.open('<c:url value="/lectureOff/seqList.pop"/>?LEC_TYPE_CHOICE=${params.LEC_TYPE_CHOICE}', '_blank', 'scrollbars=no,toolbar=no,resizable=yes,width=1040,height=670');
}
...
function fn_PayListPop(leccode){
	window.open('<c:url value="/lectureOff/payList.pop"/>?LEC_TYPE=JONG&LECCODE=' + leccode , '_blank', 'scrollbars=yes,toolbar=no,resizable=yes,width=1040,height=670');
}
