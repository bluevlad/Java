			...
			if(json && json.length > 0) {
				$(json).each(function(index, mouigosa){			
					...
					if(mouigosa.LIST_YN == 'Y') {
						...
						if(mouigosa.LECCODE.substring(0,1) == 'D') {
							TYPE = "Y"
							$("#"+target).append('<tr><td style=text-align:left;>' + mouigosa.SUBJECT_TITLE + '</td><td style=text-align:right;>' + mouigosa.SUBJECT_REAL_PRICE + '</td><td style=text-align:right;><span id="REAL_TOTAL_PRICE' + index + '">' + mouigosa.PRICE + '</span></tr>');
						}
					}
				});	
			}
...
function goup(LECCODE) {
	...
	window.open('<c:url value="/productOrder/pop_price_change.pop"/>?LECCODE=' + LECCODE
			 ...
			 + '&paycode=' + $("#paycode").val() + '&sdate=' + $("#sdate").val() + '&edate=' + $("#edate").val()
			 + '&currentPage=' + $("#currentPage").val() + '&pageRow=' + $("#pageRow").val()
			 + '&TOP_MENU_ID=' + $("#TOP_MENU_ID").val() + '&MENUTYPE=' + $("#MENUTYPE").val() + '&L_MENU_NM=' + $("#L_MENU_NM").val(), '_blank', 'location=no,resizable=yes,width=800,height=200,top=0,left=0,location=no,scrollbars=no');
}

$(function() {
	setDateFickerImageUrl('<c:url value='/resources/img/common/icon_calendar01.png'/>');
	initDatePicker1("REG_DT");
	$('img.ui-datepicker-trigger').attr('style','margin-left:5px; vertical-align:middle; cursor:pointer;');
});

$(function() {
	setDateFickerImageUrl('<c:url value='/resources/img/common/icon_calendar01.png'/>');
	initDatePicker1("DUE_DT");
	$('img.ui-datepicker-trigger').attr('style','margin-left:5px; vertical-align:middle; cursor:pointer;');
});
...
function checkParam() {
	...
	if(sts == "I") {
		$('#searchFrm').attr('action','<c:url value="/productOrder/registrationInsert.do"/>').submit();
	}else{
		$("#TYPE").val(TYPE);
		$('#searchFrm').attr('action','<c:url value="/productOrder/registrationUpdate.do"/>').submit();
	}
}
...
<input type="hidden" id="TYPE" name="TYPE" value="" />
	...
	<h2>● 수강신청관리 > <strong>수강신청</strong></h2>	
    	...
    	<tr>
   		...
   		<tr>
    		<th >품목<br><input type="button" onclick='javascript:product_change();' value="품목변경" /></th>
   		...
   		<tr>
    		<th >관리자 추가할인</th>
   			<td >
				...
				&nbsp;
				<input name="PRICE_DISCOUNT_TYPE" id="PRICE_DISCOUNT_TYPE2" value="C" type="radio" onchange="radio_chk();" <c:if test="${searchMap.PRICE_DISCOUNT_TYPE == 'C' }">checked="checked"</c:if>/>금액(원)
				&nbsp;
   				<input type="text" id="PRICE_DISCOUNT" name="PRICE_DISCOUNT"  title="레이블 텍스트" value="0" style="width:10%; background:#FFECEC;" onkeyup="chk(this,'PRICE_DISCOUNT');" onblur="chk(this,'PRICE_DISCOUNT');"/>
