...
<%--                	<c:if test="${status.index eq 0 }"> --%>
              ...
              <td>
              	<c:choose>
					...
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV125'}">
						<c:set var="statuscodename" value="발송완료"/>
					</c:when>
				</c:choose>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="12">
      <tr>
        <td align="left" bgcolor="#FFFFFF"><p>▣ 결제 정보&#13;</p></td>
      </tr>
    </table>
	...
	<tr>
		...
		<td >
			...
			<fmt:formatNumber value="${tot_price}" groupingUsed="true" />
		</td>
		...
		<td >
			...
			<c:choose>
				...
				<c:when test="${list.PAYCODENAME eq 'PAY130'}">
					<c:choose>
						<c:when test="${list.VACCT eq '' or list.VACCT eq null}">
							${list.VACCT}
						</c:when>
