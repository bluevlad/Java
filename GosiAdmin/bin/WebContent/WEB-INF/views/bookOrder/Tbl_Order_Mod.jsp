...
function submit_onclick() 
{
	if(confirm("변경내역을 수정하시겠습니까?")) {		
		...
		$('#myform').attr('action','<c:url value="/bookOrder/insert.do"/>').submit();
	}
}
...
function wmv_pmp_modify()
{
	...
	if(confirm("상품을 변경 하시겠습니까?")) {
		$('#myform2').attr('action','<c:url value="/bookOrder/wmv_pmp_update.do"/>').submit();
	}
}
...
/* 	alert("강사 정보 팝업 추후개발");
	return;
	 ...
	 window.open('<c:url value="/bookOrder/view.pop"/>?leccode=' + leccode, '_blank', settings); */
}

// 금액수정 버튼 - 20130118 각각의 판매가 마다 금액수정을 만들기 위해 추가됨
function submit_money(i){
	
	if(document.myform2.realprice.value == null || document.myform2.realprice.value == ""){
		alert("금액을 입력해 주세요");
		return;
    }
	...
	if(confirm("상품을 변경 하시겠습니까?")) {
		...
		$('#myform3').attr('action','<c:url value="/bookOrder/submit_money.do"/>').submit();
	}
}
...
function refund_money_delete(i){
	...
	if(confirm("상품을 삭제 하시겠습니까?")) {
		...
		$('#myform3').attr('action','<c:url value="/bookOrder/refund_money_delete.do"/>').submit();
	}
}
...
function goList() {
	if(sts == "ju"){
		$('#myform2').attr('action','<c:url value="/bookOrder/ju_list.do"/>').submit();
	}else if(sts == "jic"){
		$('#myform2').attr('action','<c:url value="/bookOrder/jic_list.do"/>').submit();
	}else{
		$('#myform2').attr('action','<c:url value="/bookOrder/tol_list.do"/>').submit();
	}
...
<!--content -->
  <div id="content">
	<h2>● 교재배송관리 &gt; <strong>상세조회</strong></h2>
	...
	<input type="hidden" id="sts" name="sts" value="${searchMap.sts}" />
    ...
    <table class="table02">
        ...
        <c:if test="${not empty lec_list}">
         ...
         <input type="hidden" name="mgntno" id="mgntno${status.index}" value="${lec_list.MGNTNO}"> <!-- 20130118 상품코드 하나하나 마다 id값 다르게 하기위해 뒤에 i값을 주었음 -->
            ...
            <tr>
              ...
              <td>
              	<a href="javascript:view('${lec_list.MGNTNO}')">${lec_list.NAME}</a>&nbsp;&nbsp;&nbsp;
               	${lec_list.SDATE}
              </td>
              
              <td>
              	<c:if test="${fn:substring(lec_list.MGNTNO, 0, 1) ne  'L' and status.index eq 0 }"> <!-- 교재가 아닌것만 링크 걸리게 수정 ck  -->
              ...
              <td><fmt:formatNumber value="${lec_list.PRICE}" groupingUsed="true" />원<br />(<fmt:formatNumber value="${discount_pay}" groupingUsed="true" />)</td>
              <td>${lec_list.RATE}%</td>
              <td>${lec_list.DISCOUNTPER}%</td>
                          
              <td>
               	<c:if test="${status.index eq 0 }">
	              	<input type="text" name="realprice" id="realprice${status.index}" value="${lec_list.REALPRICE}" size="8" onkeyup="chk(this,'${status.index}')" onblur="chk(this,'${status.index}')"><br />
              ...
              <td><fmt:formatNumber value="${lec_list.TOT_PRICE}" groupingUsed="true" />원</td>
              
              <td>  
              	<c:choose>
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV100'}">
						<c:set var="statuscodename" value="무통장입금"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV105'}">
						<c:set var="statuscodename" value="입금완료"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV110'}">
						<c:set var="statuscodename" value="배송준비중"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV120'}">
						<c:set var="statuscodename" value="배송중"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV130'}">
						<c:set var="statuscodename" value="배송완료"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV140'}">
						<c:set var="statuscodename" value="취소요청"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV150'}">
						<c:set var="statuscodename" value="취소완료"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV160'}">
						<c:set var="statuscodename" value="교환요청"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV170'}">
						<c:set var="statuscodename" value="교환배송중"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV180'}">
						<c:set var="statuscodename" value="교환완료"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV220'}">
						<c:set var="statuscodename" value="환불요청"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV230'}">
						<c:set var="statuscodename" value="환불완료"/>
					</c:when>
					
					<c:when test="${lec_list.STATUSCODENAME eq 'DLV240'}">
		...
		<tr>
          ...
          <td colspan="5" align="right"><fmt:formatNumber value="${tot_sum}" groupingUsed="true" /></td>
        </tr>
    ...
    <table style="width:100%;">
      <tr>
        <td align="left" bgcolor="#FFFFFF"><p>▣ 주문자 정보</p></td>
      </tr>
    </table>
    ...
    <!--//주문자정보 테이블--> 
   	...
   	<tr>
   		<th width="14%">주문번호</th>
		<td width="20%">
			<input type="text" id="orderno" name="orderno"  title="레이블 텍스트"  size="30"  maxlength="30" style="width:70%; background:#FFECEC;" value="${searchMap.orderno}" readonly />
		</td>
    ...
    <!--//배송지 테이블--> 
   	...
   	<tr>
   		<th width="14%">송장번호</th>
		<td width="43%">
			<input type="text" id="sendno" name="sendno"  title="레이블 텍스트"  size="100"  maxlength="100" style="width:30%; background:#FFECEC;" value="${del_list.SENDNO}" />
		</td>
		
		<th width="13%">수령인</th>
		<td width="30%">
			<input type="text" id="d_name" name="d_name"  title="레이블 텍스트"  size="100"  maxlength="100" style="width:50%; background:#FFECEC;" value="${del_list.USERNAME}" />
		</td>
	</tr>
	<tr>
   		<th >전화</th>
		<td >
			<input type="text" id="d_telno" name="d_telno"  title="레이블 텍스트"  size="100"  maxlength="100" style="width:30%; background:#FFECEC;" value="${del_list.TELNO}" />
		</td>
		
		<th >휴대폰</th>
		<td >
			<input type="text" id="d_cellno" name="d_cellno"  title="레이블 텍스트"  size="100"  maxlength="100" style="width:50%; background:#FFECEC;" value="${del_list.CELLNO}" />
		</td>
	</tr>
	...
	<tr>
   		<th >주소</th>
		<td>
			 <%-- <input type="text" name="zipcd1" id="zipcd1" style="width:10%;" value="${del_list.ZIPCD_SET1}" readonly="readonly">
			 -
			 <input type="text" name="zipcd2" id="zipcd2" style="width:10%;" value="${del_list.ZIPCD_SET2}" readonly="readonly"> --%>
	         ...
	         <br>
	         <input type="text" name="juso" id="juso" style="width:95%;" value="${del_list.ADDR}">
		</td>
	...
	<tr>
   		<th >메모</th>
		<td >
			<textarea rows="3" cols="50" name="d_memo" id="d_memo"><c:out value='${del_list.MEMO}'/></textarea>
		</td>
   		<th >배송일</th>
		<td style="vertical-align:middle;">
			${del_list.SENDDATE}
    ...
    <!--//결제 정보 테이블--> 
   	<table class="table01">
   	<tr>
   		<th width="14%">총구매금액</th>
		<td width="20%">
			<fmt:formatNumber value="${list.PRICE}" groupingUsed="true" />원
		</td>
		
		<th width="13%">추가금액(택배)</th>
		<td width="20%">
			<fmt:formatNumber value="${list.ADDPRICE}" groupingUsed="true" />원
			
			<c:if test="${first_zipcd eq '69' and first_zipcd ne null or first_zipcd ne ''}">
	...
	<tr>
   		<th >총지불금액</th>
		<td >		
			<c:set var="tot_price" value="${list.PRICE + list.ADDPRICE - list.POINT + chuga_add}"/>
		...
		<td >
			...
			${paycodename}
       		...
       		<c:if test="${list.PAYCODENAME == 'PAY120'}">
	...
	<tr>
   		<th >입금자명</th>
		<td colspan="5">
			<input type="text" id="payname" name="payname"  title="레이블 텍스트"  size="20"  maxlength="20" style="width:20%; background:#FFECEC;" value="${list.PAYNAME}" />
		</td>
	</tr>
	<tr>	
		<th >TID</th>
