...
function fn_Submit(){
	...
	*/
	...
	if(confirm("운영자를 등록하시겠습니까?")){
//		$("#USER_PWD").val(Base64.encode($("#USER_PWD").val()));
		$("#frm").attr("action","<c:url value='/memberAdminManagement/memberAdminInsertProcess.do' />");
		$("#frm").submit();
	}
}
function fn_List(){
...
<input type="hidden" id="L_MENU_NM" name="L_MENU_NM" value="${L_MENU_NM}" />
	...
	<h2>● 회원관리 > 관리자조회 > <strong>관리자 등록</strong></h2>	
    	<table class="table01">
   		<tr>
   			<th width="20%">회원ID</th>
  			<td>
	   			<input type="text" id="USER_ID" name="USER_ID" value="" size="20"  maxlength="13" title="회원아이디" style="width:18%;background:#FFECEC;" onKeyUp="checkData(this);"/>
	   			<input name="input" type="button" onClick="fn_IdCheck();" value="중복체크"> ☞ ID는 영문소문자[a~z], 숫자[0~9], 특수문자[ '_', '-' ] 포함하여 4자이상 13자 이하로 구성하세요
  			</td>
  		</tr>  		
   		<tr>
   			<th>성명</th>
  			<td><input type="text" id="USER_NM" name="USER_NM" value="" size="20"  maxlength="25" title="성명" style="width:28%;background:#FFECEC;"/></td>
  		</tr>  		
   		<tr>
   			<th>별명</th>
  			<td><input type="text" id="USER_NICKNM" name="USER_NICKNM" value="" size="20"  maxlength="25" title="별명" style="width:28%;background:#FFECEC;"/></td>
  		</tr>  		
   		<tr>
   			<th>비밀번호</th>
  			<td>
   		...
   		<tr>
   			<th>전화번호</th>
  			<td>
	   			<input type="text" id="PHONE_NO" name="PHONE_NO" value="" size="20"  maxlength="80" title="전화번호" style="width:28%; background:#FFECEC;"/>
  			</td>
  		</tr>
   		<tr>
   			<th>이메일</th>
  			<td>
	   			<input type="text" id="EMAIL" name="EMAIL" value="" size="20"  maxlength="80" title="이메일" style="width:28%; background:#FFECEC;"/>
  			</td>
  		</tr>
		<tr>
   			<th>회원권한</th>
  			<td>
  				<select  id="ADMIN_ROLE" name="ADMIN_ROLE">
					<c:forEach items="${categoryList}" var="item" varStatus="loop">
	   					<option value="${item.SITE_ID}">[${item.SITE_ID}]${item.SITE_NM}</option>
	   				</c:forEach>
   				</select>
  			</td>
  		</tr>		
		<tr>
  			...
  			<td>
  				<select id="USER_POSITION" name="USER_POSITION">
   					...
   					<option value="COP">경찰</option>
   					<option value="GWJ">광주</option>
  		...
  		<tr>
   		...
   		<tr>
   			<th>메모</th>
  			<td>
	   			<textarea id="MEMO" name="MEMO" ROWS="3" maxlength="4000" style="width:98%;"></textarea>
  			</td>
  		</tr>
	</table>
	<!--//테이블--> 
