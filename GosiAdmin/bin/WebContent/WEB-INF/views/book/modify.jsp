   	...
   	<table class="table01">
		...
		<tr>
   		...
   		<tr>
   			<th>목차</th>
  			<td>
	   			<textarea id="BOOK_CONTENTS" name="BOOK_CONTENTS" ROWS="7" onkeyup="fn_GetLen(1);" style="width:68%;">${view[0].BOOK_CONTENTS}</textarea>
  			</td>
  		</tr>  		
   		<tr>
   			<th>메모(목차2)</th>
  			<td>
	   			<textarea id="BOOK_MEMO" name="BOOK_MEMO" ROWS="7" onclick="fn_GetLen(2);" style="width:68%;">${view[0].BOOK_MEMO}</textarea>
  			</td>
  		</tr>		
		<tr>
   			<th>도서가격</th>
...
function fn_GetLen(n){
	var size = 0;
	if(n==1){
		for ( var i = 0; i < $("#BOOK_CONTENTS").val().length; i++) {
			size += this.charByteSize($("#BOOK_CONTENTS").val().charAt(i));
			if(size>3990){
				alert("목차 내용이 초과되었습니다.");
				return;
			}
		}
	}else{
		for ( var i = 0; i < $("#BOOK_MEMO").val().length; i++) {
			size += this.charByteSize($("#BOOK_MEMO").val().charAt(i));
			if(size>3990){
				alert("메모(목차2) 내용이 초과되었습니다.");
				return;
			}
		}
	}
