...
<input type="hidden" id="EVENT_NO" name="EVENT_NO" value="${ params.EVENT_NO }"/>
    	...
    	<table class="table01">
		      ...
		      <tr>
		        ...
		        <td class="tdLeft">
					<input type="text" id="START_DATE" name="START_DATE" size="10" readonly="readonly" value="${ view.START_DATE }">
		      ...
		      <tr>
		        <th>종료일</th>
		        <td class="tdLeft">
					<input type="text" id="END_DATE" name="END_DATE" size="10" readonly="readonly" value="${ view.END_DATE }">
		      ...
		      <tr>
		        <th width="10%">
		        	<input type="radio" id="CONTNT_GUBUN" name="CONTNT_GUBUN" value="IMG" 
						checked="checked" onclick="javascript:contntEvent();"/><br/>이미지</th>
		        <td class="tdLeft">
		            ...
		            <ul class="bannerFile" id="contentsImg">
						<c:if test="${!empty view.CONTENTS_IMG}" >
							<li>
								${fn:substring(view.CONTENTS_IMG, fn:indexOf(view.CONTENTS_IMG, '/')+1, fn:length(view.CONTENTS_IMG))}
								<a href="javascript:deleteImage('contentsImg');"> X </a>
							</li>
		        ...
		        <td class="tdLeft">
		            ...
		            <ul class="bannerFile" id="issuThumImg">
						<c:if test="${!empty view.ISSUE_THUMBNAIL}" >
							<li>
								${fn:substring(view.ISSUE_THUMBNAIL, fn:indexOf(view.ISSUE_THUMBNAIL, '/')+1, fn:length(view.ISSUE_THUMBNAIL))}
								<a href="javascript:deleteImage('issuThumImg');"> X </a>
							</li>
							<li><img src="<c:url value="/imgFileView.do?path=${view.ISSUE_THUMBNAIL}"/>" width="150" height="214"></li>
						</c:if> 
		            </ul>		        	
				</td>
		      </tr>
		  ...
		  <br/>
		  ● 옵션 1 [선택리스트설정] <input type="checkbox" id="OPTION1" name="OPTION1" value="Y" onclick="javascript:fn_Display();" 
		...
		<br/>
		<!--//테이블--> 
    	● 옵션 2 [댓글기능설정] <input type="checkbox" id="OPTION2" name="OPTION2" value="Y" onclick="javascript:fn_Display();"
			...
			<c:if test="${ view.OPTION2_YN ne 'Y' }">style="display: none;"</c:if>>
		      ...
		      <tr>
		        ...
		        <td class="tdLeft tdValign">
		        	...
		        	<input type="checkbox" id="OPTION2_EVENT" name="OPTION2_EVENT" value="${view.OPTION2_EVENT }"  onclick="javascript:fn_Option2Chk();" <c:if test="${view.OPTION2_EVENT eq 'Y' }">checked="checked"</c:if>/>이벤트 페이지 &nbsp;&nbsp;&nbsp;
		        	<input type="checkbox" id="OPTION2_POPUP" name="OPTION2_POPUP" value="${view.OPTION2_POPUP }"  onclick="javascript:fn_Option2Chk();" <c:if test="${view.OPTION2_POPUP eq 'Y' }">checked="checked"</c:if>/>바로신청(팝업)페이지
		        </td>
		      </tr>
		</table>
		<br/>
		...
		<br/>
		● 옵션 4 [바로신청(팝업)설정] <input type="checkbox" id="OPTION4" name="OPTION4" value="Y" onclick="javascript:fn_Display();" <c:if test="${ view.OPTION4_YN eq 'Y' }">checked="checked"</c:if>/> (선택시 노출)
		<table class="table01" id="option4Table" <c:if test="${ view.OPTION4_YN ne 'Y' }">style="display: none;"</c:if>>
		      <tr>
		        <th width="10%">팝업타이틀</th>
		        <td width="100%">
		        	<input type="text" id="POPUP_TITLE" name="POPUP_TITLE" value="${view.POPUP_TITLE}" style="width: 50%">
		        </td>
		      </tr>
		      <%-- <tr>
		        <th>팝업링크</th>
		        <td width="100%">
		        	<input type="text" id="POPUP_LINK" name="POPUP_LINK" value="${view.POPUP_LINK}"  style="width: 50%">&nbsp;&nbsp;<span style="color: blue;">(배너관리에서 해당 링크 적용)</span>
		        </td>
		      </tr> --%>
		</table>
