...
<input type="hidden" id="LECCODE" name="LECCODE" value="">
    ...
    <table class="table01">
		<tr>
			...
			<td colspan="3">
	            ...
	            <label for="SEARCHFORM"></label>
	           	<select name="S_LEC_BAESU" id="S_LEC_BAESU">
					<option value="">--배수검색--</option>
					<option value="00" <c:if test="${params.S_LEC_BAESU == '00'}">selected="selected"</c:if>>배수제한없음</option>					
					<option value="15" <c:if test="${params.S_LEC_BAESU == '15'}">selected="selected"</c:if>>1.5배수</option>					
					<option value="20" <c:if test="${params.S_LEC_BAESU == '20'}">selected="selected"</c:if>>2배수</option>					
					<option value="25" <c:if test="${params.S_LEC_BAESU == '25'}">selected="selected"</c:if>>2.5배수</option>					
					<option value="30" <c:if test="${params.S_LEC_BAESU == '30'}">selected="selected"</c:if>>3배수</option>					
	          </select>
			</td>
		</tr>
        <tr>
    ...
    <ul class="boardBtns">
    	<!-- <li><a href="javascript:fn_oldFreeToNewFree();">무료특강으로 마이그레이션-함부로 클릭 금지</a></li>
    	<li><a href="javascript:fn_oldBogangFreeToNewBogangFree();">보강으로 마이그레이션-함부로 클릭 금지</a></li> -->
	...
	<table class="table02">
		...
		<c:forEach items="${list}" var="item" varStatus="loop">
			...
			<tr id="TR_${item.BRIDGE_LECCODE}" <c:if test="${item.SUBJECT_ISUSE eq 'Y'}">class="vitality"</c:if>>
		        ...
		        <td class="txt03"><c:if test="${item.LEC_BAESU eq '00' }">배수제한없음</c:if><c:if test="${item.LEC_BAESU ne '00' }">${item.LEC_BAESU/10}배수</c:if></td>
...
function fn_oldFreeToNewFree(){
	// 예전 무료강좌를 현재 무료 강좌로 마이그레이션
	$.ajax({
		type : "GET",
		url : '<c:url value="/lecture/oldFreeToNewFree.do"/>',
...
function fn_oldBogangFreeToNewBogangFree(){
	// 예전 보강를 현재 무료 강좌로 마이그레이션
	$.ajax({
		type : "GET",
		url : '<c:url value="/lecture/oldBogangFreeToNewBogangFree.do"/>',
		contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		dataType : "json",
		async : false,
		cache : false,
