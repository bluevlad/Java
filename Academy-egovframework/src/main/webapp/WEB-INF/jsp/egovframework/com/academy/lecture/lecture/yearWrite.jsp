...
<input type="hidden" id="SUBJECT_OFF_OPEN_DAY" name="SUBJECT_OFF_OPEN_DAY" value=""/>
	...
	<table class="table01">
		<tr>
        	<th width="25%">직종</th>
			<th width="80%">
				<c:if test="${params.LEC_TYPE_CHOICE ne 'Y'}">필수과목</c:if>
				<c:if test="${params.LEC_TYPE_CHOICE eq 'Y'}">필수과목</c:if>
			</th>
		</tr>
        <tr>
			<td>
	...
	<table class="table01">
        		...
        		<table class="tdTable" id="ADDAREA2">
					<tr>
              		...
              		<tr class="basic_space">
              			<c:if test="${params.LEC_TYPE_CHOICE ne 'Y'}">
              				<td colspan="4">&nbsp;</td>
              			</c:if>
              			<c:if test="${params.LEC_TYPE_CHOICE eq 'Y'}">
              				<td colspan="5">&nbsp;</td>
		...
		<tr>
			<th>선택강사(선택갯수)</th>
			<td><input type="text" id="TEACHERNO" name="TEACHERNO" value="" size="8" maxlength="3" style="background:#FFECEC;IME-MODE:disabled;" onKeyDown="fn_OnlyNumber(this);"/> 개</td>
		</tr>		
		<tr>
        	<th>원가</th>
...
function fn_Submit(){
	...
	if($("input[name='MST_LECCODE']").length < 1){
 	...
 	if($.trim($("#SUBJECT_PRICE").val()) == ""){
 		if ($("#LEC_TYPE_CHOICE").val() != 'Y') {  // 선택형 종합반(임의대로)인 경우 원가를 입력하지 않는다.
	 		alert("원가를 입력하세요");
	 		$("#SUBJECT_PRICE").focus();
	        return;
 		}
	...
	if(confirm("강의를 등록하시겠습니까?")){
		...
		$("#frm").attr("action","<c:url value='/lecture/yearSave.do' />");
		$("#frm").submit();
	}
}
