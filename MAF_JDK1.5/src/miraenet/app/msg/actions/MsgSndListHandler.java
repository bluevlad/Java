 ...
 * 작성된 날짜 : 2005-01-25
 */
public class MsgSndListHandler extends SessionedAction   {
	...
	public String doWork(MyHttpServletRequest request,
		...
		int currentPage = MafUtil.parseInt(pageNum);//현재 페이지
		...
		count = msgmst.getSndCount(super.oDb, usn_snd);

		if (count > 0) {
			msgmstlist = msgmst.getMsgSndlist(super.oDb, usn_snd, pageSize, startRow);
		} else {
