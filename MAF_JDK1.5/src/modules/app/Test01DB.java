...
public class Test01DB {
	private static Test01DB  instance;
	private Log logger = LogFactory.getLog(Test01DB.class);
	
	private Test01DB() {
	}

	public static synchronized Test01DB  getInstance() {
		if (instance == null) instance = new Test01DB();
		return instance;
	}
	...
	public int updateOne(MdbDriver oDb, Map param) throws MdbException {
		final String sql = " update TEST_01 set "   +
			"id = :id  ,  " +
			"title = :title  ,  " +
			"contents = :contents  ,  " +
			"reg_dt = :reg_dt  ,  " +
	...
	public int insertOne(MdbDriver oDb, Map param) throws MdbException {
		final String sql = " INSERT INTO TEST_01"   +
		"    ( id, title, contents, reg_dt, stard_date, end_date, category, reg_nm)"   +
		"  VALUES"   +
		"    ( :id, :title, :contents, :reg_dt, :stard_date, :end_date, :category, :reg_nm)"  ;
