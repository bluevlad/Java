 ...
 *
 */
public class Exam_Manager extends AdminAction {
    ...
    public String doWork(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
        ...
        bank_setBean tBean = new bank_setBean();
        bank_exam_setBean eBean = new bank_exam_setBean();
        ...
        if(cmd.equals("list")) {
        ...
        else if(cmd.equals("set_list")){ //교과목에 등록된 문제은행세트 리스트를 보여준다.

            String sjtcode = _req.getP("sjtcode");
            SjtMstBean item = sdao.getSjtView(oDb, sjtcode);

            list = dao.getSetList(oDb, sjtcode);
        ...
        else if(cmd.equals("set")){ //교과목에 등록된 문제은행 세트를 구성한다..

            List list_set = null;

            String sjtcode = _req.getP("sjtcode");
            String set_no = _req.getP("set_no");
            SjtMstBean item = sdao.getSjtView(oDb, sjtcode);

            list_set = dao.getSetList(oDb, sjtcode);
            if ("".equals(set_no) || set_no == null){
                if(list_set.size() > 0) {
                    Map x = (Map) list_set.get(0);
                    set_no = x.get("set_no").toString();
                }
                else{
                    list_set = null;
                }
            }
            list = dao.getSetExamList(oDb, sjtcode, set_no);
            
            _req.setAttribute("item", item);
            _req.setAttribute("list", list);
            _req.setAttribute("list_set", list_set);
            _req.setAttribute("sjtcode", sjtcode);
            _req.setAttribute("set_no", set_no);
            _req.setAttribute("v_page", v_page+"");
            forward = "set";
        }
            ...
            String sjtcode = _req.getP("sjtcode");
            String chp_sel = _req.getP("chp_sel");
            String chapter = _req.getP("chapter");
            String sel_chapter = _req.getP("sel_chapter");
        ...
        else if(("set_ins").equals(cmd)){ //문제은행 세트를 구성한다.

            String sjtcode = _req.getP("sjtcode");
            int ret = 0;

            Setter setter = new Setter(tBean, _req);
            setter.setProperty("*");

            try {
                    tBean.setReg_id(usn);
                    ret = dao.insertSet(oDb, tBean);
            }   catch (Throwable e) {
            }
            
            if(ret > 0){
                forward = "message";
                cmdExt = cmdExt + "&cmd=set_list&sjtcode="+sjtcode;
                _req.setAttribute( "message", Message.INSERT_OK);
                _req.setAttribute( "next", super.control_action+cmdExt);
        ...
        else if(("set_pop").equals(cmd)) { // 문제은행 세트를 등록한다.

            String sjtcode = _req.getP("sjtcode");
            String set_no = _req.getP("set_no");
            String etype = _req.getP("etype");
            String chapter = _req.getP("chapter");
            List list_chp = null;
            page_size= 5;
            
            list = dao.getExamList(oDb, sort, v_page, page_size, sjtcode, etype, chapter, set_no);
            count = dao.getExamCount(oDb, sjtcode, etype, chapter, set_no);
            list_chp = dao.getChpList(oDb, sjtcode);
            ...
            _req.setAttribute("sjtcode", sjtcode);
            _req.setAttribute("etype", etype);
            _req.setAttribute("chapter", chapter);
            _req.setAttribute("set_no", set_no);
            _req.setAttribute("navigator", navigator);
            _req.setAttribute("list", list);
            _req.setAttribute("list_chp", list_chp);
        ...
        else if(("ins_set").equals(cmd)){ //문제은행 세트를 구성한다.

            String[] ids = _req.getParameterValues("ids");
            String sjtcode = _req.getP("sjtcode");
            int ret = 0;

            Setter setter = new Setter(eBean, _req);
            setter.setProperty("*");

            if(null != ids) {
                for(int i = 0; i < ids.length; i++) {
                    eBean.setReg_id(usn);
                    eBean.setExam_no(MafUtil.parseInt(ids[i]));
                    ret = dao.insertExamSet(oDb, eBean);
                }
            }
            
            if(ret > 0){
                forward = "message";
                cmdExt = cmdExt + "&cmd=set&sjtcode="+sjtcode;
                _req.setAttribute( "message", Message.INSERT_OK);
                _req.setAttribute( "next", super.control_action+cmdExt);
            ...
            if(ret>0){
                ...
                forward = "error";
                cmdExt = cmdExt + "&cmd=write&sjtcode="+sjtcode;
                _req.setAttribute( "message", "등록된 문제는 삭제할수 없습니다.");
                _req.setAttribute( "next", super.control_action+cmdExt);
            }            
        }
        else if(("exam_set_del").equals(cmd)) {  //문제은행 세트에 등록된 문제를 삭제한다.

            String[] ids = _req.getParameterValues("ids");
            String sjtcode = _req.getP("sjtcode");
            int set_no = _req.getIntParameter("set_no");
            ...
            if(null != ids) {
                try{
                    oDb.setAutoCommit(false);
                    for(int i =0; i < ids.length; i++) {
                        ret = dao.deleteExamSet(oDb, MafUtil.parseInt(ids[i]), sjtcode, set_no);
