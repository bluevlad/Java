...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr >
		<td class="td"><div style="width:650px;height:100px;overflow:auto;"><table border=0 cellspacing=0 cellpadding=1>
			<tr>
			<c:forEach items="${attItems}" var="data">	
                <mh:indexOf var="idx" value="${data.content_type}" needle="image"/>
				<c:if test="${idx >-1}">
					<c:url var="imgUrl" value="/pds/board/${MBOARD.bid}/thumb/${data.real_filename}"/>
		            <td width="100"><img src='<c:out value="${imgUrl}"/>' border="0" width="80" 
						style="cursor: pointer;" 
						onClick="mboard_popup(<c:out value="'${MBOARD.bid}', '${data.c_index}', '${data.seq}'"/>)"></td>
				</c:if>
			</c:forEach>
			</tr>
		</table></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr ><strong><mfmt:message bundle="board" key="title.board.uploadfile"/></strong>
		<td class="td"><table border="0" cellspacing="0" cellpadding="2">
			...
			<tr>
			<td>&nbsp;<a href="<c:out value="${imgUrl}"/>?cmd=save" target="_blank"><img src='<c:out value="${MBOARD.CPATH}"/>/images/paperclip.gif' alt="" border="0"></a></td>
