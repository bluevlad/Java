...
%>
<chart caption='Histogram' xAxisName='Score' yAxisName='peoples' showValues='0' decimals='0' formatNumberScale='0'> <c:forEach
    var="item" items="${list}">
    <c:choose>
        <c:when test="${item.chk=='T'}"><c:set var="color" value="FF9C00"/></c:when>
        <c:otherwise><c:set var="color" value="9CCE31"/></c:otherwise>
    </c:choose>
    <set label='~<c:out value="${item.sc_range}"/>' value='<c:out value="${item.cnt}"/>' color='<c:out value="${color}"/>'/>
