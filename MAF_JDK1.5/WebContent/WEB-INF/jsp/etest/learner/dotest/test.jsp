...
<table width="850" border="0" cellspacing="0" cellpadding="0"
    style="height:26px;color:#E6E6E6;background:#595959;border-bottom:1px solid #000000;">
    <tr>
        <td>
...
<table width="850" height="46" border="0" cellspacing="0" cellpadding="0" 
    style='background:url(<c:url value="/images/etest/top_bg.gif"/>) repeat-x;table-layout:fixed;'>
    <col width="755" />
    <col width="95" />
    <tr >
        ...
        <td><img src='<c:url value="/images/etest/navi_top_3.gif"/>' border="0" /></td>
    </tr>
</table>
</div>
<!-- div rl -->
<div id="div_navi"  onScroll="SetDivPosition()"><jsp:include page="_navi.jsp" flush="true" /></div>
<div id="content">
<form action="<%=request.getAttribute("controlaction")%>" method="post" name="myform" id="myform">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td valign="top"><input type="hidden" name="exmid" value="<%=request.getParameter("exmid") %>" /> <input type="hidden"
            name="pageno" value="" /> <input type="hidden" name="cmd" value="" /> <input type="hidden" name="ansqid" value="" />
        ...
        <table border="0" cellspacing='0' cellpadding='0' width="100%" bordercolor="#f0f0f0">
            ...
            <tr>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td align="center" ><span style="color:#F7941D; font-size:30px;"><%=qseq%><a name="q_<%=qseq%>"/></span><br>
                        Mark <input type="checkbox" name="m_<%=qseq%>" value="Y" onClick="toggleMark(this,'<%=qseq%>')"
                            <%if("Y".equals(rec.get("mark_yn"))) { out.print ("CHECKED"); } %> ></td>
                    </tr>
                </table>
                </td>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    %>
                    <tr>
                        <td><input type="<%=iType%>" id="<%=ansId%>" name="<%=ansId%>" value="<%=j%>"
                            <% if(view.isViewCheck(j)) {out.print("CHECKED");} %> onClick="sendAns('<%=ansId%>','<%=qseq%>')" />&nbsp;<%=view.getView(j)%></td>
                    </tr>
                    <%
                    		}
                    ...
                    %>
                    <tr>
                        <td><input type="text" name="<%=ansId%>" value="<%=view.getUserAnswer() %>"
                            onChange="sendAns('<%=ansId%>','<%=qseq%>')"></td>
                    </tr>
                    <%
                    	}
            ...
            <td colspan="3" align="center"> 
                <c:choose>
                    <c:when test="${cur_page > 1 }">
                        <mf:button bundle="button" key="button.exm.previous" onclick="goPage('${cur_page-1 }')"/>
                    </c:when>
                ...
                <c:choose>
                    <c:when test="${cur_page < rstinfo.maxpage }">
                        <mf:button bundle="button" key="button.exm.next" onclick="goPage('${cur_page+1 }')"/>
                    </c:when>
                    <c:otherwise></c:otherwise>
                </c:choose>
                </td>
