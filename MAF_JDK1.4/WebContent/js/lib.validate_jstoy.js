...
* 김상준 
	- 2006.10.09 firefox에서는 elValue가 String으로 익식 못하는 문제 발생 하여 String으로 새로 생성하여 사용 
	- 2006.10.11 alphanum 옵션 추가 
***********************************************
*/
...
{
    try {
		var FormCheckerObject = new FormChecker;
...
FormChecker.prototype.validate = function(form) {
    ...
    for (var i=0, s=form.elements.length; i<s; i++) {
		...
		// 2006.10.09 김상준 
		// firefox에서는 elValue가 String으로 익식 못하는 문제 발생 하여 String으로 새로 생성하여 사용 
		// var elValue = form.getValue(el.name);
        //var elValue = String(form.getValue(el.name));
		var elValue = String(el.value);
...
FormChecker.prototype.func_alphanum = function(el) {
