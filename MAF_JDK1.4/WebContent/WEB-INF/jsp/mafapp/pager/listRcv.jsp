...
<!--
	...
	function doDelete(){
	    ...
	    if(frm) {
			...
			if(chkcnt == 0 ) {
			    ...
			    frm.${mrt:mvcCmd()}.value = "deleteRcv"; 
			    frm.submit();
			}
		}
	}	
	function doView(msgid){
	    var frm = getObject("myform");
			if(frm) {
    		    frm.${mrt:mvcCmd()}.value = "msgViewRcv";
...
<table width="100%" border="0" cellspacing="0" cellpadding="2">
	...
	<input type="hidden" name="${mrt:mvcCmd()}" value="listRcv">
    ...
    <tr>
	    <td>
		    ...
		    <tr>
				...
				<th class="list ">No</th>
				<th class="list" nowrap><mform:header name="msg_snd_nm"/><mform:THSort id='msg_snd_nm'/></th>
				<th class="list" nowrap><mform:header name="msg_title"/><mform:THSort id='msg_title'/></th>
				<th class="list" nowrap><mform:header name="file_name"/></th>
				<th class="list BR" nowrap><mfmt:message bundle="mafpager" key="form.rcv_dt"/><mform:THSort id='msg_dt'/></th>
		    </tr>
			...
			<tr onMouseOver="tr_over(this)" onMouseOut="tr_out(this)">
				...
				<td class="list center">${mhtml:null2nbsp(item.msg_snd_nm)}&lt;${item.msg_snd_userid}&gt;</td>
				...
				<td class="list center">${mhtml:null2nbsp(item.file_name)}</td>
				<td class="list BR center">${mfmt:autoDateTime(item.msg_dt)}</td>
