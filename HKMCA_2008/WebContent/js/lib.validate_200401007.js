...
var FildBackColor;
/// 에러메시지 포멧 정의 ///
var NO_BLANK = "{name+을를} 입력하시기 바랍니다";
...
function validate(mfrm) {
	...
	for (i = 0; i < len ; i++ ) {
        ...
        //네스케이프에서 예외가 발생하여 try{}catch{}로 묶음 -- 2004.09.20일 김윤철
        try{
        ...
        if (el.getAttribute("REQUIRED") != null) {
			...
			if (el.name != "keyword" || mfrm.elements["sel_type"].value != "0"){	//전체 검색을 위해 추가
        ...
        if (maxbyte != null && el.value != "") {
            if (el.value.bytes() > parseInt(maxbyte)) {
                return doError(el,"{name}의 길이가 초과되었습니다 (최대 "+maxbyte+"바이트),"+el.value.bytes());
            }
        }
...
function isValidUserid(el) {
    ...
    return (pattern.test(el.value)) ? true : doError(el,"{name+은는} 5자이상 20자 미만이어야 하고,\n 영문,숫자, _ 문자만 사용할 수 있습니다");
}
...
// textarea의 내용 길이 체크 및 현재 입력된 문자수 표시
// param a	현재까지 입력된 값을 표시하는 부분의 id
