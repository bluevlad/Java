...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;" class=no_border>
    <tr>
        <td>
                ...
                <tr>
                    <th>
                                <c:if test="${navigator.startPage > navigator.screenSize}">
                                    ...
                                    src='<c:url value="/jmf_images/navigator/pre.gif"/>'></a>&nbsp;&nbsp;&nbsp;
                                </c:if>
                                    <c:forEach var="i" begin="${navigator.startPage}" end="${navigator.endPage}">
                                        <c:choose>
                                        <c:when test="${i == navigator.currentPage}">
                                            <c:if test="${i%10 != 1}">|</c:if>&nbsp;&nbsp;<c:out value="${i}"/>&nbsp;
                                        </c:when>
                                        <c:otherwise>
                                            <c:if test="${i%10 != 1}">|</c:if>&nbsp;&nbsp;<a href='javascript:mnavi_goPage(<c:out value="${i}"/>)'><c:out value="${i}"/></a>&nbsp;
                                        </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                <c:if test="${navigator.endPage < navigator.pageCount}">
                                    &nbsp;
                                    <a href='javascript:mnavi_goPage(<c:out value="${navigator.startPage + navigator.screenSize}"/>)'><img
                                    src='<c:url value="/jmf_images/navigator/next.gif"/>'></a>
                                </c:if>
                    </th>
                </tr>
                <tr>
