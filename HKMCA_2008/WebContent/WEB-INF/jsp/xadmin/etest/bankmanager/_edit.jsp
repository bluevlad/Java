...
<table width="100%" border="0" cellpadding="2" cellspacing="0" class="view"> 
	...
	<col width="80%"/>
    <tr>
        <th><mfmt:message bundle="table.exm_bnk" key="quetitle"/></th>
        <td><textarea name="quetitle" style="width:98%" rows='2' required><c:out value="${item.quetitle}"/></textarea></td>
    </tr>
    <tr>
        <th><mf:label name="quetext"/></th>
        <td><textarea name="quetext" style="width:98%" rows='3' hname="<mf:header name="quetext"/>"><c:out value="${item.quetext}"/></textarea></td>
    </tr>
    <tr>
        <th><mfmt:message bundle="table.exm_bnk" key="queimag"/></th>
        <td>
            <input type="file" name="queimag" style="width:98%" value=''>
    ...
    <tr style='<c:out value="display:${Sview}"/>' id='<c:out value="SviewTEXT${status.count}"/>'>
        ...
        <td colspan="2"><mf:textarea cssStyle="width:98%" rows="3" name="Squeviw${status.count}" value="${viw}"/></td>
    </tr>
    <!-- multi select -->
    <tr style='<c:out value="display:${Mview}"/>' id='<c:out value="MviewTEXT${status.count}"/>' >
        ...
        <th ><input type="checkbox" name="queansw_Mcheck" value='<c:out value="${status.count}"/>' <c:if test="${t >= 0}">checked</c:if> />
        	<mfmt:message bundle="table.exm_bnk" key="queviw${status.count}"/></th>
        <td colspan="2"><textarea style="width:98%" rows=3 name='<c:out value="Mqueviw${status.count}"/>' ><c:out value="${viw}"/></textarea></td>
    </tr>
    </c:forEach>
    <tr style="display:'none'" id="answer">
        <th><mf:label name="queansw"/></th>
        <td colspan="2"><mf:textarea cssStyle="width:98%" rows="3" name="queansw_word" value="${item.queansw}"/></td>
    </tr>
    <tr>
        <th><mf:label name="quedesc"/></th>
        <td colspan="2"><mf:textarea cssStyle="width:98%" rows="3" name="quedesc" value="${item.quedesc}"/></td>
    </tr>    
