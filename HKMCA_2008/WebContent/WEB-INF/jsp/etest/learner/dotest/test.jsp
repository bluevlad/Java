...
<script language="javascript">
    ...
    function doFin(sel){
        var sw = true;    
        if(sel==1){
            if(!confirm('<jfmt:message bundle="common" key="table.message.exm.end"/>')){
                sw=false;
            }    
         }else if(sel==2){
            alert('<jfmt:message bundle="common" key="table.message.exm.timeout"/>');
         }    
        if(sw){
            sendFinish() ;
        }
...
<div id="rl">
<table width="850" border="0" cellspacing="0" cellpadding="0" style="height:26px;color:#E6E6E6;background:#595959;border-bottom:1px solid #000000;">
    <tr>
        <td>
...
<table width="850" height="46" border="0" cellspacing="0" cellpadding="0" style='background:url(<c:url value="/images/etest/top_bg.gif"/>) repeat-x;table-layout:fixed;'>
    <col width="755" />
    <col width="95" />
    <tr >
            ...
            <td colspan="3" align="center"> 
                <c:choose>
                    <c:when test="${cur_page > 1 }">
                        <jf:button bundle="button" key="button.exm.previous" onclick="goPage('${cur_page-1 }')"/>
                    </c:when>
                ...
                <c:choose>
                    <c:when test="${cur_page < rstinfo.maxpage }">
                        <jf:button bundle="button" key="button.exm.next" onclick="goPage('${cur_page+1 }')"/>
                    </c:when>
                    <c:otherwise></c:otherwise>
                </c:choose>
                </td>
