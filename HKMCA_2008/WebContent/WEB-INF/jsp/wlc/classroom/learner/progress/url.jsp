...
<!--
function resize() {
    var w = <mf:out value="${v_width}"/>+20;
    var h = <mf:out value="${v_height}"/>+150;
...
// 경로 및 파라미터를 생성.xxx
var LMSServer = '<mf:out value="${lmsserver}"/>';   
var LMSExecName = '<mf:out value="${lmsservlet}"/>';
var lec_cd = '<mf:out value="${lec_cd}"/>';
var usn = '<mf:out value="${usn}"/>';
var item_id = '<mf:out value="${itm_id}"/>';
...
function ClickHandler() {    
    ...
    if (e.button == 2 || e.button == 3 || e.keyCode == 93) {
        window.alert("<mf:out value="${m1}" escapeJS="true"/>");
    } else if (e.ctrlKey || e.shiftKey || e.altKey) {
        window.alert("<mf:out value="${m2}" escapeJS="true"/>");
    }   
}
...
var save_interval="<mf:out value="${save_interval}"/>";
...
function saveTime(flag) {
    ...
    if(flag=='E') {
        var sendParameter   = "?cmd=save_time"
                            ...
                            + "&lec_cd="   +lec_cd
                            + "&usn="    +usn
                            + "&itm_id=<mf:out value="${citem.itm_id}"/>"
...
<table width=100% border=0 cellspacing=0 cellpadding=0>
    <tr>
        <td>
            <iframe id="Viewer_Frame" width=100% height=<mf:out value="${v_height}"/> frameborder=0 scrolling=yes src="<mf:out value="${citem.htmurl}"/>" marginwidth=0 marginheight=0></iframe>
        </td>
    </tr>
</table>