    ...
    <tr>
        <td align="right">
            ...
            <c:when test="${item.rst_status == 'R'}">
                <mf:button bundle="button" key="etest.start" onclick="eWlcTestGo('${item.exmid}','${item.lec_num}');" icon="icon_add" />
            </c:when>
            <c:when test="${item.rst_status == 'F'}">
                <mf:button bundle="button" key="etest.viewResult" onclick="viewResult('${item.exmid}');" icon="icon_edit" />
            </c:when>
            </c:choose>
