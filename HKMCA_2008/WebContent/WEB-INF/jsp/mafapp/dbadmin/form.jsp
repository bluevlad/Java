	...
	<tr>
		<td valign="top"><table border="0" cellpadding="2" cellspacing="0" class="table">	
			...
			<c:forEach items="${columns}" var="item">
			  <tr>
			    <th class="th" nowrap>${(item.nullable=="Y")?"":"<span id='lbl_required'>*</span>"}${empty(item.comments)?fn:toLowerCase(item.name) : mhu:null2nbsp(item.comments)}</th> 
			    <td class="td"><input type="text" name="${fn:toLowerCase(item.name)}" 
			        value="${mhu:stripQuote(item.data_default)}" 
			        size="20" maxlength="${item.dlength}"
			        hname="${empty(item.comments)?fn:toLowerCase(item.name) : mhu:null2nbsp(item.comments)}"
	...
	<tr>
		...
		<td><textarea cols="80" rows="25" name="frmins">
			...
			<c:forEach items="${columns}" var="item">  <tr>
			    <th class="th" nowrap>${(item.nullable=="Y")?"":"<span id='lbl_required'>*</span>"}${empty(item.comments)?fn:toLowerCase(item.name) : mhu:null2nbsp(item.comments)}</th> 
			    <td class="td"><input type="text" name="${fn:toLowerCase(item.name)}" value="${mhu:stripQuote(item.data_default)}" size="20" maxlength="${item.dlength}"
			        hname="${empty(item.comments)?fn:toLowerCase(item.name) : mhu:null2nbsp(item.comments)}" ${(item.nullable=="Y")?"":"required"} ${(item.type=="NUMBER")?"option='number'":""} ></td>
			  </tr>
			</c:forEach>
				<tr>
					<td colspan="2" align="center"><input type="submit" value="저장"><input type="reset" value="취소"></td>
...
&lt;form action="\${control_action}" method="post" name="myform" id="myform"&gt;
    ...
    &lt;tr&gt;
      <c:forEach items="${columns}" var="item">&lt;td class="th" align=center &gt;${mhu:null2nbsp(item.comments)}&lt;/td&gt;
      </c:forEach>				
    &lt;/tr&gt;
