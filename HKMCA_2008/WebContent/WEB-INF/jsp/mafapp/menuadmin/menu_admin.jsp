        ...
        <td>
			...
			<c:forEach var="item" items="${sites}">
                <option value='<c:out value="${item.site}"/>' 
                	<c:if test="${item.site == site}">selected</c:if>><c:out value="${item.site_title}"/>&nbsp;[<c:out value="${item.site}"/>]</option>
			</c:forEach>
            </select>
...
<tr >
	...
	<td height="550">
	   <iframe src='<c:url value="${control_action}" >
	   				...
	   				<c:param name="site" value="${site}"/></c:url>' name="frmMenuEdit" id="frmMenuEdit" width="100%" height="100%" marginwidth="0" marginheight="0"></iframe>
	</td>
</tr>
</table>
