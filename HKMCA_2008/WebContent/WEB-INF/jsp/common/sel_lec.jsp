...
<script>
    function os_init() {
        ...
        if (dest == "os_lec"){
            pageAJAXComm.getLec(getObject('os_lec'),'os_lec','<%=request.getParameter("os_sjt")%>');
        } else if (dest == "os_sjt"){
...
//            alert(serverData);
            ...
            for( var i=0; i < evalData.list.length; i++ ) {
               ...
               option.text = evalData.list[i].nm;
               k.options.add(option);
               if(option.value == '<%=request.getParameter("os_lec")%>') {
                   option.selected = true;
                   var obj = getObject("os_sjt");
                   pageAJAXComm.getLec(obj,"os_sjt",'<%=request.getParameter("os_crs")%>');
                }
               if(option.value == '<%=request.getParameter("os_sjt")%>') {
                   option.selected = true;
                   var obj = getObject("os_crs");
                   pageAJAXComm.getLec(obj,"os_crs",'');
                }
    ...
    Event.observe(window, 'load', os_init);
            ...
            <select name="os_crs" onChange="pageAJAXComm.getNew(this,'os_sjt')">
            <option value=''>-</option>
            </select>
            <select name="os_sjt" onChange="pageAJAXComm.getNew(this,'os_lec')">
            <option value=''>-</option>
            </select>
            <select name="os_lec">
            <option value=''>-</option>
