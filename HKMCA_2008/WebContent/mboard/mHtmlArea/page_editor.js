...
this.btnList = {
    ...
    "justifyleft":    ['JustifyLeft',          '좌측 정렬',       'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        '가운데 정렬',     'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         '우측 정렬',      'editor_action(this.id)',  'ed_align_right.gif'],
    ...
    "insertimage":    ['InsertImage',          '이미지 추가/수정',       'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          'Table 추가',       'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'HTML 소스보기',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '크게 보기',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    ...
    //"cu_resetimgsize":   ['cu_ResetImgSize', '이미지 크기 원래대로',  'editor_action(this.id)',  'recalc_image_size.gif'],
   // end: custom buttons
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "extend/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
