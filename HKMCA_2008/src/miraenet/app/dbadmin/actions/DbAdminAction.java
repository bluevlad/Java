...
public class DbAdminAction extends MafAction {
    ...
    public void preProcess(MyHttpServletRequest _req, HttpServletResponse _res)
        throws MafException {
	        ...
	        if (oa == null) {
	            new MafException("Manager init Fail!!!");
	        }
	        this.owner = _req.getParameter("owner", "UBQ");
...
//	        doWork(_req, response);
			...
			this.packageName = _req.getP("packageName", MafUtil.nvl(CookieUtil.getValue(_req, "packageName"),"modules"));
			this.className = StringUtils.replace(_req.getP("className", StringUtils.capitalize(tabsname)),"_","");
