...
public abstract class MafBundleSupport extends BodyTagSupport {
	...
	public static LocalizationContext getLocalizationContext(PageContext pc, String basename) {
		...
		Locale pref = MafSetLocaleSupport.getLocale(pc, Config.FMT_LOCALE);
		if (pref != null) {
		...
		if (locCtxt == null) {
			...
			pref = MafSetLocaleSupport.getLocale(pc, Config.FMT_FALLBACK_LOCALE);
			if (pref != null) {
				bundle = findMatch(basename, pref);
				if (bundle != null) {
					locCtxt = new LocalizationContext(bundle, pref);
		...
		if (locCtxt != null) {
			...
			if (locCtxt.getLocale() != null) {
				MafSetLocaleSupport.setResponseLocale(pc, locCtxt.getLocale());
			}
		} else {
	...
	private static LocalizationContext findMatch(PageContext pageContext, String basename) {
		...
		for (Enumeration oEnum = pageContext.getRequest().getLocales(); oEnum.hasMoreElements();) {
			/*
			 ...
			 */
			Locale pref = (Locale) oEnum.nextElement();
