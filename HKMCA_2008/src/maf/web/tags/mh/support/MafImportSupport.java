...
import maf.web.tags.support.Util;
...
public class MafImportSupport extends BodyTagSupport implements TryCatchFinally, ParamParent {
	//*********************************************************************
	// Public constants
	/**
	 * 
	...
	private Log logger = LogFactory.getLog(MafImportSupport.class);
	...
	private MafParamSupport.ParamManager params; // parameters
	private String urlWithParams; // URL with parameters, if applicable

	//*********************************************************************
	// Constructor and initialization
	public MafImportSupport() {
		super();
		init();
	}
	...
	public int doStartTag() throws JspException {
		...
		urlWithParams = null;
		params = new MafParamSupport.ParamManager();
		// check the URL
	...
	/**
	 * debug 모드 이면 사용자 브라우져에 에러를 남기며 그렇지 않으면 throw만 발생 
	 */
	public void doCatch(Throwable t) throws Throwable {
		String errorMsg = maf.logger.Trace.getStackTrace(t);
		
		logger.error("import url in : " + targetUrl() + "\n"+ errorMsg);
		if (MafProperties.DEBUG) {
			JspWriter writer = pageContext.getOut();
			if (writer != null) {
				
					writer.print("<table border='1' background='#ffffff'>" +
							"<tr><td style='font-size:12px'><strong> mh:import error in !!! " + this.url + "</strong></td></tr>" + 
					        "<tr><td style='font-size:12px'>" + HTMLUtil.nl2br(errorMsg) + "</td></tr>" +
					        "</table>");
	...
	private String acquireString() throws IOException, JspException {
		if (isAbsoluteUrl) {
			...
			if (!(pageContext.getRequest() instanceof HttpServletRequest && pageContext.getResponse() instanceof HttpServletResponse)) throw new JspTagException(
																																									Resources
	...
	private Reader acquireReader() throws IOException, JspException {
		if (!isAbsoluteUrl) {
			...
			try {
				...
				Reader r = null;
				if (charEncoding != null && !charEncoding.equals("")) r = new InputStreamReader(i, charEncoding);
				else {
					String responseAdvisoryEncoding = uc.getContentEncoding();
					if (responseAdvisoryEncoding != null) r = new InputStreamReader(i, responseAdvisoryEncoding);
					else
	...
	private class ImportResponseWrapper extends HttpServletResponseWrapper {
		...
		public PrintWriter getWriter() {
			if (isStreamUsed) throw new IllegalStateException(Resources.getMessage("IMPORT_ILLEGAL_STREAM"));
			isWriterUsed = true;
		...
		public ServletOutputStream getOutputStream() {
			if (isWriterUsed) throw new IllegalStateException(Resources.getMessage("IMPORT_ILLEGAL_WRITER"));
			isStreamUsed = true;
			return sos;
		}
		...
		public String getString() throws UnsupportedEncodingException {
			if (isWriterUsed) return sw.toString();
				...
				if (charEncoding != null && !charEncoding.equals("")) return bos.toString(charEncoding);
				else
	...
	private String targetUrl() {
		if (urlWithParams == null) urlWithParams = params.aggregateParams(url);
	...
	public static boolean isAbsoluteUrl(String url) {
		...
		int colonPos;
		if ((colonPos = url.indexOf(":")) == -1) return false;
		// if we DO have a colon, make sure that every character
		// leading up to it is a valid scheme character
		for (int i = 0; i < colonPos; i++)
			if (VALID_SCHEME_CHARS.indexOf(url.charAt(i)) == -1) return false;
