 ...
 * 
 */
public class WlcSetManagerAction extends MafAction {
	...
	public void cmdList(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		WlcSetManagerDB.getList(super.oDb, navigator, param, wb.getWhereString(param), false);
	...
	public void cmdEdit(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		result.setAttribute("item", WlcSetManagerDB.getOne(super.oDb, param));
		result.setAttribute("bnk", WlcSetManagerDB.getQueMaxCnt(super.oDb, param));
		result.setAttribute("setitems", WlcSetManagerDB.getSetAllItems(super.oDb, param));
//		result.setAttribute("subject", LectureMngDB.getCrsSub(super.oDb, param));
//		result.setAttribute("CertiList", CourseMngDB.getCertiList(super.oDb, param));
	...
	public void cmdUpdate(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		try {
			...
			rcnt = WlcSetManagerDB.updateSetOne(super.oDb, param);
			rcnt = WlcSetManagerDB.deleteSetItems(super.oDb, param);
			...
			if("R".equals(param.get("exmtype"))){
				if(a_queids != null) {
					for(int i = 0; i < a_queids.length; i ++ ) {
						...
						rcnt = WlcSetManagerDB.insertSetItems(super.oDb, param);
					}
				}
			} else{
				if(v_queids != null) {
				...
				Map param2 = WlcSetManagerDB.getQueMaxCnt(super.oDb, param);
			...
			WlcSetManagerDB.updateMaxCnt(super.oDb, param);
			oDb.commit();
			chk = true;
		} catch (Exception e) {
			oDb.rollback();
	...
	public void cmdUpdateItems(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		try {
			...
			WlcSetManagerDB.updateMaxCnt(super.oDb, param2);
			oDb.commit();
			chk = true;
		} catch (Exception e) {
			oDb.rollback();
	...
	public void cmdInsert(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		try {
			oDb.setAutoCommit(false);
			rcnt = WlcSetManagerDB.insertOne(super.oDb, param);
			oDb.commit();
			chk = true;
		} catch (Exception e) {
			oDb.rollback();
	...
	public void cmdDelete(MyHttpServletRequest _req, HttpServletResponse _res) throws MafException {
		...
		try {
			oDb.setAutoCommit(false);
			rcnt = WlcSetManagerDB.deleteSetItems(super.oDb, param);
			rcnt = WlcSetManagerDB.deleteSet(super.oDb, param);
			oDb.commit();
			chk = true;
		} catch ( Exception e) {
			oDb.rollback();
