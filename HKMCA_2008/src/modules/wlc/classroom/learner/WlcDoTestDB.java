...
public class WlcDoTestDB extends BaseDAO {
	/**
...
//		final String sql = " SELECT   /*+INDEX(i pk_exm_rst_item)*/"   +
//			"          i.usransw, b.quetitle, b.quetext, b.queimag, b.queviw1, b.queviw2, b.queviw3, b.queviw4,"   +
//			"          b.queviw5, b.queviw6, b.queviw7, b.queviw8, b.queviw9, b.queviw10, b.quecount, b.quetype,"   +
//			"          b.quelevel, b.queid, i.qseq, i.mark_yn"   +
//			"     FROM exm_rst_item i, exm_bnk b"   +
//			"    WHERE i.exmid = :exmid AND i.usn = :usn and lecnumb=:lecnumb AND i.pageno = :pageno AND i.queid = b.queid"   +
//			" ORDER BY qseq"  ;
		final String sql = " SELECT   /*+INDEX(i pk_exm_rst_item)*/"   +
		"          i.usransw, "   +
		" 		 DECODE (bl.queid, NULL, b.quetitle, bl.quetitle) quetitle, "   +
		" 		 DECODE (bl.queid, NULL, b.quetext, bl.quetext) quetext, "   +
        "        DECODE (b.queimag, NULL, bl.queimag, b.queimag) queimag, "   +
	...
	public static int updateRst(MdbDriver oDb, Map param) throws MdbException {
		...
		final String sql_make = " INSERT INTO wlc_exm_rst_item"   +
		"             (exmid, lec_cd, usn, qseq, queid, quescore, pageno)"   +
		"    SELECT x.exmid, m.lec_cd, x.usn, x.RANK, x.queid, x.quescore, CEIL (common.divide (x.RANK, :npage)) page"   +
		"      FROM (SELECT m.exmid, i.setid, i.queid, i.qseq, b.quescore, b.quelevel, r.usn,"   +
		"                   ROW_NUMBER () OVER (ORDER BY DBMS_RANDOM.VALUE) RANK"   +
		"              FROM wlc_exm_set_item i, wlc_exm_bnk b, wlc_exm_set s, wlc_exm_mst m, wlc_exm_rst r"   +
		"             WHERE r.exmid = :exmid"   +
		"               AND r.usn = :usn"   +
		"               AND r.exmid = m.exmid"   +
		"               AND m.setid = s.setid"   +
		...
		final String sql_make_new_x = " INSERT INTO wlc_exm_rst_item"   +
		"             (exmid, lec_cd, qseq, queid, quescore, pageno, usn)"   +
		"    SELECT p.exmid, p.lec_cd, p.RANK, p.queid, p.quescore, CEIL (common.divide (p.RANK, p.exmpage)) page, :usn"   +
		"      FROM (SELECT d.*, ROW_NUMBER () OVER (ORDER BY DBMS_RANDOM.VALUE) RANK"   +
		"              FROM (SELECT m.exmid, m.lec_cd, i.setid, i.queid, i.qseq, b.quescore, b.quelevel,"   +
		...
		final String  sql_make_new = " INSERT INTO wlc_exm_rst_item"   +
		"             (exmid, lec_cd, qseq, queid, quescore, pageno, usn)"   +
		" SELECT p.exmid, p.lec_cd, p.RANK, p.queid, p.quescore, " +
		...
		"                                                    qseq " +
		"                                                   )) RANK " +
		"          FROM (SELECT m.exmid, m.lec_cd, i.setid, i.queid, i.qseq, b.quescore, " +
	...
	public static int setFin(MdbDriver oDb, Map param) throws MdbException {
		...
		" FROM wlc_exm_rst_item ri, wlc_exm_bnk b"   +
	...
	public static Map  getRstInfo(MdbDriver oDb, Map param) throws MdbException {
		...
		"   FROM wlc_exm_rst er, wlc_exm_mst em"   +
